...
def _validate_trainable_state_schema(torch: Any, payload: Mapping[str, Any]) -> bool:
    """Validate legacy U0 or the one authenticated four-surface U60 payload."""
    ...
    if step != 60 or checkpoint_sha != AUTHENTICATED_U60_CHECKPOINT_SHA256:
        raise ArtifactError("four-surface state requires authenticated UPDATE_060")
...
def _published_pre_recipe_policy(
    config: Mapping[str, Any], global_step: int, *, authenticated_u60_continuation: bool = False
