...
def test_resident_adam_uses_fp64_state_and_update_arithmetic():
    source = inspect.getsource(official.ModernGreenResidentEngine.__init__)
    optimizer_source = inspect.getsource(_fp64_state_adam)
    assert "_fp64_state_adam" in source
    assert 'dtype=torch.float64' in optimizer_source
    assert "gradient64" in optimizer_source
    assert "clip_grad" not in optimizer_source
    assert "foreach=False" not in source


def test_api_admits_only_exact_u2_to_u6_numerical_stability_probe():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 2" in source
    assert '== "adam-fp64-state-and-update-arithmetic"' in source
    assert "numerical_stability_resume and requested == (6,)" in source


def test_api_admits_only_exact_u6_to_u10_numerical_stability_continuation():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 6" in source
    assert '== "adam-fp64-state-and-update-arithmetic"' in source
    assert "numerical_stability_continuation and requested == (10,)" in source


def test_fp64_repair_uses_power_of_two_equivalent_gradient_scaling():
    source = inspect.getsource(official.ModernGreenResidentEngine)
    optimizer_source = inspect.getsource(_fp64_state_adam)
    assert "2.0 ** -192" in source
    assert "2.0 ** -96" not in source
    assert source.count("loss * self.equivalent_gradient_scale") == 2
    assert 'state["gradient_scale"] = gradient_scale' in optimizer_source
    assert "group[\"eps\"] * gradient_scale" in optimizer_source
    assert ".div_(gradient_scale)" not in optimizer_source
    assert "clip_grad" not in source


def test_api_admits_exact_u8_to_u10_numerical_stability_tail_recovery():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 8" in source
    assert "numerical_stability_tail_recovery and requested == (10,)" in source


def test_api_admits_exact_u10_to_u14_numerical_stability_tail_continuation():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 10" in source
    assert "numerical_stability_tail_continuation and requested == (14,)" in source


def test_api_admits_exact_u13_to_u14_numerical_stability_final_recovery():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 13" in source
    assert "numerical_stability_final_recovery and requested == (14,)" in source


def test_api_admits_exact_u14_to_u18_numerical_stability_boundary_continuation():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 14" in source
    assert "numerical_stability_boundary_continuation and requested == (18,)" in source


def test_api_admits_exact_u17_to_u18_numerical_stability_final_segment_recovery():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 17" in source
    assert "numerical_stability_final_segment_recovery and requested == (18,)" in source


def test_api_admits_exact_u18_to_u22_measured_gradient_calibration():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "start_update == 18" in source
    assert 'config.get("authorized_gradient_calibration")' in source
    assert '== "measured-pre-gradient-max-ratio-v1"' in source
    assert 'config.get("activation_checkpointing") is True' in source
    assert "step_time_boundary_continuation and requested == (22,)" in source


def test_api_admits_exact_u21_to_u22_post_load_gradient_calibration_recovery():
    source = inspect.getsource(api_module.ResidentRepairAPI.continue_two_spark_real)
    assert "gradient_calibration_final_recovery" in source
    assert "start_update == 21" in source
    assert "gradient_calibration_final_recovery and requested == (22,)" in source


def test_resident_engine_applies_only_exact_measured_gradient_lr_divisors():
    source = inspect.getsource(official.ModernGreenResidentEngine.__init__)
    assert '"luts": 4.6, "norms": 4.3, "outputs": 5.2' in source
    assert 'optimizer_lrs[name] / lr_calibration_divisors[name]' in source
    assert '"measured-pre-gradient-max-ratio-v1"' in source


def test_loaded_optimizer_scheduler_lr_calibration_is_post_load_and_idempotent():
    from types import SimpleNamespace

    engine = object.__new__(official.ModernGreenResidentEngine)
    engine.config = {
        "authorized_gradient_calibration": "measured-pre-gradient-max-ratio-v1",
        "gradient_lr_divisors": {"luts": 4.6, "norms": 4.3, "outputs": 5.2},
    }
    engine.optimizer = SimpleNamespace(param_groups=[
        {"lr": 1.0e-3, "initial_lr": 1.0e-3},
        {"lr": 1.0e-4, "initial_lr": 1.0e-4},
        {"lr": 1.0e-3, "initial_lr": 1.0e-3},
    ])
    engine.scheduler = SimpleNamespace(
        base_lrs=[1.0e-3, 1.0e-4, 1.0e-3],
        _last_lr=[7.320889187045983e-4, 7.320889187045983e-5, 7.320889187045983e-4],
    )

    engine._restore_measured_gradient_lr_calibration()
    first = (
        [group["lr"] for group in engine.optimizer.param_groups],
        list(engine.scheduler.base_lrs),
        list(engine.scheduler._last_lr),
    )
    assert first[0] == [1.0e-3 / 4.6, 1.0e-4 / 4.3, 1.0e-3 / 5.2]
    assert first[1] == [1.0e-3 / 4.6, 1.0e-4 / 4.3, 1.0e-3 / 5.2]
    assert first[2] == [7.320889187045983e-4 / 4.6, 7.320889187045983e-5 / 4.3, 7.320889187045983e-4 / 5.2]
    assert [group["gradient_lr_divisor"] for group in engine.optimizer.param_groups] == [4.6, 4.3, 5.2]
