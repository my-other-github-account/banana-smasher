...
def _published_pre_recipe_policy(
    ...
    *,
    ...
    authenticated_checkpoint = config.get("checkpoint_sha256")
    authenticated_u64 = (
        authenticated_u60_continuation
        and authenticated_checkpoint == AUTHENTICATED_U64_CHECKPOINT_SHA256
    )
    authenticated_u60 = (
        authenticated_u60_continuation
        and authenticated_checkpoint in {
            AUTHENTICATED_U60_CHECKPOINT_SHA256,
            AUTHENTICATED_U64_CHECKPOINT_SHA256,
        }
    )
    ...
    step = int(global_step)
    if not (0 <= step < 64 or (terminal_scheduler_cursor or authenticated_u64) and step == 64):
