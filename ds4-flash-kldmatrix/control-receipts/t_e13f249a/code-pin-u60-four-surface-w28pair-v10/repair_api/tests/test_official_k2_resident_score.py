...
class OfficialK2ResidentScoreTests(unittest.TestCase):
    def test_w28_full64_entrypoint_requires_complete_sealed_pair_batch(self):
        balanced = (28, 56)
        with self.assertRaisesRegex(ArtifactError, "complete aligned batch"):
            _physical_canary_batch_windows((28,), 4, balanced)
        self.assertEqual(
            _physical_canary_batch_windows((28,), 2, balanced),
            balanced,
        )
    ...
    def test_score_resume_round_trips_exact_binary64_terms_before_next_attempt(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                hashlib.sha256(Path(cast(str, source_file)).read_bytes()).hexdigest(),
            )
