...
def test_static_w28_eval_isolates_dynamic_cache_per_decoder_layer(monkeypatch) -> None:
    import sys
    import types
    import torch
    ...
    cache_utils.DynamicCache = FakeDynamicCache
    transformers.cache_utils = cache_utils
    monkeypatch.setitem(sys.modules, "transformers", transformers)
    monkeypatch.setitem(sys.modules, "transformers.cache_utils", cache_utils)

    class FakeLayer:
        def __call__(self, current, **kwargs):
            cache_objects.append(kwargs["past_key_values"])
            return current

    engine = ModernGreenResidentEngine.__new__(ModernGreenResidentEngine)
    engine.student = SimpleNamespace(
        config=SimpleNamespace(),
        model=SimpleNamespace(model=SimpleNamespace(layers=[FakeLayer(), FakeLayer()])),
    )
    engine.first = 0
    engine.last = 1
    engine.activation_checkpointing = False
    engine._positional = lambda ids, template, cache: (None, None, None)

    hidden = torch.zeros(1, 2, 1, 3)
    ids = torch.zeros(1, 2, dtype=torch.long)
    engine._run_layers(hidden, ids, False)
...
def test_published_pre_validation_uses_imported_sealed_builder_attention(monkeypatch) -> None:
    ...
    assert engine.sealed_builder_binding == {
        ...
        "provider_wrapper_sha256": "0d4ece20b602fc59ffef349183db2bea0861b4a7f7c0ef93e50fd728310e7371",
        "provider_expert_sha256": "fc612f7863ad9d09a9faf11e203a9d20739b7dbb273b982fc3d36ee01d15a9b4",
...
def test_published_pre_validation_can_select_supported_sdpa_mechanics(monkeypatch) -> None:
    import repair_api.official_k2_resident_score as sealed_builder_binding
    ...
    engine.config = {
        "resident_validation_proof": True,
        "recipe_id": "published_pre_lower_lr_warmup16_cosine64_v1",
        "resident_validation_attention_implementation": "sdpa",
    }
    engine.published_pre_recipe = True
    engine.corpus_path = Path("corpus")
    engine.teacher_root = Path("teacher")
    ...
    assert __import__("os").environ["BR_ATTN_IMPL"] == "sdpa"
    assert engine.sealed_builder_binding["attention_implementation"] == "sdpa"


def test_published_pre_validation_can_select_resident_packed_provider(monkeypatch) -> None:
    import repair_api.official_k2_resident_score as sealed_builder_binding

    monkeypatch.setattr(sealed_builder_binding, "_configured_attention_implementation", lambda _config: "eager")
    engine = ModernGreenResidentEngine.__new__(ModernGreenResidentEngine)
    engine.config = {
        "resident_validation_proof": True,
        "recipe_id": "published_pre_lower_lr_warmup16_cosine64_v1",
        "resident_validation_attention_implementation": "sdpa",
        "resident_validation_expert_implementation": "packed_cuda_bf16_boundary",
    }
    engine.published_pre_recipe = True
    engine.corpus_path = Path("corpus")
    engine.teacher_root = Path("teacher")
    ...
    engine.base = SimpleNamespace(T=SimpleNamespace(CKPT=None, DEV=None))
    engine.student = SimpleNamespace()
    engine.torch = SimpleNamespace(manual_seed=lambda _seed: None, cuda=SimpleNamespace(manual_seed_all=lambda _seed: None))

    engine._configure_base()

    assert __import__("os").environ["FAST_K2_SEALED_FULL_WEIGHT_BF16"] == "0"
    assert __import__("os").environ["FAST_K2_SEALED_PROJECTION_BF16"] == "1"
    assert engine.sealed_builder_binding["expert_implementation"] == "packed_cuda_bf16_boundary"
...
def test_static_w28_provider_matches_accepted_pre_artifact_and_route_state() -> None:
    ...
    assert hashlib.sha256(wrapper_path.read_bytes()).hexdigest() == (
        "0d4ece20b602fc59ffef349183db2bea0861b4a7f7c0ef93e50fd728310e7371"
    )
    assert hashlib.sha256(expert_path.read_bytes()).hexdigest() == (
        "fc612f7863ad9d09a9faf11e203a9d20739b7dbb273b982fc3d36ee01d15a9b4"
    )
...
def test_published_pre_full64_streams_exact_mb2_groups_without_changing_windows(
    ...
    corpus = [{"token_ids": [1] * 1024, "real_len": 1024} for _ in range(500)]
    corpus_path = tmp_path / "corpus.json"
    corpus_path.write_text(__import__("json").dumps(corpus))
    teacher_root = tmp_path / "teacher"
    teacher_root.mkdir()
...
def test_public_validate_delegates_to_the_existing_trainer_object(tmp_path: Path) -> None:
    ...
    trainer = Trainer()
    trainer.candidate_read_proof = {
        "candidate_artifact_sha256": "a" * 64,
        "candidate_checkpoint_reads": 1,
        "candidate_payload_reads": 43,
        "relative_l2": 0.125,
    }
