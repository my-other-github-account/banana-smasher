...
def test_published_pre_declaration_only_bypasses_for_validated_u60() -> None:
    config = {
        "recipe_id": PUBLISHED_PRE_RECIPE_ID,
        "checkpoint_sha256": AUTHENTICATED_U60_CHECKPOINT_SHA256,
        "lr_scale": 0.5,
    }
    with pytest.raises(ArtifactError, match="declaration drift"):
        _published_pre_recipe_policy(config, 60)
    _base_lrs, multiplier, windows = _published_pre_recipe_policy(
        config, 60, authenticated_u60_continuation=True
    )
    assert multiplier > 0.0
    assert windows == [28, 56]

    wrong = {**config, "checkpoint_sha256": "0" * 64}
    with pytest.raises(ArtifactError, match="declaration drift"):
        _published_pre_recipe_policy(
            wrong, 60, authenticated_u60_continuation=True
        )

    u64 = {
        **config,
        "checkpoint_sha256": "4cab94648b62e15dbcc0be4e3701c5c2b38af71f8662011455ba0312448da624",
    }
    base_lrs, _multiplier, _windows = _published_pre_recipe_policy(
        u64, 64, authenticated_u60_continuation=True, terminal_scheduler_cursor=True
    )
    assert EXPERT_PLANE_SURFACE in base_lrs


def test_authenticated_u60_policy_exposes_fourth_base_lr() -> None:
    config = {
        "recipe_id": PUBLISHED_PRE_RECIPE_ID,
        "checkpoint_sha256": AUTHENTICATED_U60_CHECKPOINT_SHA256,
    }
    ...
    assert base_lrs[EXPERT_PLANE_SURFACE] == U60_EXPERT_PLANE_BASE_LR
    assert list(base_lrs) == ["luts", "norms", "outputs", EXPERT_PLANE_SURFACE]


def test_terminal_scheduler_cursor_64_is_not_an_update_cursor() -> None:
    config = {
        "recipe_id": PUBLISHED_PRE_RECIPE_ID,
        "checkpoint_sha256": AUTHENTICATED_U60_CHECKPOINT_SHA256,
    }
    with pytest.raises(ArtifactError, match="cursor must be within U0..U63"):
        _published_pre_recipe_policy(
            config, 64, authenticated_u60_continuation=True
        )
    _base_lrs, multiplier, _windows = _published_pre_recipe_policy(
        config,
        64,
        authenticated_u60_continuation=True,
        terminal_scheduler_cursor=True,
    )
    assert multiplier == pytest.approx(0.1)


def test_terminal_scheduler_cursor_is_wired_only_into_lambdalr() -> None:
    ...
    assert source.count("terminal_scheduler_cursor=True") == 1


def test_all_resident_recipe_policy_calls_carry_validated_u60_identity() -> None:
    from repair_api.modern_green_resident import ModernGreenResidentEngine

    source = inspect.getsource(ModernGreenResidentEngine)
    assert source.count(
...
def test_exact_authenticated_u64_successor_remains_loadable_for_validation() -> None:
    state = {
        "luts": {},
        "norms": {},
        "outputs": {},
        EXPERT_PLANE_SURFACE: _u60_planes(),
    }
    payload = {
        "state": state,
        "next_update": 64,
        "identity": {
            ...
            "checkpoint": "UPDATE_064",
            "next_update": 64,
            "parent_checkpoint_sha256": AUTHENTICATED_U60_CHECKPOINT_SHA256,
            "parent_identity_sha256": "f0f5479111be56a34c32d21dba214372097350c8072cf7da4aa9fdeb37fdf4bd",
            "state_sha256": "dc5b702f7c90ebe1161980ea648425a8f9103588eb24d8af1a32597e33547e19",
            "optimizer_scheduler_lineage": "fresh-published-pre-adam-lambdalr",
            "identity_sha256": "2bbb634af51cbc3e3c0c8c575fec12844a9f59c4a2b4c7cbf8d2a737efb16f7b",
        },
    }
    assert _validate_trainable_state_schema(torch, payload) is True
