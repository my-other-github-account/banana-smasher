...
AUTHENTICATED_U64_CHECKPOINT_SHA256 = "4cab94648b62e15dbcc0be4e3701c5c2b38af71f8662011455ba0312448da624"
CANONICAL_TRAIN_BALANCED64_WINDOWS = (
    10, 12, 19, 24, 37, 45, 57, 60, 61, 63, 66, 69, 70, 75, 81, 89,
    90, 109, 112, 113, 120, 128, 145, 146, 147, 148, 151, 159, 216, 226,
    230, 244, 247, 267, 274, 276, 278, 279, 292, 303, 310, 322, 329, 333,
    339, 346, 352, 378, 383, 386, 388, 397, 404, 405, 412, 419, 421, 435,
    445, 450, 463, 487, 490, 506,
)


def _admit_resident_validation_windows(
    windows: tuple[int, ...], *, artifact_windows: tuple[int, ...], checkpoint_sha256: str
) -> str:
    """Keep legacy manifests, but bind authenticated U64 to canonical Balanced64."""
    if windows == (28,):
        return "W28"
    if checkpoint_sha256 == AUTHENTICATED_U64_CHECKPOINT_SHA256:
        if windows == CANONICAL_TRAIN_BALANCED64_WINDOWS:
            return "CANONICAL_BALANCED64"
        raise ArtifactError(
            "authenticated U64 validate_release requires the canonical Balanced64 roster"
        )
    if windows == artifact_windows:
        return "ARTIFACT_BALANCED64"
    raise ArtifactError(
        "resident stage validate_release requires windows=[28] or the exact ordered Balanced64 manifest"
    )


def _require_scorer_aligned_preupdate_gate(
    diagnostic: Mapping[str, Any], *, tolerance: float = 1.0e-12
) -> dict[str, Any]:
    """Fail closed unless training and scorer consume one exact W28 fixture."""
    equal = int(diagnostic.get("support_index_equal_count", -1))
    total = int(diagnostic.get("support_index_total", -1))
    training_kld = float(diagnostic.get("new_loss_group_token_kld_mean", math.nan))
    scorer_kld = float(diagnostic.get("old_tailfix_token_kld_mean", math.nan))
    delta = abs(training_kld - scorer_kld)
    if total <= 0 or equal != total:
        raise ArtifactError(
            f"scorer-aligned pre-update support identity failed: {equal}/{total}"
        )
    if not math.isfinite(delta) or delta > tolerance:
        raise ArtifactError(
            "scorer-aligned pre-update KLD equality failed: "
            f"training={training_kld!r} scorer={scorer_kld!r} "
            f"delta={delta!r} tolerance={tolerance!r}"
        )
    return {
        "status": "PASS",
        "support_index_equal_count": equal,
        "support_index_total": total,
        "training_kld": training_kld,
        "scorer_kld": scorer_kld,
        "kld_abs_delta": delta,
        "tolerance": tolerance,
    }
...
def _candidate_read_and_one_layer_delta_proof(
    engine: Any,
    *,
    layer: int,
    ...
    """Prove the resident score path consumed candidate bytes before scoring."""
    torch = engine.torch
    student = engine.student
    payload_reads = int(student.payload_disk_reads)
    if payload_reads <= 0:
        raise ArtifactError("candidate payload read proof requires payload_disk_reads > 0")
    expert = student.experts[layer]
    packed = expert.packed_w1
    su = expert._projection_plane("w1", "SU")
    sv = expert._projection_plane("w1", "SV")
    k = int(su.shape[1])
    x = torch.linspace(-1.0, 1.0, k, device=student.device, dtype=torch.float32)
    x = x.to(torch.bfloat16).reshape(1, k).contiguous()
    assignment = torch.zeros(1, device=student.device, dtype=torch.int64)
    candidate = expert._project(
        "w1", x, assignment, packed, student.sources[layer].master, su, sv
    ).float()

    prefix = f"layers.{layer}.ffn.experts.0.w1"
    native_packed = student.get_tensor(prefix + ".weight").to(student.device)
    native_scale = student.get_tensor(prefix + ".scale").to(student.device)
    byte = native_packed.view(torch.uint8)
    magnitudes = torch.tensor(
        [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0],
        device=student.device,
        dtype=torch.float32,
    )
    levels = torch.cat((magnitudes, -magnitudes))
    native = torch.stack(
        (levels[(byte & 0xF).long()], levels[(byte >> 4).long()]), dim=-1
    ).flatten(-2)
    scales = torch.exp2(native_scale.view(torch.uint8).float() - 127.0)
    native = native * scales.repeat_interleave(32, -1)
    native_output = x.float() @ native.float().transpose(0, 1)
    delta_l2 = float(torch.linalg.vector_norm(candidate - native_output).item())
    native_l2 = float(torch.linalg.vector_norm(native_output).item())
    relative_l2 = delta_l2 / max(native_l2, 1.0e-30)
    if not math.isfinite(relative_l2) or not (0.0 < relative_l2 < 1.0):
        raise ArtifactError(
            f"one-layer native-to-candidate delta is not nonzero-small: {relative_l2!r}"
        )
    return {
        "candidate_artifact_path": str(checkpoint_path),
        "candidate_artifact_sha256": checkpoint_sha256,
        "candidate_artifact_bytes": checkpoint_path.stat().st_size,
        "candidate_checkpoint_reads": 1,
        "candidate_payload_reads": payload_reads,
        "candidate_payload_bytes": int(student.routed_payload_bytes),
        "native_tensor_reads": 2,
        "layer": layer,
        "expert": 0,
        "projection": "w1",
        "delta_l2": delta_l2,
...
class ResidentRepairAPI:
    ...
    def validate(
        ...
        *,
        ...
        candidate_read_proof = getattr(trainer, "candidate_read_proof", None)
        if candidate_read_proof is not None:
            if (
                int(candidate_read_proof.get("candidate_checkpoint_reads", 0)) <= 0
                or int(candidate_read_proof.get("candidate_payload_reads", 0)) <= 0
                or not (0.0 < float(candidate_read_proof.get("relative_l2", 0.0)) < 1.0)
            ):
                raise ArtifactError("candidate read/delta proof is not admissible")
            result["candidate_read_proof"] = dict(candidate_read_proof)
    ...
    def stage_two_spark_real(
        ...
        *,
        ...
        checkpoint_path = self.artifact.checkpoint_path(start)
        try:
            payload = _load_torch(checkpoint_path)
        except Exception as exc:
        ...
        payload["checkpoint_sha256"] = str(start_sha)
        ...
        candidate_read_proof = None
        if config.get("candidate_read_proof_required") is True:
            candidate_read_proof = _candidate_read_and_one_layer_delta_proof(
                engine,
                layer=ranges[rank][0],
                checkpoint_path=checkpoint_path,
                checkpoint_sha256=str(start_sha),
            )
            setattr(engine, "candidate_read_proof", candidate_read_proof)
        ...
        ready = {
            ...
            "state_counts": {name: len(values) for name, values in state.items()},
            "candidate_read_proof": candidate_read_proof,
            "expert_plane_candidate_read": getattr(engine, "expert_plane_candidate_read", None),
        ...
        if action == "validate_release":
            ...
            try:
                _admit_resident_validation_windows(
                    windows,
                    artifact_windows=self.windows,
                    checkpoint_sha256=str(start_sha),
                )
            except ArtifactError:
                engine.close()
    ...
    def continue_two_spark_real(
        ...
        *,
        ...
        numerical_stability_continuation = (
            start_update == 6
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        numerical_stability_tail_recovery = (
            start_update == 8
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        numerical_stability_tail_continuation = (
            start_update == 10
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        numerical_stability_final_recovery = (
            start_update == 13
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        numerical_stability_boundary_continuation = (
            start_update == 14
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        numerical_stability_final_segment_recovery = (
            start_update == 17
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        step_time_boundary_continuation = (
            start_update == 18
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
            ...
            and config.get("authorized_optimizer_arithmetic_repair")
                == "adam-fp64-state-and-update-arithmetic"
            and config.get("authorized_gradient_calibration")
                == "measured-pre-gradient-max-ratio-v1"
            and config.get("gradient_lr_divisors")
                == {"luts": 4.6, "norms": 4.3, "outputs": 5.2}
            and config.get("activation_checkpointing") is True
            and start_meta.get("optimizer_scheduler_lineage")
                == "fresh-published-pre-adam-lambdalr"
        )
        gradient_calibration_final_recovery = (
            start_update == 21
            and str(start).startswith("SCHEDULE_")
            and config.get("recipe_id") == published_pre_recipe
            and config.get("fresh_published_pre_lineage") is True
        ...
        if validation_proof:
            ...
            published_pre_schedule_resume
            or numerical_stability_resume
            or numerical_stability_continuation
            or numerical_stability_tail_recovery
            or numerical_stability_tail_continuation
            or numerical_stability_final_recovery
            or numerical_stability_boundary_continuation
            or numerical_stability_final_segment_recovery
            or step_time_boundary_continuation
            or gradient_calibration_final_recovery
        ):
            pass
        ...
        valid_numerical_stability_probe = numerical_stability_resume and requested == (6,)
        valid_numerical_stability_continuation = (
            numerical_stability_continuation and requested == (10,)
        )
        valid_numerical_stability_tail_recovery = (
            numerical_stability_tail_recovery and requested == (10,)
        )
        valid_numerical_stability_tail_continuation = (
            numerical_stability_tail_continuation and requested == (14,)
        )
        valid_numerical_stability_final_recovery = (
            numerical_stability_final_recovery and requested == (14,)
        )
        valid_numerical_stability_boundary_continuation = (
            numerical_stability_boundary_continuation and requested == (18,)
        )
        valid_numerical_stability_final_segment_recovery = (
            numerical_stability_final_segment_recovery and requested == (18,)
        )
        valid_step_time_boundary_continuation = (
            step_time_boundary_continuation and requested == (22,)
        )
        valid_gradient_calibration_final_recovery = (
            gradient_calibration_final_recovery and requested == (22,)
        )
        ...
        if not (
            ...
            or valid_numerical_stability_probe
            or valid_numerical_stability_continuation
            or valid_numerical_stability_tail_recovery
            or valid_numerical_stability_tail_continuation
            or valid_numerical_stability_final_recovery
            or valid_numerical_stability_boundary_continuation
            or valid_numerical_stability_final_segment_recovery
            or valid_step_time_boundary_continuation
            or valid_gradient_calibration_final_recovery
            or valid_milestones
        ):
            raise ArtifactError(
        ...
        if validation_proof:
            ...
            if (
                config.get("objective_ab_probe_w28") is True
                or config.get("scorer_aligned_preupdate_gate") is True
            ):
                diagnostic = pre_validation.get("objective_ab_probe_w28")
                if not isinstance(diagnostic, Mapping):
                    engine.close()
                    raise ArtifactError("objective A/B probe produced no W28 diagnostic")
                if config.get("scorer_aligned_preupdate_gate") is True:
                    pre_validation["scorer_aligned_preupdate_gate"] = (
                        _require_scorer_aligned_preupdate_gate(diagnostic)
                    )
                if config.get("objective_ab_probe_w28") is True:
                    result = {
                        "schema": "banana-smasher-objective-ab-w28-v1",
                        "status": "PASS",
                        ...
                        "objective_ab": dict(diagnostic),
                    }
                    self._write_immutable_receipt(receipt_path, result)
                    engine.close()
                    return result
        if config.get("w28_forward_mode_ab_probe") is True:
            if not validation_proof or not isinstance(pre_validation, Mapping):
                engine.close()
                raise ArtifactError("W28 forward-mode A/B requires static pre-validation")
            expected = config.get("w28_forward_mode_ab_expected_static_kld")
            objective = pre_validation.get("objective_ab_probe_w28")
            observed = objective.get("old_tailfix_token_kld_mean") if isinstance(objective, Mapping) else None
            if expected is None or observed is None or float(observed) != float(expected):
                engine.close()
                raise ArtifactError("W28 forward-mode A/B static instrument identity failed")
            diagnostic = engine.diagnose_w28_forward_modes()
            result = {
                "schema": "banana-smasher-w28-forward-mode-ab-v1",
                "status": "PASS",
                "task_id": config.get("task_id"),
                "rank": rank,
                "world_size": 2,
                ...
                "basis_sha256": basis,
                "canonical_git_pin": config.get("canonical_git_pin"),
                "static_pre_validation": dict(pre_validation),
                "forward_mode_ab": diagnostic,
                "optimizer_steps": 0,
                "scheduler_steps": 0,
                "checkpoint_saves": 0,
            }
            ...
            engine.close()
            return result
        if config.get("w28_batch_context_ab_probe") is True:
            if not validation_proof or not isinstance(pre_validation, Mapping):
                engine.close()
                raise ArtifactError("W28 batch-context A/B requires static pre-validation")
            expected = config.get("w28_batch_context_ab_expected_static_kld")
            objective = pre_validation.get("objective_ab_probe_w28")
            observed = objective.get("old_tailfix_token_kld_mean") if isinstance(objective, Mapping) else None
            if expected is None or observed is None or float(observed) != float(expected):
                engine.close()
                raise ArtifactError("W28 batch-context A/B static instrument identity failed")
            diagnostic = engine.diagnose_w28_batch_context()
            result = {
                "schema": "banana-smasher-w28-batch-context-ab-v1",
                "status": "PASS",
                "task_id": config.get("task_id"),
                "rank": rank,
                "world_size": 2,
