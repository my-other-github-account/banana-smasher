...
def test_resident_stage_release_loads_exact_state_without_training():
    with tempfile.TemporaryDirectory() as directory:
        ...
        full64_ready = root / "FULL64_READY.json"
        full64_control = root / "FULL64_CONTROL.json"
        full64_receipt = root / "FULL64_VALIDATE.json"
        full64_control.write_text(json.dumps({
            "action": "validate_release",
            "windows": list(range(64)),
            "teacher_root": "/validation-teacher",
            "receipt_path": str(full64_receipt),
        }))
        with patch("repair_api.api._load_torch", return_value=payload), patch(
            "repair_api.modern_green_resident.ModernGreenResidentEngine", FakeEngine
        ):
            full64 = ResidentRepairAPI.open(root).stage_two_spark_real(
                "UPDATE_020",
                config=config,
                ready_path=full64_ready,
                control_path=full64_control,
                poll_seconds=0.01,
            )
        full64_engine = FakeEngine.instances[-1]
        assert full64["status"] == "VALIDATED_AND_RELEASED_WITHOUT_TRAINING"
        assert full64_engine.validate_calls == [(tuple(range(64)), Path("/validation-teacher"))]
        assert full64_engine.advance_calls == []
        assert full64_engine.closed is True
