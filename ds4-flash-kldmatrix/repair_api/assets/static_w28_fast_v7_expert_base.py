...
class FullyResidentGroupedV7Experts(nn.Module):
    ...
    def forward(
        ...
        route_multiply_input = routed_output
        route_multiply_weight = route_weight.to(dtype=routed_output.dtype)
        routed_output = route_multiply_input * route_multiply_weight
        route_weight_callsite = getattr(self, "_route_weight_callsite_trace", None)
        if route_weight_callsite is not None:
            code = type(self).forward.__code__
            route_weight_callsite.update({
                "module_file": __file__,
                "co_filename": code.co_filename,
                "co_firstlineno": int(code.co_firstlineno),
                "input": route_multiply_input.detach(),
                "route_weight": route_multiply_weight.detach(),
                "output": routed_output.detach(),
            })
        routed_output = routed_output.to(hidden_states.dtype)
