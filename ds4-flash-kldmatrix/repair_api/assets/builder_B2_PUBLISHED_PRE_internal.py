...
DEV = "cuda"
SUP = 8192
CHECKPOINT = "/home/dnola/missions/PRE_SCORE_t_9e5a36e1/track-b-s2/inputs/PRE_f9bffe04.pt"
...
def load_u1():
    ...
    if value.get("format") != "banana-smasher-qtip2-v7-joint-checkpoint-v1" or value.get("identity_sha256") != CANDIDATE_IDENTITY or int(value.get("next_update", -1)) != 0:
...
def tensor_tap(value):
    """Hash the sealed builder's actual tensor bytes at a named boundary."""
    tensor = value.detach().contiguous()
    cpu = tensor.to(device="cpu")
    raw = cpu.view(torch.uint8).numpy().tobytes()
    flat = cpu.reshape(-1)[:8]
    sample = ([float(item) for item in flat.float().tolist()]
              if flat.dtype.is_floating_point else
              [int(item) for item in flat.to(torch.int64).tolist()])
...
class ShardCache:
    ...
    def get(self, shard):
        # Evict the prior shard before fetching the next.  The original
        # fetch-then-evict order assumes room for two ~3.6GB shards, while this
        # assigned host has room for exactly one plus the 4GiB safety reserve.
        with self.lock:
            if shard not in self.have and self.keep == 1:
...
def build_layer_sd(L, wm, get_tensor, mode, planes=None):
    ...
    # Apply the exact dense U1 state reloaded from the immutable serialized
    # public-runner checkpoint. Names are the accepted runner's module names.
    prefix = f"model.layers.{L}."
    for name, value in U1["state"]["norms"].items():
        if name.startswith(prefix):
            sd[name[len(prefix):] + ".weight"] = value.to(DEV).to(torch.bfloat16)
    gain_key = f"model.layers.{L}.self_attn.o_b_proj.output_log_gain"
    gain = U1["state"]["outputs"][gain_key].clamp(-GAIN_CLAMP, GAIN_CLAMP)
    sd["self_attn.o_b_proj.weight"] = sd["self_attn.o_b_proj.weight"] * torch.exp(gain).to(DEV).to(torch.bfloat16)
...
def main():
    ...
    ap.add_argument("--remote", default=
        "dnola@192.168.200.6:models/hf/DeepSeek-V4-Flash")
    ...
    ap.add_argument("--tap-fixture", default=None,
                    help="diagnostic-only same-input boundary trace")
    ap.add_argument("--intended-basis-index", default=None,
                    help="model index whose SHA must equal the production basis")
    ap.add_argument("--intended-basis-sha256", default=None)
    a = ap.parse_args()

    assert a.mode == "bf16" or a.ref_dir, "--ref-dir required in cand modes"
    assert a.mode != "planes" or a.planes_dir, "--planes-dir required"
    if a.tap_fixture and (a.mode == "bf16" or not a.windows):
        raise RuntimeError("--tap-fixture requires one explicit candidate window")
    if bool(a.intended_basis_index) != bool(a.intended_basis_sha256):
        raise RuntimeError("intended basis index and SHA must be supplied together")
    if a.intended_basis_index and sha256(a.intended_basis_index) != a.intended_basis_sha256:
        raise RuntimeError("BASIS_GATE_MODEL_INDEX_MISMATCH")
    actual_basis_index = os.path.join(a.meta_dir, "model.safetensors.index.json")
    if a.intended_basis_sha256 and sha256(actual_basis_index) != a.intended_basis_sha256:
        raise RuntimeError("BASIS_GATE_ACTUAL_META_INDEX_MISMATCH")
    ...
    if not todo:
        log("nothing to do")
        return 0
    if a.tap_fixture and len(todo) != 1:
        raise RuntimeError("--tap-fixture requires exactly one candidate window")
    ...
    wm = json.load(open(actual_basis_index))["weight_map"]
    ...
    model.model.norm.weight = torch.nn.Parameter(
        U1["state"]["norms"]["model.norm"].to(DEV).to(torch.bfloat16), requires_grad=False)
    ...
    for c0 in range(0, len(todo), a.chunk):
        ...
        ids = ids.to(DEV)
        taps = {"ids": tensor_tap(ids)} if a.tap_fixture else None
        internal_boundaries = {}
        ...
        with torch.no_grad():
            ...
            if taps is not None:
                taps["embeddings"] = tensor_tap(embeds)
            pe = {
            ...
            for L in range(NL):
                ...
                del sd
                for mi, s in enumerate(mbs):
                    if L != 0 or taps is None:
                        hidden[mi] = lay(
                            hidden[mi], position_embeddings=pe, position_ids=pos,
                            attention_mask=masks[mi], input_ids=ids[s],
                            past_key_values=caches[mi])
                        continue
                    hidden_input = hidden[mi]
                    dtype = hidden_input.dtype
                    internal_boundaries["layer_input"] = tensor_tap(hidden_input)
                    post_a, comb_a, collapsed_a = lay.attn_hc(hidden_input)
                    norm_a = lay.input_layernorm(collapsed_a)
                    attn_output, _ = lay.self_attn(
                        norm_a, position_embeddings=pe, position_ids=pos,
                        attention_mask=masks[mi], input_ids=ids[s],
                        past_key_values=caches[mi])
                    internal_boundaries["attention_return"] = tensor_tap(attn_output)
                    residual_a = (
                        post_a.to(dtype).unsqueeze(-1) * attn_output.unsqueeze(-2)
                        + torch.matmul(comb_a.to(dtype).transpose(-1, -2), hidden_input)
                    )
                    internal_boundaries["post_attention_residual"] = tensor_tap(residual_a)
                    post_f, comb_f, collapsed_f = lay.ffn_hc(residual_a)
                    norm_f = lay.post_attention_layernorm(collapsed_f)
                    router = {}
                    def router_hook(_module, args, output):
                        router["input"] = args[0].detach()
                        router["output"] = output
                    handle = lay.mlp.gate.register_forward_hook(router_hook)
                    mlp_output = lay.mlp(norm_f, input_ids=ids[s])
                    handle.remove()
                    internal_boundaries["router_input"] = tensor_tap(router["input"])
                    router_output = router["output"]
                    if not isinstance(router_output, tuple) or len(router_output) != 3:
                        raise RuntimeError("ACCEPTED_ROUTER_OUTPUT_GEOMETRY_RED")
                    internal_boundaries["router_output"] = {
                        "logits": tensor_tap(router_output[0]),
                        "weights": tensor_tap(router_output[1]),
                        "indices": tensor_tap(router_output[2]),
                    }
                    internal_boundaries["moe_return"] = tensor_tap(mlp_output)
                    hidden[mi] = (
                        post_f.to(dtype).unsqueeze(-1) * mlp_output.unsqueeze(-2)
                        + torch.matmul(comb_f.to(dtype).transpose(-1, -2), residual_a)
                    )
                    internal_boundaries["final_residual"] = tensor_tap(hidden[mi])
                if taps is not None:
                    taps[f"L{L:03d}"] = tensor_tap(hidden[0])
                if taps is not None and L == 0 and NL == 1:
                    order = ("layer_input", "attention_return", "post_attention_residual",
                             "router_input", "router_output", "moe_return", "final_residual")
                    if tuple(internal_boundaries) != order:
                        raise RuntimeError("ACCEPTED_L000_INTERNAL_ORDER_RED")
                    if internal_boundaries["final_residual"]["sha256"] != "62b2e4027be14946b581cfddaccca09b9f3baa7c459dd91c45f41a805d7c01ec":
                        raise RuntimeError("ACCEPTED_L000_FINAL_AUTHENTICATION_RED")
                    fixture = {
                        "schema": "banana-smasher-accepted-builder-l000-internal-v1",
                        "status": "PASS", "window": wins[0],
                        "source_builder_parent_sha256": "11ead706db562197e76cdc320d5d13044bb254a411b6412326667f524ddf29ed",
                        "source_builder_instrument_sha256": sha256(__file__),
                        "source_checkpoint_sha256": sha256(CHECKPOINT),
                        "source_model_index_sha256": sha256(actual_basis_index),
                        "intended_basis_sha256": a.intended_basis_sha256,
                        "boundary_order": list(order), "boundaries": internal_boundaries,
                    }
                    temporary = a.tap_fixture + ".tmp"
                    with open(temporary, "w") as f:
                        json.dump(fixture, f, indent=2, sort_keys=False)
                        f.write("\n"); f.flush(); os.fsync(f.fileno())
                    os.replace(temporary, a.tap_fixture)
                    log("sealed authenticated accepted L000 internal boundaries")
                    return 0
            ...
            # readout
            for mi, s in enumerate(mbs):
                hc = model.model.hc_head(hidden[mi])
                h = model.model.norm(hc)
                if taps is not None:
                    taps["hc_head"] = tensor_tap(hc)
                    taps["norm"] = tensor_tap(h)
                for j in range(h.shape[0]):
                    k = wins[s.start + j]
                    rl = rlens[s.start + j]
                    logits = model.lm_head(h[j, :rl].to(torch.bfloat16)).float()
                    if taps is not None:
                        taps["logits"] = tensor_tap(logits[:1024])
                    ...
                    if a.mode == "bf16":
                        ...
                        if taps is not None:
                            taps["q_lp_at_ref"] = tensor_tap(obj["q_lp_at_ref"])
                            taps["q_argmax"] = tensor_tap(obj["q_argmax"])
                    ...
                    if taps is not None:
                        expected = ("ids", "embeddings",
                                    *(f"L{x:03d}" for x in range(43)),
                                    "hc_head", "norm", "logits",
                                    "q_lp_at_ref", "q_argmax")
                        if tuple(taps) != expected:
                            raise RuntimeError("sealed builder tap order drift")
                        fixture = {
                            "schema": "banana-smasher-sealed-builder-pre-tap-v1",
                            "status": "PASS", "window": k,
                            "source_builder_sha256": sha256(__file__),
                            "source_checkpoint_sha256": sha256(CHECKPOINT),
                            "source_model_index_sha256": sha256(os.path.join(
                                a.meta_dir, "model.safetensors.index.json")),
                            "intended_basis_sha256": a.intended_basis_sha256,
                            "candidate_path": out_p,
                            "candidate_file_sha256": sha256(out_p),
                            "teacher_path": os.path.join(a.ref_dir, f"t8192_win{k}.pt"),
                            "teacher_file_sha256": sha256(os.path.join(
                                a.ref_dir, f"t8192_win{k}.pt")),
                            "taps": taps,
                        }
                        temporary = a.tap_fixture + ".tmp"
                        with open(temporary, "w") as f:
