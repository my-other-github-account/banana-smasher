...
def test_sealed_pre_binding_requires_accepted_builder() -> None:
    root = Path(__file__).parents[1]
    builder = root / "assets" / "builder_B2_PUBLISHED_PRE.py"
    accepted = "11ead706db562197e76cdc320d5d13044bb254a411b6412326667f524ddf29ed"
    assert hashlib.sha256(builder.read_bytes()).hexdigest() == accepted
    assert f'BUILDER_SHA256 = "{accepted}"' in (root / "sealed_pre_forward.py").read_text()
...
def test_true_builder_control_uses_accepted_plane_source() -> None:
    from repair_api.sealed_pre_forward import sealed_builder_control_argv

    argv = sealed_builder_control_argv(
        builder_path=Path("/canonical/builder.py"),
        root=Path("/mission"),
        config={
            "rank": 1,
            "validation_teacher_root": "/teacher",
            "validation_corpus": "/corpus.json",
        },
        windows=(28,),
        label="DIAGNOSTIC",
        tap_fixture=Path("/mission/TRUE_BUILDER_TAPS.json"),
    )

    assert argv[argv.index("--mode") + 1] == "planes"
    assert "w2" not in argv
    assert argv[argv.index("--planes-dir") + 1] == "/mission/SEALED_PRE_CONTRACT.json"
    assert argv[argv.index("--tap-fixture") + 1] == "/mission/TRUE_BUILDER_TAPS.json"
