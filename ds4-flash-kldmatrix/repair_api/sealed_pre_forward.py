...
def sealed_builder_control_argv(
    *,
    builder_path: Path,
    root: Path,
    config: dict[str, Any],
    windows: tuple[int, ...],
    label: str,
    tap_fixture: Path | None = None,
) -> list[str]:
    """Return the accepted PlaneSource builder invocation used by every control."""
    ...
    argv = [
        str(builder_path), "--mode", "planes", "--planes-dir", str(root / "SEALED_PRE_CONTRACT.json"),
    ...
    if tap_fixture is not None:
        argv.extend(("--tap-fixture", str(tap_fixture)))
    return argv
...
def _run_builder(builder: Any, *, root: Path, config: dict[str, Any], windows: tuple[int, ...], label: str) -> tuple[list[dict[str, Any]], float]:
    ...
    try:
        sys.argv = sealed_builder_control_argv(
            builder_path=Path(builder.__file__), root=root, config=config,
            windows=windows, label=label,
        )
