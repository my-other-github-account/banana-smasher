...
def _compare_post_w2_chain(
    control: dict[str, Any], product: dict[str, Any]
) -> dict[str, Any]:
    """Compare the mandated ordered post-W2 chain with an exact self-control."""
    boundary_order = (
        "raw_bf16_w2", "fp32_provider_exposure", "route_weight_multiply",
        "reshape_order_gather_buffer", "per_token_reduction", "mlp_return",
        "residual_combine",
    )
    if tuple(control) != boundary_order or tuple(product) != boundary_order:
        raise RuntimeError("POST_W2_CHAIN_BOUNDARY_ORDER_RED")

    def compare(left: Any, right: Any) -> dict[str, Any]:
        row = _compare_tensor_boundary(left, right)
        row["control"].update(
            stride=[int(value) for value in left.stride()],
            storage_offset=int(left.storage_offset()),
        )
        row["variant"].update(
            stride=[int(value) for value in right.stride()],
            storage_offset=int(right.storage_offset()),
        )
        return row

    self_boundaries = {
        name: compare(control[name], control[name].clone()) for name in boundary_order
    }
    self_exact = all(row["exact"] for row in self_boundaries.values())
    if not self_exact:
        raise RuntimeError("POST_W2_COMPARATOR_SELF_CONTROL_RED")
    boundaries = {
        name: compare(control[name], product[name]) for name in boundary_order
    }
    first = next(
        (name for name in boundary_order if not boundaries[name]["exact"]), None
    )
    return {
        "comparator_self_control_exact": self_exact,
        "comparator_self_control": self_boundaries,
...
def _normalize_provider_authentic_trace(
    ...
    """Normalize the restored provider's built-in, output-transparent trace."""
    required = (
        "route_key", "route_weight", "gate", "up", "activated", "w2_down",
        "weighted_routed_output",
    )
    missing = [name for name in required if name not in provider_trace]
    if missing:
        raise RuntimeError(
            "AUTHENTIC_PROVIDER_TRACE_COVERAGE_RED:" + ",".join(missing)
        )
    route_key = provider_trace["route_key"]
    route_count = int(top_k_index.numel())
    if tuple(route_key.shape) != (route_count, 3):
        raise RuntimeError("AUTHENTIC_PROVIDER_ROUTE_KEY_GEOMETRY_RED")
    ...
    if not (
        torch.equal(route_key[:, 0], expected_experts)
        and torch.equal(route_key[:, 1], expected_slots)
        and torch.equal(route_key[:, 2], expected_tokens)
    ):
        raise RuntimeError("AUTHENTIC_PROVIDER_ROUTE_KEY_SEMANTICS_RED")
    route_weight = provider_trace["route_weight"]
    if not torch.equal(route_weight, top_k_weights.reshape(-1)):
        raise RuntimeError("AUTHENTIC_PROVIDER_ROUTE_WEIGHT_SEMANTICS_RED")
    weighted = provider_trace["weighted_routed_output"]
    ordered = weighted.reshape(
        hidden_states.shape[0], top_k_index.shape[1], weighted.shape[-1]
    )
    capture = {
        "authentic_w2_output": provider_trace["w2_down"],
        "authentic_route_weights": route_weight.reshape(-1, 1),
        "authentic_token_indices": route_key[:, 2],
        "authentic_expert_indices": route_key[:, 0],
        "authentic_weighted_buffer": weighted,
        "authentic_ordered_weighted_buffer": ordered,
    }
    assembly = {
        "hidden_states": hidden_states,
        "routed_output": provider_trace["w2_down"],
        "top_k_index": top_k_index,
        "top_k_weights": top_k_weights,
        "final_return": final_return,
    }
    return capture, assembly


@contextmanager
def _capture_static_provider_w2(module: Any, route_capture: Any) -> Any:
    """Mark the authentic static provider's W2 return without changing bytes."""
    original_project = module._project

    def observed_project(*args: Any, **kwargs: Any) -> Any:
        value = original_project(*args, **kwargs)
        projection = args[0] if args else kwargs.get("projection")
        assignments = args[2] if len(args) > 2 else kwargs.get("assignments")
        if projection == "w2":
            if assignments is None:
                raise RuntimeError("AUTHENTIC_STATIC_W2_ASSIGNMENTS_MISSING")
            route_capture.capture_w2(value, assignments)
        return value

    module._project = observed_project
    try:
        yield
    finally:
        module._project = original_project


def _static_provider_replay_arguments(
    module: Any, routed_hidden: Any, expert_index: Any, route_count: int,
) -> tuple[Any, ...]:
    """Recreate the active static provider's two required projection arguments."""
    import inspect

    for provider_type in type(module).__mro__:
        implementation = provider_type.__dict__.get("_project")
        if implementation is None:
            continue
        parameters = inspect.signature(implementation).parameters
        if not {"route_rows_per_sample", "route_metadata"}.issubset(parameters):
            continue
        implementation_globals = implementation.__globals__
        metadata_builder = implementation_globals.get("grouped_route_metadata")
        expert_count = implementation_globals.get("EXPERTS")
        if not callable(metadata_builder) or not isinstance(expert_count, int):
            raise RuntimeError("STATIC_PROVIDER_REPLAY_METADATA_SOURCE_MISSING")
        route_rows_per_sample = int(route_count)
        route_metadata = metadata_builder(
            expert_index, expert_count, input_tensor=routed_hidden
        )
        return route_rows_per_sample, route_metadata
    return ()
...
def _run_one_layer_with_expert_trace(
    ...
    post_w2_capture: dict[str, Any] = {}
    route_weight_callsite_capture: dict[str, Any] = {}
    mlp_capture: dict[str, Any] = {}
    ...
    def replay_resident(
        ...
        projection_tail = _static_provider_replay_arguments(
            module, routed_hidden, expert_index,
            int(hidden_states.shape[0]) * int(top_k_index.shape[1]),
        )
        gate = module._project(
            "w1", routed_hidden, expert_index, module.packed_w1,
            lut_master, module.su_w1, module.sv_w1, *projection_tail,
        )
        up = module._project(
            "w3", routed_hidden, expert_index, module.packed_w3,
            lut_master, module.su_w3, module.sv_w3, *projection_tail,
        )
        ...
        down = module._project(
            ...
            lut_master, module.su_w2, module.sv_w2, *projection_tail,
        )
    ...
    def transparent_forward(
        ...
        authentic_provider_trace: dict[str, Any] = {}
        module._authentic_expert_trace = authentic_provider_trace
        module._route_weight_callsite_trace = route_weight_callsite_capture
        ...
        def capture_post_w2(raw: Any, exposure: Any) -> None:
            if post_w2_capture:
                raise RuntimeError("POST_W2_OBSERVER_CALLED_MORE_THAN_ONCE")
            post_w2_capture.update(raw=raw.detach(), exposure=exposure.detach())
        ...
        try:
            with (
                ...
                patch.object(
                    resident_runtime,
                    "_POST_W2_CHAIN_OBSERVER",
                    capture_post_w2,
                    create=True,
                ),
                _capture_static_provider_w2(module, route_capture),
                route_capture,
            ):
                authentic = original_forward(hidden_states, top_k_index, top_k_weights)
        finally:
            module.__dict__.pop("_a30_route_capture", None)
            module.__dict__.pop("_authentic_expert_trace", None)
            module.__dict__.pop("_route_weight_callsite_trace", None)
        if authentic_provider_trace:
            normalized, provider_assembly = _normalize_provider_authentic_trace(
                authentic_provider_trace, hidden_states, top_k_index,
                top_k_weights, authentic,
            )
            authentic_capture.update(normalized)
            assembly_capture["variant"] = provider_assembly
        else:
            required = {
    ...
    def capture_mlp_return(_module: Any, _args: Any, value: Any) -> None:
        tensor = _first_tensor(value)
        if tensor is None:
            raise RuntimeError("POST_W2_MLP_RETURN_TENSOR_ABSENT")
        mlp_capture["value"] = tensor.detach()

    mlp_handle = layer.mlp.register_forward_hook(capture_mlp_return)
    try:
    ...
    if "value" not in mlp_capture:
        raise RuntimeError("POST_W2_MLP_RETURN_CAPTURE_MISSING")
    trace["mlp_return"] = mlp_capture["value"].cpu()
    trace["residual_combine"] = output.detach().cpu()
    if resident:
        if tuple(post_w2_capture) != ("raw", "exposure"):
            raise RuntimeError("POST_W2_RAW_EXPOSURE_CAPTURE_MISSING")
        flat_experts = replay_inputs["top_k_index"].reshape(-1).to(torch.int64)
        positions = torch.nonzero(flat_experts == 204, as_tuple=False).reshape(-1)
        if positions.numel() == 0:
            raise RuntimeError("POST_W2_TRACE_EXPERT_204_INACTIVE")
        route_count = int(replay_inputs["top_k_index"].shape[1])
        token_count = int(replay_inputs["top_k_index"].shape[0])
        tokens = torch.div(positions, route_count, rounding_mode="floor")
        slots = positions.remainder(route_count)
        selected_order = torch.argsort(slots * token_count + tokens, stable=True)
        selected_positions = positions[selected_order]
        trace["authentic_w2_raw_selected"] = post_w2_capture["raw"][
            selected_positions
        ].cpu()
        trace["authentic_w2_exposure_selected"] = post_w2_capture["exposure"][
            selected_positions
        ].cpu()
        trace["authentic_weighted_selected"] = authentic_capture[
            "authentic_weighted_buffer"
        ...
        required_callsite = {
            "module_file", "co_filename", "co_firstlineno",
            "input", "route_weight", "output",
        }
        if set(route_weight_callsite_capture) != required_callsite:
            missing = sorted(required_callsite - set(route_weight_callsite_capture))
            extra = sorted(set(route_weight_callsite_capture) - required_callsite)
            raise RuntimeError(
                "ROUTE_WEIGHT_CALLSITE_CAPTURE_RED:"
                f"missing={missing}:extra={extra}"
            )
        trace["route_weight_callsite"] = {
            "module_file": str(
                Path(route_weight_callsite_capture["module_file"]).resolve()
            ),
            "co_filename": str(
                Path(route_weight_callsite_capture["co_filename"]).resolve()
            ),
            "co_firstlineno": int(
                route_weight_callsite_capture["co_firstlineno"]
            ),
            "input": _tensor_tap(route_weight_callsite_capture["input"]),
            "route_weight": _tensor_tap(
                route_weight_callsite_capture["route_weight"]
            ),
            "output": _tensor_tap(route_weight_callsite_capture["output"]),
        }
...
def _seal_attempt106_step3(
    progress_path: Path,
    *,
    pin: str,
    control_tap: dict[str, Any],
    control_flat66: float,
    traced_control_tap: dict[str, Any],
    first_divergent: str | None,
    pre_gemm_comparison: dict[str, Any],
    boundaries: dict[str, Any],
) -> None:
    """Persist the computed internal-boundary verdict before any kill gate."""
    atomic(progress_path, {
        ...
        "step3_resident_comparison": {
            "status": "EXACT" if first_divergent is None else "DIVERGENT",
            "first_divergent_boundary": first_divergent,
            "pre_gemm_comparison": pre_gemm_comparison,
            "boundaries": boundaries,
        },
...
def _sealed_runtime_expert_trace_ab(
    ...
    with torch.no_grad():
        if rank == 0:
            ...
            control_post_w2 = {
                "raw_bf16_w2": control_trace["w2_down"],
                "fp32_provider_exposure": control_trace["w2_down"].float(),
                "route_weight_multiply": control_trace["weighted_routed_output"],
                "reshape_order_gather_buffer": control_trace[
                    "full_weighted_routed_output"
                ].reshape(-1, control_trace["full_weighted_routed_output"].shape[-1]),
                "per_token_reduction": control_trace["authentic_routed_return"],
                "mlp_return": control_trace["mlp_return"],
                "residual_combine": control_trace["residual_combine"],
            }
            product_post_w2 = {
                "raw_bf16_w2": variant_trace["authentic_w2_raw_selected"],
                "fp32_provider_exposure": variant_trace[
                    "authentic_w2_exposure_selected"
                ],
                "route_weight_multiply": variant_trace["authentic_weighted_selected"],
                "reshape_order_gather_buffer": variant_trace[
                    "authentic_ordered_weighted_buffer"
                ].reshape(-1, variant_trace["authentic_ordered_weighted_buffer"].shape[-1]),
                "per_token_reduction": variant_trace["authentic_routed_return"],
                "mlp_return": variant_trace["mlp_return"],
                "residual_combine": variant_trace["residual_combine"],
            }
            post_w2_chain = _compare_post_w2_chain(
                control_post_w2, product_post_w2
            )
            diagnostic_decoder = _load_r20_grouped_decoder()
            product_expert = variant_layer.mlp.experts
            control_w2_weight = control_layer.mlp.experts.down_proj[204]
            product_w2_weight = diagnostic_decoder.sealed_bf16_full_weight(
                product_expert.packed_w2[204],
                product_expert.plane_source.wire_lut().reshape(-1).contiguous(),
                product_expert.su_w2[204], product_expert.sv_w2[204],
            ).transpose(0, 1).contiguous()
            control_w2_witness = _pre_gemm_tensor_witness(
                control_w2_weight, role="accepted_planesource_expert_204_w2_weight"
            )
            product_w2_witness = _pre_gemm_tensor_witness(
                product_w2_weight, role="attempt45_product_expert_204_w2_weight"
            )
            matrix_fields = ("sha256", "dtype", "shape", "stride")
            matrix_exact = all(
                control_w2_witness.get(field) == product_w2_witness.get(field)
                for field in matrix_fields
            )
            if not matrix_exact:
                raise RuntimeError("POST_W2_EXACT_MATRIX_BYTES_RED")

            import inspect
            provider_module = inspect.getmodule(type(product_expert))
            projection_frames = []
            for provider_type in type(product_expert).__mro__:
                implementation = provider_type.__dict__.get("_project")
                code = getattr(implementation, "__code__", None)
                if code is not None:
                    projection_frames.append({
                        "class": provider_type.__name__,
                        "module": provider_type.__module__,
                        "co_filename": str(Path(code.co_filename).resolve()),
                        "co_firstlineno": int(code.co_firstlineno),
                    })
            post_w2_receipt = {
                "schema": "banana-smasher-attempt46-post-w2-chain-v1",
                "status": (
                    "POST_W2_CHAIN_EXACT" if post_w2_chain["first_unequal_boundary"] is None
                    else "POST_W2_CHAIN_DIVERGENT"
                ),
                "task_id": TASK,
                "canonical_code_commit": pin,
                "basis_sha256": BASIS,
                ...
                "layer": 0,
                "same_w28_input": True,
                "exact_matrix_bytes": matrix_exact,
                "matrix_comparison": {
                    "control": control_w2_witness,
                    "product": product_w2_witness,
                },
                "executing_provider_module": type(product_expert).__module__,
                "executing_provider_module_file": str(
                    Path(str(getattr(provider_module, "__file__", ""))).resolve()
                ),
                "w2_projection_frames": projection_frames,
                "route_weight_callsite": variant_trace["route_weight_callsite"],
                **post_w2_chain,
                "created_unix": time.time(),
            }
            post_w2_receipt_name = os.environ.get(
                "POST_W2_CHAIN_RECEIPT_NAME", "ATTEMPT46_POST_W2_CHAIN.json"
            )
            if Path(post_w2_receipt_name).name != post_w2_receipt_name:
                raise RuntimeError("POST_W2_CHAIN_RECEIPT_NAME_RED")
            post_w2_path = root / post_w2_receipt_name
            post_w2_receipt["receipt_sha256"] = atomic(
                post_w2_path, post_w2_receipt
            )
            local["post_w2_chain"] = post_w2_receipt
            print(json.dumps({
                "path": str(post_w2_path),
                "status": post_w2_receipt["status"],
                "first_unequal_boundary": post_w2_chain["first_unequal_boundary"],
                "receipt_sha256": post_w2_receipt["receipt_sha256"],
            }, sort_keys=True), flush=True)
            # Seal the first unequal internal boundary before enforcing any
            # downstream trace-integrity kill gate.  A kill gate is diagnostic
            # evidence too; it must not discard the already-computed science.
            _seal_attempt106_step3(
                progress_path,
                pin=pin,
                control_tap=control_tap,
                control_flat66=control_flat66,
                traced_control_tap=traced_control_tap,
                first_divergent=local_first_divergent,
                pre_gemm_comparison=local["pre_gemm_comparison"],
...
def main() -> None:
    ...
    transport_timeout_seconds = 3600 if os.environ.get("W28_FULL_CALL_TREE_PATH") else 900
