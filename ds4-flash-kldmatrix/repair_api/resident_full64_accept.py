...
def _run_one_layer_with_expert_trace(
    ...
    def transparent_forward(
        ...
        try:
            with patch.object(torch, "gather", capture_authentic_gather), route_capture:
                authentic = original_forward(hidden_states, top_k_index, top_k_weights)
        finally:
    ...
    trace = {"route_key": route_key[order]}
    trace.update({
...
def _sealed_runtime_expert_trace_ab(
    ...
    if (
        pre_gemm.get("first_unequal_boundary") is None
        and not boundaries["gate"]["exact"]
    ):
        candidate = {
            "status": "MULTIPLE_GEMM_EXECUTION_MECHANISMS_PLAUSIBLE_STOP",
            "candidates": [
                "CUDA BF16 GEMM algorithm/workspace selection",
                "operand lifetime or allocator-address-sensitive kernel selection",
                "hidden library accumulation-mode state not represented by public tensor metadata",
            ],
            "repair_authorized": False,
        }
    else:
        candidate = {
            "status": "PRE_GEMM_BOUNDARY_LOCALIZED",
            "first_unequal_boundary": pre_gemm.get("first_unequal_boundary"),
            "repair_authorized": False,
        }
    receipt = {
