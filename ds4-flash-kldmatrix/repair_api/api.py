...
def _validate_published_pre_resume_start(
    ...
    *,
    ...
    sealed_u69_sha256 = "bf5b0166ad249814428f22aae6bd80ffc3325009b240aa694128e52aa4e505bf"
    ...
    sealed_fresh_pre_u63_checkpoint = (
        start_update == 63
        and start_meta.get("sha256") == sealed_u63_sha256
        and config.get("checkpoint_sha256") == sealed_u63_sha256
        and start_meta.get("optimizer_scheduler_lineage")
            == "fresh-published-pre-adam-lambdalr"
    )
    authenticated_fresh_pre_u63 = (
        sealed_fresh_pre_u63_checkpoint
        and config.get("recipe_id") == sealed_u22_config["recipe_id"]
        and config.get("published_pre_checkpoint_sha256") == published_pre_sha256
    ...
    sealed_fresh_pre_u64_checkpoint = (
        start_update == 64
        and start_meta.get("sha256") == sealed_u64_sha256
        and config.get("checkpoint_sha256") == sealed_u64_sha256
        and start_meta.get("optimizer_scheduler_lineage")
            == "fresh-published-pre-adam-lambdalr"
    )
    authenticated_fresh_pre_u64 = (
        sealed_fresh_pre_u64_checkpoint
        and config.get("recipe_id") == sealed_u22_config["recipe_id"]
        and config.get("published_pre_checkpoint_sha256") == published_pre_sha256
        and config.get("lr_scale") == 0.375091552734375
    ...
    sealed_fresh_pre_u65_checkpoint = (
        start_update == 65
        and start_meta.get("sha256") == sealed_u65_sha256
        and config.get("checkpoint_sha256") == sealed_u65_sha256
        and start_meta.get("optimizer_scheduler_lineage")
            == "fresh-published-pre-adam-lambdalr"
    )
    authenticated_fresh_pre_u65 = (
        sealed_fresh_pre_u65_checkpoint
        and config.get("recipe_id") == sealed_u22_config["recipe_id"]
        and config.get("published_pre_checkpoint_sha256") == published_pre_sha256
        and config.get("lr_scale") == 0.375091552734375
    ...
    sealed_fresh_pre_u69_checkpoint = (
        start_update == 69
        and start_meta.get("sha256") == sealed_u69_sha256
        and config.get("checkpoint_sha256") == sealed_u69_sha256
        and start_meta.get("optimizer_scheduler_lineage")
            == "fresh-published-pre-adam-lambdalr"
    )
    authenticated_fresh_pre_u69 = (
        sealed_fresh_pre_u69_checkpoint
        and config.get("recipe_id") == sealed_u22_config["recipe_id"]
        and config.get("published_pre_checkpoint_sha256") == published_pre_sha256
        and config.get("lr_scale") == 0.375091552734375
    ...
    if start_update <= 0 or start_update > 69:
        raise ArtifactError("published PRE scored resume must start inside U1..U69")
    if start_update % 4 and not (
        ...
        or sealed_fresh_pre_u63_checkpoint
        or sealed_fresh_pre_u65_checkpoint
        or sealed_fresh_pre_u69_checkpoint
    ):
        raise ArtifactError(
    ...
    if sealed_fresh_pre_u38_checkpoint and not authenticated_fresh_pre_u38:
        raise ArtifactError("published PRE scored resume identity drift")
    if sealed_fresh_pre_u63_checkpoint and not authenticated_fresh_pre_u63:
        raise ArtifactError("published PRE scored resume identity drift")
    if sealed_fresh_pre_u64_checkpoint and not authenticated_fresh_pre_u64:
        raise ArtifactError("published PRE scored resume identity drift")
    ...
    if sealed_fresh_pre_u68_checkpoint and not authenticated_fresh_pre_u68:
        raise ArtifactError("published PRE scored resume identity drift")
    if sealed_fresh_pre_u69_checkpoint and not authenticated_fresh_pre_u69:
        raise ArtifactError("published PRE scored resume identity drift")
    identity_fields = (
        "recipe_id",
    ...
    if (
        ...
        or authenticated_fresh_pre_u69
        ):
        return
...
class ResidentRepairAPI:
    ...
    def continue_two_spark_real(
        ...
        *,
        ...
        broad_rot16_milestone_limit = (
            80
            if config.get("scientific_identity")
                == "t_f76a1035 sealed U64 broad ROT16 sole-variable diet U65-to-U68"
                and config.get("controlled_windows_per_update") == 16
                and config.get("train_windows") == list(range(20, 84))
            else 64
        ...
        valid_milestones = (
            ...
            and all(
                value > start_update
                and value <= broad_rot16_milestone_limit
                and value % 4 == 0
