...
STATIC_W28_GROUPED_EXPERT_SHA256 = "e5831f2082419916f1564d4d792cdc4be6790e16be5f4af1d13445c4973dffa7"
...
def _sealed_builder_accumulate_routes(
    ...
    """Match the sealed builder's BF16 weighted rows and reshape-sum return."""
    # Attempt51 proved the weighted rows exact and localized the first unequal
    # operation to this wrapper's ascending-expert index_add_.  The reference
    # restores token/slot geometry and performs one reshape+sum over BF16 rows.
    # Attempt53 authenticated FP32 provider exposure on both rails but proved
    # the first unequal executed tensor at route multiplication: the builder
    # rounds that exposure back to BF16 before multiplying BF16 route weights.
    weighted = (
        routed_output.to(hidden_states.dtype)
        * top_k_weights.reshape(-1, 1).to(dtype=hidden_states.dtype)
    )
...
def _sealed_builder_down_projection(
    ...
    *,
    ...
    """Execute builder-native expert-local BF16 F.linear for every W2 call."""
    import torch

    down = torch.empty(
        (x.shape[0], sv_w2.shape[1]), device=x.device, dtype=torch.bfloat16
    )
    for expert_index in torch.unique(assignments, sorted=True).tolist():
        mask = assignments == expert_index
        expert_x = x[mask].to(torch.bfloat16).contiguous()
    ...
    # Attempt46 authenticated this ABI separately from the native BF16 GEMM:
    # the provider exposes the already-rounded BF16 values as FP32, while the
    # route-multiply boundary performs the next BF16 round before assembly.
    exposure = down.float()
    observer = globals().get("_POST_W2_CHAIN_OBSERVER")
    if callable(observer):
        observer(down, exposure)
    return exposure


def _write_l000_w2_runtime_header(instance: Any, x: Any, value: Any) -> None:
    """Seal the provider/code/dtype actually executing the first live W2 call."""
    if int(getattr(instance, "L", -1)) != 0 or getattr(
        instance, "_sealed_w2_runtime_header_done", False
    ):
        return
    required = os.environ.get("REQUIRE_W2_RUNTIME_HEADER", "0") == "1"
    path_value = os.environ.get("W2_RUNTIME_HEADER_PATH")
    build_value = os.environ.get("BUNDLE_BUILD_RECEIPT_PATH")
    if not path_value or not build_value:
        if required:
            raise RuntimeError("L000_W2_RUNTIME_HEADER_CONFIGURATION_MISSING")
        return
    build_path = Path(build_value).expanduser().resolve()
    if not build_path.is_file():
        raise RuntimeError("L000_W2_BUNDLE_BUILD_RECEIPT_MISSING")
    bundle_build = json.loads(build_path.read_text())
    if bundle_build.get("canonical_resident_sha256") != bundle_build.get(
        "bundle_resident_sha256"
    ):
        raise RuntimeError("L000_W2_BUNDLE_SOURCE_SHA_MISMATCH")
    module = sys.modules.get(type(instance).__module__)
    module_file = Path(str(getattr(module, "__file__", ""))).resolve()
    frames = []
    for cls in type(instance).__mro__:
        function = cls.__dict__.get("_project")
    ...
    row = {
        "schema": "banana-smasher-l000-w2-runtime-header-v1",
        "status": "PASS_L000_W2_RUNTIME_HEADER",
        "task_id": os.environ.get("BANANA_SMASHER_TASK_ID"),
        "executing_provider_module": type(instance).__module__,
        "executing_provider_module_file": str(module_file),
        "w2_projection_frames": frames,
        "live_w2_input_dtype": str(x.dtype),
        "live_w2_output_dtype": str(value.dtype),
    ...
    instance._sealed_w2_runtime_header_done = True
...
def _bind_sealed_gate_up_projection(
    ...
    *,
    ...
    down_projection: Any = None,
) -> Any:
    """Replace 942c projections with builder-native BF16 F.linear boundaries."""
    explicitly_bound = (
    ...
    if down_projection is not None and not callable(down_projection):
        raise ArtifactError("sealed BF16 down projection helper is unavailable")
    ...
    class SealedCombinedGateUpProjectionExpert(provider_class):
        ...
        def _project(self, *args: Any, **kwargs: Any) -> Any:
            ...
            if projection == "w3":
                ...
                self._sealed_combined_up = None
                return up
            if projection == "w2" and down_projection is not None:
                if len(args) < 5:
                    raise RuntimeError("SEALED_DOWN_PROJECTION_CALL_GEOMETRY_DRIFT")
                value = down_projection(
                    args[1], args[2], self.packed_w2, args[4], self.su_w2, self.sv_w2
                )
                _write_l000_w2_runtime_header(self, args[1], value)
                if self._sealed_aligned_positions is not None:
                    witness = getattr(self, "_sealed_gate_up_last_witness", None)
                    ...
                    aligned["activated"] = self._sealed_tensor_witness(
                        args[1][self._sealed_aligned_positions]
                    )
                    aligned["w2_down"] = self._sealed_tensor_witness(
                        value[self._sealed_aligned_positions]
                    )
                return value
...
def _install_runtime_modules(config: Mapping[str, Any]) -> Any:
    ...
    if (
        ...
        if callable(full_weight_builder):
            ...
            def bound_down_projection(*args: Any) -> Any:
                return _sealed_builder_down_projection(
                    *args, full_weight_builder=full_weight_builder
                )
