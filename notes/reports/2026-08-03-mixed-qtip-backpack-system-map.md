...
A fresh run may begin with these declared external authorities:
...
8. supported Linux ARM64 SM120/SM121 hardware for the physical gate.

No prebuilt mixed pack, pre-repair model, repair checkpoint, service image, or runtime overlay is an external prerequisite. Each must be generated by the run.

## 4. Standalone run-root layout

The target portable run root is:
...
Final export reads the immutable selected source plus the same run's:
...
| Surface | Implemented now | Required for easy first-run deployment |
|---|---|---|
| explicit dimensions | `smash backpack-dimensions` | pipeline generates all ledgers and calls it automatically |
| exact selection | `smash knapsack` | one run manifest builds/merges inputs and seals assignment automatically |
| pre-repair export | `smash export` | run-root API chooses the same-run selected source automatically |
| repair export | all seven repair arguments are fail-closed | run-root API resolves them from same-run descriptors; no manual path discovery |
...
| receipts | hashes and statuses exist | remove private absolute paths; use run-root-relative paths/content IDs |
...
The standalone pipeline must stop before deployment if any of these occur:
...
- output path escapes the run root;
