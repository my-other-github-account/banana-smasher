...
def _persist_l000_raw_tensors(
    root: Path,
    rank: int,
    *,
    mlp_input: Any,
    control_mlp_output: Any,
    product_mlp_output: Any,
) -> dict[str, dict[str, Any]]:
    """Persist immutable same-forward L000 operands without changing scorer math."""
    raw_root = root / "raw_l000_same_forward"
    raw_root.mkdir(parents=True, exist_ok=True)
    tensors = {
        "pre_layer_input": mlp_input,
        "true_builder_mlp_output": control_mlp_output,
        "product_mlp_output": product_mlp_output,
    }
    receipts: dict[str, dict[str, Any]] = {}
    for name, value in tensors.items():
        path = raw_root / f"{name}.rank{rank}.pt"
        torch.save(value.detach().cpu(), path)
        receipts[name] = {
            "path": str(path),
            "file_sha256": sha(path),
            "tensor": _tensor_tap(value),
        }
    return receipts
...
def _l000_moe_subtap(
    engine: Any, *, window: int, root: Path, rank: int, pin: str,
    checkpoint_path: Path, projection_binding: dict[str, Any],
) -> dict[str, Any]:
    """Compare L000 routed-MoE subops on the exact TRUE-builder MLP input."""
    ...
    def capture_mlp(layer: Any, hidden: Any) -> tuple[Any, Any, Any]:
        seen: dict[str, Any] = {}

        def before(_module: Any, args: Any) -> None:
            seen["input"] = args[0].detach()

        def after(_module: Any, _args: Any, output: Any) -> None:
            seen["output"] = output.detach()

        h0 = layer.mlp.register_forward_pre_hook(before)
        h1 = layer.mlp.register_forward_hook(after)
        try:
            output, _attention = _run_one_layer_with_attention(
                engine, layer, hidden, ids
            )
        finally:
            h0.remove()
            h1.remove()
        if tuple(seen) != ("input", "output"):
            raise RuntimeError("L001_MLP_HOOK_COVERAGE_RED")
        return output, seen["input"], seen["output"]

    def route(mlp: Any, hidden_states: Any) -> tuple[Any, Any, Any, Any]:
        flat = hidden_states.reshape(-1, hidden_states.shape[-1])
        if bool(mlp.is_hash):
            logits, weights, indices = mlp.gate(hidden_states, ids)
        else:
            logits, weights, indices = mlp.gate(hidden_states)
        return flat, logits, weights, indices

    def control_trace(mlp: Any, hidden_states: Any) -> dict[str, Any]:
        flat, logits, weights, indices = route(mlp, hidden_states)
        token_index = (
            torch.arange(flat.shape[0], device=flat.device)
            .unsqueeze(1).expand_as(indices).reshape(-1)
        )
        expert_index = indices.reshape(-1).to(torch.int64)
        expert_input = flat[token_index].contiguous()
        gate = torch.empty(
            (expert_input.shape[0], mlp.experts.intermediate_dim),
            device=flat.device, dtype=torch.bfloat16,
        )
        up = torch.empty_like(gate)
        down = torch.empty(
            (expert_input.shape[0], flat.shape[-1]),
            device=flat.device, dtype=torch.bfloat16,
        )
        active = torch.unique(expert_index, sorted=True)
        first_expert = int(active[0].item())
        for expert in active.tolist():
            mask = expert_index == expert
            gate_up = F.linear(
                expert_input[mask], mlp.experts.gate_up_proj[expert]
            )
            gate[mask], up[mask] = gate_up.chunk(2, dim=-1)
        activation = mlp.experts._apply_gate(torch.cat((gate, up), dim=-1))
        for expert in active.tolist():
            mask = expert_index == expert
            down[mask] = F.linear(
                activation[mask], mlp.experts.down_proj[expert]
            )
        weighted = (
            down * weights.reshape(-1, 1).to(down.dtype)
        ).to(flat.dtype)
        final = torch.zeros_like(flat)
        for expert in active.tolist():
            mask = expert_index == expert
            final.index_add_(0, token_index[mask], weighted[mask])
        shared = mlp.shared_experts(hidden_states)
        output = final.view_as(hidden_states) + shared
        return {
            ...
            "w2": mlp.experts.down_proj[first_expert],
            "down_projection": down,
            "weighted": weighted,
            "per_slot": weighted.view(indices.shape[0], indices.shape[1], -1),
            "return_assembly": final,
            "residual": shared,
            "output": output,
            "first_active_expert": first_expert,
        }

    def variant_trace(mlp: Any, hidden_states: Any) -> tuple[dict[str, Any], dict[str, Any]]:
        flat, logits, weights, indices = route(mlp, hidden_states)
        token_index = (
            torch.arange(flat.shape[0], device=flat.device)
            .unsqueeze(1).expand_as(indices).reshape(-1)
        ...
        expert_input = flat[token_index].contiguous()
        expert = mlp.experts
        lut_master = expert.plane_source.wire_lut().reshape(-1).contiguous()
        calls: list[dict[str, Any]] = []

        def project(name: str, x: Any, packed: Any, su: Any, sv: Any) -> Any:
            value = expert._project(
                name, x, expert_index, packed, lut_master, su, sv
            )
            calls.append({
                "projection": name, "input_dtype": str(x.dtype),
                "output_dtype": str(value.dtype), "shape": list(value.shape),
            })
            return value

        gate = project("w1", expert_input, expert.packed_w1, expert.su_w1, expert.sv_w1)
        up = project("w3", expert_input, expert.packed_w3, expert.su_w3, expert.sv_w3)
        activation = expert.act(gate) * up
        down = project("w2", activation, expert.packed_w2, expert.su_w2, expert.sv_w2)
        weighted = (
            down * weights.reshape(-1, 1).float()
        ).to(flat.dtype)
        from repair_api.modern_green_resident import _sealed_builder_accumulate_routes
        final = _sealed_builder_accumulate_routes(flat, down, indices, weights)
        authentic_final = expert(flat, indices, weights)
        if not torch.equal(final, authentic_final):
            raise RuntimeError("L001_RECONSTRUCTED_RETURN_NOT_AUTHENTIC")
        shared = mlp.shared_experts(hidden_states)
        output = final.view_as(hidden_states) + shared
        first_expert = int(torch.unique(expert_index, sorted=True)[0].item())
        decoder = _load_r20_grouped_decoder()
        w2 = decoder.sealed_bf16_full_weight(
            expert.packed_w2[first_expert], lut_master,
            expert.su_w2[first_expert], expert.sv_w2[first_expert],
        ).transpose(0, 1).contiguous()
        dtype_assertion = {
            "layer": 0,
            "layer_listed_in_global_binding": 0 in projection_binding.get(
                "bf16_flinear_repair", {}
            ).get("layers", []),
            "instance_projection_override_present": "_r20_original_project" in expert.__dict__,
            "calls": calls,
            "w2_output_is_bfloat16": str(down.dtype) == "torch.bfloat16",
            "status": (
                "PHYSICALLY_APPLIED_BF16"
                if str(down.dtype) == "torch.bfloat16" else
                "SCOPE_BOUND_BUT_W2_EXPOSED_NON_BF16"
            ),
        }
        return ({
            "router_logits": logits,
    ...
    with torch.no_grad():
        if rank == 0:
            ...
            variant_l0 = engine.student.model.model.layers[0]
            ...
            if int(free_bytes) < (22 << 30):
                raise RuntimeError(f"L000_MOE_SUBTAP_GPU_HEADROOM_RED:{free_bytes}:{22 << 30}")
            ...
            control_model.eval()
            sd0 = builder.build_layer_sd(
                0, engine.student.wm, engine.student.get_tensor, "planes", planes
            )
            control_l0 = builder.materialize_layer(
                control_model, 0, sd0, engine.student.config
            )
            _control_out, control_mlp_input, control_mlp_output = capture_mlp(
                control_l0, hidden.clone()
            )
            variant_mlp_input = control_mlp_input
            if bool(variant_l0.mlp.is_hash):
                variant_mlp_output = variant_l0.mlp(
                    variant_mlp_input, input_ids=ids
                )
            else:
                variant_mlp_output = variant_l0.mlp(variant_mlp_input)
            ...
            mlp_input_gate = _compare_tensor_boundary(
                control_mlp_input, variant_mlp_input
            )
            if not mlp_input_gate["exact"]:
                raise RuntimeError("L000_MOE_SUBTAP_MLP_INPUT_RED")
            control = control_trace(control_l0.mlp, control_mlp_input)
            variant, dtype_assertion = variant_trace(
                variant_l0.mlp, variant_mlp_input
            )
            order = (
                "expert_input", "gate", "up", "activation", "w2",
                "down_projection", "weighted", "per_slot",
                "return_assembly", "residual", "output",
            )
            boundaries = {
                name: _compare_tensor_boundary(control[name], variant[name])
                for name in order
            }
            first = next(
            ...
            local.update({
                "run6970_control_moe_return_sha256": "71aad538858c0515162f06feadc9a95e9ed92b037a50a2c8c19b53e278fc4987",
                "run6970_product_moe_return_sha256": "c240b5c0327943c5e5fdfdde6d714fbd577e4c319ef1c3183e37370648fec85d",
                "run6970_router_output_exact": True,
                "mlp_input_gate": mlp_input_gate,
                "authentic_mlp_control": _tensor_tap(control_mlp_output),
                "authentic_mlp_variant": _tensor_tap(variant_mlp_output),
                "raw_same_forward": raw_same_forward,
                "boundary_order": list(order),
                "boundaries": boundaries,
                "first_divergent_subop": first,
                "runtime_dtype_assertion": dtype_assertion,
                "control_source_binding": control_binding,
                "layer_class": {
                    "control": type(control_l0.mlp).__name__,
                    "variant": type(variant_l0.mlp).__name__,
                    "is_hash": bool(control_l0.mlp.is_hash),
                },
            })
            del control_l0, control_model, sd0, control, variant
            gc.collect()
            torch.cuda.empty_cache()
    gathered: list[Any] = [None, None]
    engine.dist.all_gather_object(gathered, local)
    result = gathered[0]
    receipt = {
        "schema": "banana-smasher-l000-moe-subtap-v1",
        "status": (
            "EXACT_L000_MOE_PARITY"
            if result.get("first_divergent_subop") is None
            else "L000_MOE_DIVERGENCE_LOCALIZED"
        ),
        "task_id": TASK, "canonical_code_commit": pin,
        "basis_sha256": BASIS, "checkpoint_sha256": CHECKPOINT,
    ...
    path = root / "receipts" / f"L000_MOE_SUBTAP.rank{rank}.json"
    receipt["receipt_sha256"] = atomic(path, receipt)
    print(json.dumps({"receipt_path": str(path), **receipt}, sort_keys=True), flush=True)
    return receipt
...
def _load_r20_grouped_decoder() -> Any:
    ...
    path = Path("/home/dnola/missions/FULL64_REPAIR_t_686041f5/run6940_balanced64_b7e7bbf/canonical-b7e7bbf/ds4-flash-kldmatrix/repair_api/assets/fast_k2_grouped.py")
    # Bind the decoder bytes shipped by canonical commit da2a586.  The older
    # diagnostic-only digest predates the accepted 106BR grouped wrapper and
    # incorrectly rejects the provider that the resident engine actually uses.
...
def _sealed_full_weight_projection(
    ...
    *, full_weight_builder: Any,
    ...
    counts = torch.bincount(assignments, minlength=experts).to(torch.int64)
    output = torch.empty(
    ...
    for expert_index in torch.nonzero(counts, as_tuple=False).flatten().tolist():
        ...
        output[mask] = torch.nn.functional.linear(expert_x, full_weight.T.contiguous())
    # Match the canonical resident adapter in modern_green_resident.py:548-556:
    # execute the GEMM in BF16, then expose the projection to the provider as FP32.
    return output.float()
...
def _install_r20_full_weight_projection(
    engine: Any, *, layers: set[int] | None = None
) -> dict[str, Any]:
    ...
    for layer_index, expert in sorted(engine.student.experts.items()):
        if layers is not None and int(layer_index) not in layers:
            continue
...
def main() -> None:
    ...
    repair_scope = os.environ.get("FAST_K2_GLOBAL_BF16_FLINEAR_SCOPE", "")
    if repair_scope:
        if repair_scope == "all":
            repair_layers = None
        elif repair_scope == "L000":
            repair_layers = {0}
        else:
            raise RuntimeError("FAST_K2_GLOBAL_BF16_FLINEAR_SCOPE_INVALID")
        projection_binding["bf16_flinear_repair"] = (
            _install_r20_full_weight_projection(engine, layers=repair_layers)
        )
    torch.cuda.synchronize()
    ...
    if os.environ.get("L000_MOE_SUBTAP_ONLY", "0") == "1":
        _l000_moe_subtap(
            engine, window=28, root=root, rank=rank, pin=pin,
            checkpoint_path=checkpoint_path,
