...
Teacher mode (--mode bf16):
  ...
  fp4 e2m1 block-32 e8m0 routed experts), HF deepseek_v4 SDPA graph,
...
def main():
    ...
    ap.add_argument("--attention-implementation", choices=("eager", "sink-sdpa"),
                    default="sink-sdpa",
                    help="attention equation and matching mask representation")
    a = ap.parse_args()
    ...
    from transformers.models.deepseek_v4.modeling_deepseek_v4 import (
        ALL_ATTENTION_FUNCTIONS, DeepseekV4RotaryEmbedding)
    ...
    # Stock Transformers SDPA ignores DeepseekV4's per-head attention sinks.
    # Reuse the canonical public-API adapter which encodes each sink as an
    # appended zero-value KV token; do not fork another attention equation.
    sink_implementation = "official_k2_sink_corrected_sdpa"
    implementation = ("eager" if a.attention_implementation == "eager"
                      else sink_implementation)
    if implementation == sink_implementation:
        ALL_ATTENTION_FUNCTIONS.register(
            implementation, ModernGreenResidentEngine._sink_corrected_sdpa_forward
        )
    model.config._attn_implementation = implementation
    model.config._attn_implementation_internal = implementation
    config = model.config
    model.eval()
    ...
    for c0 in range(0, len(todo), a.chunk):
        ...
        with torch.no_grad():
            ...
            masks, hidden = [], []
            # Masking utilities dispatch by their own public implementation
            # registry. Build the semantically equivalent additive eager mask,
            # then restore the sink-corrected runtime adapter before layer forwards.
            # Additive masks preserve compressed block_bias values when
            # DeepseekV4Attention extends the pre-concat sliding mask.
            mask_implementation = "eager"
            config._attn_implementation = mask_implementation
            config._attn_implementation_internal = mask_implementation
            try:
