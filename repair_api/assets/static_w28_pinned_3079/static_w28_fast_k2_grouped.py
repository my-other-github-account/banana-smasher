...
def block_hadamard_128(
    value: torch.Tensor, *, route_rows_per_sample: int | None = None
) -> torch.Tensor:
    """Apply H128 with a fixed per-window launch geometry when requested."""
    if value.shape[-1] % 128:
        raise ValueError("last dimension must be divisible by 128")
    if route_rows_per_sample is None:
        route_chunks = (value,)
    else:
        rows = int(route_rows_per_sample)
        if value.ndim != 2 or rows < 1 or value.shape[0] % rows:
            raise ValueError("packed H128 route rows must divide fixed window geometry")
        route_chunks = value.split(rows, dim=0)
    if _FWHT_BACKEND == "quack":
        import math
        from quack.hadamard import hadamard_transform

        if not value.is_cuda or value.dtype not in (torch.float16, torch.bfloat16, torch.float32):
            raise RuntimeError("required Quack FWHT needs contiguous CUDA fp16/bf16/fp32 blocks")
        _FWHT_STATS["quack_calls"] += 1
        transformed = [
            hadamard_transform(
                chunk.reshape(-1, 128).contiguous(), scale=1 / math.sqrt(128)
            ).reshape_as(chunk)
            for chunk in route_chunks
        ]
        return transformed[0] if len(transformed) == 1 else torch.cat(transformed, dim=0)
    ...
    transformed = [
        (chunk.reshape(-1, 128).contiguous() @ matrix).reshape_as(chunk)
        for chunk in route_chunks
    ]
    return transformed[0] if len(transformed) == 1 else torch.cat(transformed, dim=0)
...
_STATS = {
    ...
    "cpu_relay_bytes": 0,
    "sort_calls": 0,
    "route_metadata_builds": 0,
    "route_metadata_reuses": 0,
    "sorted_input_reuses": 0,
    "routed_rows": 0,
...
def grouped_route_metadata(
    assignments: torch.Tensor,
    experts: int,
    *,
    input_tensor: torch.Tensor | None = None,
) -> dict[str, torch.Tensor | int]:
    """Build immutable routing metadata once for all three projections."""
    if assignments.ndim != 1 or assignments.dtype != torch.int64:
        raise ValueError("assignments must be one-dimensional int64")
    ...
    offsets = torch.cat(
        (
            torch.zeros(1, device=assignments.device, dtype=torch.int32),
    ...
    expert_ids = torch.arange(experts, device=assignments.device, dtype=torch.int32)
    work_experts = torch.repeat_interleave(
        expert_ids, chunks.to(torch.int64)
    ).contiguous()
    chunk_prefix = chunks.cumsum(0, dtype=torch.int32) - chunks
    work_ordinal = torch.arange(
        work_experts.numel(), device=assignments.device, dtype=torch.int32
    )
    ...
    active_experts = (
        torch.nonzero(counts, as_tuple=False).flatten().to(torch.int32).contiguous()
    )
    _STATS["sort_calls"] += 1
    _STATS["route_metadata_builds"] += 1
    metadata: dict[str, torch.Tensor | int] = {
        "assignments_data_ptr": assignments.data_ptr(),
        "assignments_numel": assignments.numel(),
        "experts": experts,
        "order": order,
        "inverse_order": inverse_order,
        "sorted_assignments": sorted_assignments,
        "offsets": offsets,
        "work_experts": work_experts,
        "work_starts": work_starts,
        "active_experts": active_experts,
    }
    if input_tensor is not None:
        if input_tensor.ndim != 2 or input_tensor.shape[0] != assignments.numel():
            raise ValueError("cached route input geometry mismatch")
        metadata["input_data_ptr"] = input_tensor.data_ptr()
        metadata["sorted_input"] = input_tensor[order].contiguous()
    return metadata
...
def grouped_packed_projection(
    ...
    *,
    ...
    route_metadata: dict[str, torch.Tensor | int] | None = None,
) -> torch.Tensor:
    ...
    if route_metadata is None:
        route_metadata = grouped_route_metadata(assignments, experts)
    else:
        if (
            route_metadata.get("assignments_data_ptr") != assignments.data_ptr()
            or route_metadata.get("assignments_numel") != assignments.numel()
            or route_metadata.get("experts") != experts
        ):
            raise ValueError("route metadata does not belong to these assignments")
        _STATS["route_metadata_reuses"] += 1
    order = route_metadata["order"]
    inverse_order = route_metadata["inverse_order"]
    sorted_assignments = route_metadata["sorted_assignments"]
    offsets = route_metadata["offsets"]
    work_experts = route_metadata["work_experts"]
    work_starts = route_metadata["work_starts"]
    active_experts = route_metadata["active_experts"]
    assert isinstance(order, torch.Tensor)
    assert isinstance(inverse_order, torch.Tensor)
    assert isinstance(sorted_assignments, torch.Tensor)
    assert isinstance(offsets, torch.Tensor)
    assert isinstance(work_experts, torch.Tensor)
    assert isinstance(work_starts, torch.Tensor)
    assert isinstance(active_experts, torch.Tensor)
    cached_sorted_input = route_metadata.get("sorted_input")
    if (
        route_metadata.get("input_data_ptr") == x.data_ptr()
        and isinstance(cached_sorted_input, torch.Tensor)
    ):
        sorted_x = cached_sorted_input
        _STATS["sorted_input_reuses"] += 1
    else:
    ...
    sorted_output = (
        block_hadamard_128(
            inner, route_rows_per_sample=route_rows_per_sample
        ) * sv[sorted_assignments].float()
    )
