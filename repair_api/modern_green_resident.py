...
def _published_pre_controlled_schedule(
    ...
    weight_key = "per_window_teacher_kl_weights"
    order_key = "per_window_teacher_kl_window_order"
    raw_weights = config.get(weight_key)
    raw_order = config.get(order_key)
    if (raw_weights is None) != (raw_order is None):
        raise ArtifactError("fresh PRE per-window teacher KL weight binding is incomplete")
    if raw_weights is not None:
        if (
            not isinstance(raw_weights, list)
            or len(raw_weights) != windows_per_update
            or not all(
            ...
            or not math.isclose(math.fsum(float(value) for value in raw_weights), 1.0,
                                rel_tol=0.0, abs_tol=1e-12)
        ):
            raise ArtifactError("fresh PRE per-window teacher KL weights must be positive and sum to one")
        if (
            not isinstance(raw_order, list)
            or len(raw_order) != windows_per_update
            or len(set(raw_order)) != windows_per_update
            or any(not isinstance(value, int) or isinstance(value, bool) for value in raw_order)
            or any(list(row.get("windows", [])) != raw_order for row in rows)
        ):
            raise ArtifactError("fresh PRE per-window teacher KL window order drift")
        if source_to.get(weight_key) != raw_weights or source_to.get(order_key) != raw_order:
            raise ArtifactError("fresh PRE per-window teacher KL source binding drift")
    elif weight_key in source_to or order_key in source_to:
        raise ArtifactError("fresh PRE per-window teacher KL source/config drift")
...
def _weighted_window_loss(
    torch: Any,
    token_values: list[Any],
    windows: list[int],
    config: Mapping[str, Any],
) -> Any:
    """Reduce one pipeline group while preserving declared global window weights."""
    if len(token_values) != len(windows) or not token_values:
        raise ArtifactError("per-window teacher KL loss group shape drift")
    raw_weights = config.get("per_window_teacher_kl_weights")
    if raw_weights is None:
        return torch.stack([values.mean() for values in token_values]).mean()
    raw_order = config.get("per_window_teacher_kl_window_order")
    if not isinstance(raw_order, list) or not isinstance(raw_weights, list):
        raise ArtifactError("per-window teacher KL runtime binding is incomplete")
    weight_by_window = {
        int(window): float(weight) for window, weight in zip(raw_order, raw_weights)
    }
    if any(window not in weight_by_window for window in windows):
        raise ArtifactError("per-window teacher KL runtime window order drift")
    weighted = [
        values.mean() * weight_by_window[window]
        for values, window in zip(token_values, windows)
    ]
    return torch.stack(weighted).mean() * len(raw_order)
...
class ModernGreenResidentEngine:
    ...
    def _loss_group(self, hidden: Any, group: list[int]) -> Any:
        ...
        if self.tailfix_wholesale:
            ...
            return loss
        return _weighted_window_loss(
            self.torch, token_values, group, self.config
        )
