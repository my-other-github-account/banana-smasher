...
def _load_sealed_w2_source_layer(
    model_root: Any, layer: int, device: Any
) -> dict[str, Any]:
    ...
    for projection in ("w1", "w2", "w3"):
        ...
        for expert in range(256):
            ...
            packed_rows.append(
                _pack_sealed_w2_source_codes(handle.get_tensor(weight_key))
            )
            scale_rows.append(
                handle.get_tensor(scale_key).view(torch.uint8).contiguous()
            )
    ...
    class SealedCombinedGateUpProjectionExpert(provider_class):
        ...
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            ...
            if self._sealed_source_w2_enabled:
                ...
                # Source-bound two-bit codes replace the inert QTIP-K2 wire.
                for projection in ("w1", "w2", "w3"):
        ...
        def forward(
            ...
            if self._sealed_source_w2_enabled:
                ...
                gate = _sealed_w2_source_projection(
                    routed_hidden,
                    expert_index,
                    self._sealed_w2_codes_w1,
                    self._sealed_w2_scales_w1,
                )
                up = _sealed_w2_source_projection(
                    routed_hidden,
                    expert_index,
                    self._sealed_w2_codes_w3,
                    self._sealed_w2_scales_w3,
                )
                ...
                routed_output = _sealed_w2_source_projection(
                    activated,
                    expert_index,
                    self._sealed_w2_codes_w2,
                    self._sealed_w2_scales_w2,
                )
