    ...
    def _restore_measured_gradient_lr_calibration(self) -> None:
        ...
        if needs_scheduler_scale:
            self.scheduler.base_lrs = [
                float(value) / divisors[surface]
                for surface, value in zip(("luts", "norms", "outputs"), self.scheduler.base_lrs)
            ]
            self.scheduler._last_lr = [
                float(value) / divisors[surface]
                for surface, value in zip(("luts", "norms", "outputs"), self.scheduler._last_lr)
            ]
