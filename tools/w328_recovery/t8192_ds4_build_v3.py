...
def atomic_hidden_checkpoint(root, *, layer, wins, hidden):
    """Persist a durable, immutable post-layer hidden-state boundary."""
    root = os.fspath(root)
    os.makedirs(root, exist_ok=True)
    destination = os.path.join(root, f"hidden_after_L{layer:03d}.pt")
    if os.path.exists(destination):
        raise FileExistsError(destination)
    payload = {
        "schema": "banana-smasher.w328.hidden_checkpoint.v1",
        "completed_layer": int(layer),
        "next_layer": int(layer) + 1,
        "window_order": [int(win) for win in wins],
        "hidden": [value.detach().to("cpu").contiguous() for value in hidden],
    }
    descriptor, temporary = tempfile.mkstemp(
        dir=root, prefix=f".{os.path.basename(destination)}.", suffix=".tmp"
    )
    os.close(descriptor)
    try:
    ...
    for c0 in range(0, len(todo), a.chunk):
        ...
        with torch.no_grad():
            ...
            for L in range(NL):
                ...
                if a.hidden_checkpoint_dir:
                    checkpoint = atomic_hidden_checkpoint(
                        a.hidden_checkpoint_dir, layer=L, wins=wins, hidden=hidden)
                    log(f"  L{L:02d} checkpoint {checkpoint}")
                with open(hb, "w") as f:
