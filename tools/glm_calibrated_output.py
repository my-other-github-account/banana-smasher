...
try:
 ...
 cor=json.loads((out/spec.get('correction_result_name','CORRECTION_RESULT.json')).read_text());assert cor['status']=='PASS_SINGLE_CELL_CLEAN_CALIBRATION';artifact=p.Path(cor['artifact']);assert h(artifact.read_bytes())==cor['artifact_sha256'];model=p.Path(spec['model_root']);assert h((model/'model.safetensors.index.json').read_bytes())==spec['intended_basis'];native,_=load_glm_fp8_weight(model,3,0,'fused13');runner.QTIP=p.Path(spec['qtip_root']);mods=runner.load_official_qtip();payload=torch.load(artifact,map_location='cpu',weights_only=True,mmap=True);payload['reconstructed_weight']=native.half();decoded,decode_receipt=runner.decode_packed(payload,mods[3],torch.device('cuda'));decoded=decoded.half().cpu();assert torch.isfinite(decoded).all();del payload;torch.cuda.empty_cache()
