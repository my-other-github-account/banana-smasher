...
report={'schema':'native-frozen-train-l003-capture-v1','status':'RUNNING','started':time.time(),'canonical_commit':spec['canonical_commit'],'scope':'external-document calibration, 1024 tokens, native layers0..3 only; no solver or evaluation forward','clean_fit_admission':True,'reason':'independent external document; overlap adjudication bound in spec','windows':[]}
try:
 ...
 for li,layer in enumerate(language.layers[:4]):
  ...
  with torch.no_grad():
   for ordinal,(row,ids,a,prior) in enumerate(zip(rows,tokens,activations,previous,strict=True)):
    ...
    if li==3:
     ...
     q=root/'captures'/f'xmoe_L003_win{ordinal:04d}.pt';q.parent.mkdir(exist_ok=True);tmp=q.with_name('.'+q.name+'.tmp');torch.save({'x':x,'topk':ki,'w':w,'item_id':row['item_id'],'ordinal':ordinal,'RL':1024,'layer':3,'split':'external-document-calibration','win':ordinal,'ledger_sha256':report['ledger_sha256']},tmp);os.replace(tmp,q)
