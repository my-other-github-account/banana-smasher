...
assert (expert==0 and projection=='fused13') or spec.get('prepared_config'), 'non-default expert/projection requires prepared config'
...
try:
 ...
 if spec.get('prepared_config'):
  ...
  for capfile in spec['capture_results']:
   part=json.loads(p.Path(capfile).read_text());assert part['status']=='PASS_NATIVE_CAPTURE' and part['ledger_sha256']==report['training_ledger_sha256'];assert part.get('layer',3)==layer, 'capture layer mismatch';windows.extend(part['windows'])
  ...
  for row in cap['windows']:
   original=p.Path(row['path']);assert h(original.read_bytes())==row['sha256'];d=torch.load(original,map_location='cpu',weights_only=True);assert d.get('layer',3)==layer, 'capture tensor layer mismatch';d['win']=row['ordinal'];q=cr/f'xmoe_L{layer:03d}_win{row["ordinal"]:04d}.pt';assert not q.exists();sp._atomic_torch(q,d);done=q.with_suffix('.pt.DONE.json');save(done,{'status':'PASS','meaning':'physical bytes verified, not clean admission','md5':hashlib.md5(q.read_bytes()).hexdigest(),'layer':layer,'win':row['ordinal'],'item_id':row['item_id'],'original_capture_sha256':row['sha256'],'ledger_sha256':report['training_ledger_sha256'],'clean_fit_admission':True});members.append({'window':row['ordinal'],'item_id':row['item_id'],'capture':{'path':str(q),'bytes':q.stat().st_size,'sha256':h(q.read_bytes())},'capture_done':{'path':str(done),'bytes':done.stat().st_size,'sha256':h(done.read_bytes())}})
  assert len(members)==spec['windows']
  hm=root/f'L{layer:03d}_HESSIAN_MANIFEST.json';save(hm,{'schema':'banana-smasher-hessian-layer-manifest-v1','status':'PASS','meaning':'authenticated training-only capture inventory; not scientific clean admission','layer':layer,'windows':spec['windows'],'capture_root':str(cr),'members':members,'capture_inventory_seal_sha256':h(json.dumps(cap,sort_keys=True).encode()),'clean_fit_admission':True})
  old=p.Path(spec['template_config']);config=json.loads(old.read_text());assert config['geometry']['K']==2;assert config['input_identity']['model_index']['sha256']==spec['intended_basis'];assert h(p.Path(config['input_identity']['model_index']['path']).read_bytes())==spec['intended_basis'];qp=src/'banana_smasher/qtip_runner.py';manifest=root/'QTIP_RUN_MANIFEST.json';save(manifest,{'schema':'bounded-training-correction-run-manifest-v1','canonical_commit':report['canonical_commit'],'cells':[cell],'clean_fit_admission':True,'tiers':[{'name':config['tier'],'bindings':{'qtip_runner':{'path':str(qp),'sha256':h(qp.read_bytes())}}}],'training_ledger_sha256':report['training_ledger_sha256'],'source_canary_sha256':h(old.read_bytes())})
  seed_material=h(json.dumps(cap,sort_keys=True).encode());config.update(layer=layer,expert=expert,projection=projection,fit_windows=spec['windows'],fit_capture_root=str(cr),hessian_layer_manifest=str(hm),hessian_layer_manifest_sha256=h(hm.read_bytes()),qtip_runner=str(qp),layer_census={'qtip2':1},pack_counts={'qtip2':1},materialization={'run_manifest':str(manifest),'run_manifest_sha256':h(manifest.read_bytes()),'source_config_sha256':h(hm.read_bytes())},rht_seed_material=seed_material,rht_seed=sp._canonical_rht_seed(seed_material,layer,expert,projection),clean_fit_admission=True,training_ledger_sha256=report['training_ledger_sha256'])
  runner.QTIP=p.Path(config['qtip_root']);mods=runner.load_official_qtip();closure=capture_source_closure(runner,dict(zip(('bitshift','ldlq','math_utils','kernel_decompress'),mods)));save(root/'SOURCE_CLOSURE.json',closure);config['glm_source_closure_sha256']=closure['sha256'];cp=root/f'L{layer:03d}_E{expert:03d}_{projection}.json';save(cp,config);assert json.loads((root/'SHARDS.json').read_text())['intended_basis']==spec['intended_basis']
