...
root=out/'q2_cell';root.mkdir(exist_ok=True);h=lambda b:hashlib.sha256(b).hexdigest()
def save(q,d):sp._atomic_json(q,d)
report={'schema':'single-cell-training-correction-v1','status':'RUNNING','started':time.time(),'cell':'L003/E000_fused13','tier':'K2','canonical_commit':spec['canonical_commit'],'training_ledger_sha256':spec['calibration_ledger_sha256'],'clean_fit_admission':True,'label':'external-document clean-calibrated K2 one-cell native-rest; not uniform PRE'}
try:
 ...
 report.update(config_sha256=h(cp.read_bytes()),source_closure_sha256=closure['sha256'],rht_seed=config['rht_seed'],tlut_source_sha256=h(p.Path(config['tlut_source']).read_bytes()),reference_sha256=h(p.Path(config['reference_unit']).read_bytes()));save(out/'CORRECTION_RESULT.json',report)
 result=sp.main(cp,root,3,profile_mode=False,kernel_cache_root=root/'kernel-cache');receipt=root/'solve/L003/E000_fused13/QTIP_SOLVE_RECEIPT.json';artifact=root/'solve/L003/E000_fused13/QTIP_UNIT.pt';r=json.loads(receipt.read_text());assert r['status']=='PASS' and h(artifact.read_bytes())==r['artifact_sha256'];report.update(status='PASS_SINGLE_CELL_CLEAN_CALIBRATION',artifact=str(artifact),artifact_sha256=h(artifact.read_bytes()),receipt=str(receipt),receipt_sha256=h(receipt.read_bytes()),ended=time.time())
except BaseException as e:report.update(status='FAIL',error=repr(e),traceback=traceback.format_exc(),ended=time.time());save(out/'CORRECTION_RESULT.json',report);raise
finally:save(out/'CORRECTION_RESULT.json',report)
