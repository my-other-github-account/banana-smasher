...
| Quant | Top-1 ↑ | KLD ↓ | MMLU ↑ | MMLU/BPW ↑ | MMLU/GB ↑ | Exact accounting GB | Shipped auxiliary scope | Comparison BPW | FP basis |
|---|---:|---:|---:|---:|---:|---:|---|---:|---|
| **Official native MXFP4** |  |  | **84.60%** (423/500) | **13.576** | **0.382** | 156.035 | MTP/drafter excluded; base-only reference | 4.390 | Native MXFP4; BALANCED64 not measured |
| **Unsloth IQ4** | **92.44%** (60,584/65,536) | **0.068349** | **83.40%** (417/500) | **15.188** | **0.427** | 136.662 | MTP excluded | 3.845 | FP8 e4m3 dynamic own-base |
| **EXL3 K3 routed-only + native rest** | **92.23%** (60,447/65,536) | **0.076868** | **85.20%** (426/500) | **17.255** | **0.485** | 123.999 | MTP included | 3.489 | FP8 e4m3 dynamic own-base |
| **QTIP3 uniform exact** | **91.68%** (60,084/65,536) | **0.110227** | **84.20%** (421/500) | **16.973** | **0.478** | **123.969** | MTP included | **3.488** | FP8 e4m3 dynamic own-base |
| **QTIP2.5 deterministic mixed ring** | **89.09%** (58,389/65,536) | **0.181971** | **82.80%** (414/500) | **19.261** | **0.542** | **106.657** | MTP included | **3.001** | FP8 e4m3 dynamic own-base |
...
| **QTIP2 corrected all-43** | **87.11%** (57,090/65,536) | **0.240852** | **82.40%** (412/500) | **22.838** | **0.643** | **89.330** | MTP included | **2.513** | FP8 e4m3 dynamic own-base |
...
| **Unsloth IQ2** | **84.57%** (55,422/65,536) | **0.276747** | **81.80%** (409/500) | **22.218** | **0.625** | 90.861 | MTP excluded | 2.556 | FP8 e4m3 dynamic own-base |
| **DwarfStar Q2** | **83.69%** (54,845/65,536) | **0.309521** | **80.60%** (403/500) | **21.092** | **0.593** | 93.691 | Stock MTP excluded; separate drafter included | 2.636 | FP8 e4m3 dynamic own-base |
...
| **EXL3 K2 uniform exact** | **81.78%** (53,593/65,536) | **0.366820** | **73.80%** (369/500) | **22.276** | **0.627** | **77.862** | MTP included | **2.191** | FP8 e4m3 dynamic own-base |

`MMLU/BPW` is `(MMLU percentage - 25) / comparison BPW`. `MMLU/GB` is `(MMLU percentage - 25) / complete decimal artifact GB`. Both are chance-adjusted and use exact machine-readable denominators rather than the rounded display values. Blank cells mean no sealed MMLU result is available; no score or density is projected. The full EXL3 K2.5 MMLU artifact was retired before a score terminal could be sealed, so its completed bounded-search disposition remains explicitly `ARTIFACT_UNAVAILABLE`. The Official native row is the base-only MMLU/accounting reference: 156,035,165,824 complete bytes with native MTP and any drafter excluded. No BALANCED64 Top-1/KLD terminal is sealed for that artifact, so those cells remain blank rather than being inferred from its reference role.
