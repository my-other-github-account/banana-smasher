...
The separate [MMLU-500 capability-density table](../notes/benchmarks/mmlu-density/mmlu500-v1/four-row-results.md) scores thirteen sealed rows on one immutable zero-shot question bank, including the distinct full physical alternating K2/K3 comparator. The original four per-question result sets remain unchanged. Every later row carries compact public-safe measurement provenance; the completed full EXL3 K2.5 greedy-optimizer search is separately recorded as `ARTIFACT_UNAVAILABLE`, with no inferred score.
