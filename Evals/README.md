...
| **Exact102 full64-pop Native MXFP4/D4/QTIP2 mixed Backpack** | **90.58%** (59,362/65,536) | **0.11035927227931097** | **N/A — not measured** | N/A | N/A | N/A | **102.000** | Historical partial native MTP | **2.869858586464101** | FP8 e4m3 dynamic own-base; checkpoint `f9bffe04…`; sealed PRE-repair exact assignment |
...
| **Native MXFP4/QTIP3/QTIP2 V7 mixed Backpack (3-tier)** | **88.12%** (57,753/65,536) | **0.172953** | **81.80%** (409/500) | **19.792** | **28.503** | **0.557** | **102.000** | Historical partial native MTP | **2.870** | FP8 e4m3 dynamic own-base; checkpoint `f9bffe04…`; sealed PRE-repair 3-tier mix |
| **QTIP3/QTIP2 V7 mixed Backpack (2-tier)** | **87.98%** (57,661/65,536) | **0.175773** | **N/A — not measured** | N/A | N/A | N/A | **102.000** | Historical partial native MTP | **2.870** | FP8 e4m3 dynamic own-base; checkpoint `f9bffe04…`; sealed PRE-repair 2-tier mix |
...
| **QTIP2 V7 published PRE f9bffe04** | **86.26%** (56,533/65,536) | **0.229392** | **82.40%** (412/500) | **22.819** | **32.757** | **0.642** | **89.405** | Published all-43 same-lineage PRE | **2.515** | FP8 e4m3 dynamic own-base; checkpoint `f9bffe04…` |
| **QTIP2 V7 U45 repaired** | **86.22%** (56,508/65,536) | **0.211278** | **82.60%** (413/500) | **20.343** | **29.172** | **0.572** | **100.636** | Historical partial native MTP | **2.831** | FP8 e4m3 dynamic own-base; checkpoint `ffb796ce…` |
| **QTIP2 V7 E2E improve journey U45 — `WORKED_EXAMPLE.md` output** | **86.83%** (56,907/65,536) | **0.196797** | **82.20%** (411/500) | **22.739** | **32.678** | **0.640** | **89.405** | Published all-43 same-lineage artifact; in-place repair | **2.515** | FP8 e4m3 dynamic own-base; produced by the documented `banana_smasher.improve` journey, PRE 0.22934→POST 0.19680, 45 updates; checkpoint `0c495959…`; checkpoint identity `94bed762…`; source model index `98efab45…` |
...
The **E2E improve journey U45** row is the direct output of `WORKED_EXAMPLE.md`: a stranger can run the documented `banana_smasher.improve` command chain from the published PRE checkpoint and reproduce the PRE 0.22934→POST 0.19680 trajectory in 45 updates. Its serving bytes preserve the exact 89,405,007,628-byte in-place artifact size; any size delta denotes a different artifact. The row is bound to checkpoint SHA-256 `0c49595962978f39668e8d37ea9e070d4a7adf556aedf93b90fc152c20dfc951`, checkpoint identity `94bed762ac74e663da300f9dc21ce3a6eb974b6f4e9fb32e150b865f5bc1970a`, and source model index SHA-256 `98efab455cf08dfbbbaaba6f570e1bf10bf927d2b4c3c453a59c2f6f0e3be92b`. Its MMLU cell is a complete 500/500 physical run on the immutable bank: 411 correct, terminal SHA-256 `7cee1c8791a611d66295af4ef3820ea317c942c42f027a75f7aebe895d42b94c`, qrows SHA-256 `f0dde8de9479a2c823c34a2c4489fa44b4bbb119d114a2d1ac76870ad9464edf`, and harness SHA-256 `aee58438723ccb33b5d82720df879f4e5674b3761cd9a80cbbd675c61107908b`.

The published PRE MMLU cell is bound to its [public-safe evidence and accounting readback](../archive/notes/benchmarks/mmlu-density/mmlu500-v1/evidence/PRE-f9bffe04/): 500 contiguous ordered rows, checkpoint `f9bffe04…`, source model index `98efab45…`, exact 89,405,007,628-byte all-43 accounting, zero physical resident-byte deltas against the same-lineage U45 reference, and an independently reproduced aggregate of 412/500 (82.4%) with 0.776261 gold CE bits. Its qrows SHA-256 is `4d8339096683a1049f614a49db34acd6f42fbbdbdbae4cf95c2a814bd46d3cf5`. The separate historical 100,636,011,256-byte partial-native-MTP artifact is explicitly excluded from this measurement and its numerator is not reused.

The two mixed **Backpack** rows are the same sealed 102,000,000,000-byte shipping envelope measured PRE-repair (`repair_applied=false`, checkpoint `f9bffe04…`, source model index `98efab45…`) over the identical 22,016-cell routed geometry of 43 layers × 256 experts × `{down, fused13}`, so they are directly comparable to each other. The 3-tier mix resolves 14 native MXFP4, 14,748 QTIP3 and 7,254 QTIP2 cells (93,585,408 + 69,903,847,228 + 22,969,931,238 = 92,967,363,874 selected expert bytes); the 2-tier mix resolves 14,773 QTIP3 and 7,243 QTIP2 cells (70,052,262,381 + 22,915,134,483 = 92,967,396,864). Each adds the same 9,032,112,614 fixed non-expert bytes (9,017,356,608 dense non-routed plus 14,756,006 metadata) and a metadata-reserve pad — 523,512 bytes for the 3-tier row and 490,522 for the 2-tier row — to reach exactly 102,000,000,000 bytes with zero shipping slack and an exact base-equivalent BPW of `816000000000/284334567511` = `2.8698585864641011…`. Both ship the historical partial native MTP payload (`mtp.0` only: 39 tensors, 171,235,692 bytes), the same scope label as the 89.4 GB rows. Both were scored against the same teacher manifest `d34f357f…` under one scoring-harness pin `e9f90a88…` with support-renormalized teacher top-8,192 KLD, so the 3-tier-versus-2-tier delta of `-0.0028205449343658984` mean KLD and +92 Top-1 matches is a matched comparison; their Top-1 receipts declare full-vocabulary argmax. The 3-tier MMLU cell is bound to a complete 500/500 zero-shot terminal on the same immutable question bank as the other MMLU cells — 409 correct, independently re-aggregated from its per-question rows (qrows SHA-256 `6b73a733f7623ae1a1c90768109e8c66c6a703a937520cb999971c03e9867741`, terminal `3fd7b956…`, sealed audit bundle `341d7a51…`). The 2-tier artifact has no sealed MMLU terminal, so its MMLU cell and three MMLU-derived densities are published as not measured rather than inferred from the 3-tier run or any other row.

Both mixed Backpack rows carry immutable identities. Shared across the pair: source model index `98efab45…`, PRE checkpoint `f9bffe04…`, teacher manifest `d34f357f…`, window bank `adef2300…`, frozen suite lock `d5610f11…`, and scoring-harness pin `e9f90a88…`. The 3-tier row additionally binds pack `9ede4ff8…`, virtual manifest `bbed6a44…`, materialization index `952e788f…`, assembly manifest `0cd54e97…`, assignment map `baecc11e…`, runtime adapter `06ef56ef…`, its BALANCED64 acceptance terminal `bd122f16…` over score receipt `ec71be71…` and Top-1 receipt `5dfbbcd8…`, and its MMLU-500 terminal `3fd7b956…`. The 2-tier row binds virtual manifest `e16c439b…`, materialization index `9ccc8ac0…`, runtime assignment `3be33cc6…`, pack `0a3a20ac…`, and its acceptance receipt `840d8856…` over score receipt `525b6009…` and Top-1 receipt `549730ac…`. Both rows' KLD and Top-1 were re-derived here from the 64 per-window score rows and matched bit-for-bit against the declared aggregates.

The **Exact102 full64-pop** row is a distinct sealed PRE-repair artifact, not a rescore of either earlier mixed Backpack row. Its exact 102,000,000,000-byte envelope is `2.869858586464101` whole-model BPW and resolves 2,939 native-MXFP4, 7,919 D4-K4096, and 11,158 QTIP2 routed cells. The authenticated spark-4 receipt `/home/dnola/missions/EXACT102_FULL64POP_t_a5560962_s4/run7696/FINAL_CANONICAL_BALANCED64_RECEIPT.V14.json` (SHA-256 `c56f7cceaa582295faaf2c6ed18ad2a410b5ef22c0535f4ea6821a15b896af31`) seals `64/64` windows, mean KLD `0.11035927227931097`, Top-1 `59362/65536` (`0.905792236328125`), and `193591695987` payload bytes read. This passes the sealed full64 target **KLD <= 0.14**; its W28 KLD is `0.0638451874256134`, and independent artifact/native K4 parity is exact-equal at MSE `0.00017127105093095452`. Identity bindings are virtual artifact manifest `6f33b5877a3ddd9b989fd48044779f24ed8f8aa6b7f90e0185c363c3ce8d616e`, materialization index `b9056bd8210e4a66b82b3e14b5e20f1582ec941dc979b533cc931c810b8ecb08`, PRE checkpoint `/home/dnola/missions/TRUTHFUL102_t_edbee0e2_run7536/source/checkpoint/PRE.pt` at `f9bffe04c6e1ee03ea2eefe838f68ed773179e05363d08ac509602cb740f9f70`, exact-solver assignment `5820a791aabed730d29386a6a84cb56c2c8dc86b003bf4b3e3a9ea7179f7167c` (materialized assignment `c366a5c2522e83862b91c8f67649851b30385b31451fc93d67c380bead0d9e15`), and final pack `a54cceef4ce56cfa036057dbf001d0ad3dca0243791a0399d4ffa9e6613a5652` (pack manifest `ca64b21b661aac06ecb7ddd3ad0632d3ac97d673fb005acce6b24f1763312868`). MMLU is explicitly **not measured**; no value or density is inferred.
