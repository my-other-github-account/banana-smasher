...
ROOT=Path('/dev/shm/t_012f1c90');OUT=ROOT/'down_rebase_run8570';CONTROL=Path('/dev/shm/t_1269dc5f/traceback_run8542r3');CLAIM=Path('/home/dnola/HOST_CLAIM.json');CAND='8c7ca73870fdc38866bcef30956b4342cecbe4c8'
...
if name=='quality':
 ...
 for expert,projection in [(242,'down'),(243,'down'),(242,'fused13')]:
  ...
  group='GROUP_DOWN' if projection=='down' else 'GROUP_FUSED';control=ROOT/f'alphabet_warp8_run8564/B1/warm/{group}/build/solve/L004/{cell}/QTIP_UNIT.pt';ref=qv.decode_packed_weight(torch.load(control,map_location='cpu',weights_only=True),kd,torch.device('cuda')).half().cpu()
  for label in ['C1','C2']:
...
for phase in ['setup','warm']:
 for group,cells in [('GROUP_DOWN',[(242,'down'),(243,'down')]),('GROUP_FUSED',[(242,'fused13')])]:
  ...
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();cfg=json.loads((CONTROL/f'B1/warm/{cell}/CONFIG.json').read_text());assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True;cfg['block_ldl_reference']=True;cfg['viterbi_distance_alphabet']=True;cfg['packed_conformance_on_device']=True;cfg['viterbi_num_warps']=16;cfg['viterbi_conditioned_distance_sum']=(projection=='down')
