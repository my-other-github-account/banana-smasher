...
TASK='t_012f1c90'; ROOT=Path('/dev/shm')/TASK
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 receipt('AUTHORITY.json',dict(card=TASK,allocation=json.loads((ROOT/'ALLOCATION.json').read_text()),plan=spec,plan_sha256=sha(PLAN),script_sha256=sha(__file__)))
 with SHARDS.with_suffix('.lock').open('a+') as sl:
  ...
  admitted=admit(s,TASK,basis,spec['requested_cells'])
  receipt('ADMISSION.json',dict(requested=spec['requested_cells'],admitted=admitted,basis=basis))
  receipt('SHARDS_PREIMAGE.json',s)
  for r in s['rows']:r.update(owner=TASK,operation='down_only_conditioned_rebased_sum',receipt=str(OUT/'TERMINAL.json'))
...
try:
 ...
 rows=[]
 for ref in spec.get('retained_baseline',{}).values(): assert sha(ref['path'])==ref['sha256']
