#!/usr/bin/env python3
"""LP4 t_9081cc96: function-space repair trainer (pilot layers 3,13,23,33,41).
...
BASE = os.path.expanduser(os.environ.get("V7_LP4_ROOT", str(Path(__file__).resolve().parents[1])))
sys.path.insert(0, os.path.join(BASE, "src"))
sys.path.insert(0, os.path.join(BASE, "src_lp4"))
...
TEACH = os.path.join(BASE, "teacher_calib")
CORPUS = os.path.join(BASE, "static", "windows_ds4_calib.json")
SEL = os.path.join(BASE, "static", "CALIB_SELECTION.json")
MAN = os.path.join(BASE, "static", "LP4_MANIFEST_S7LOCAL.json")
T_TRAIN = 1024
...
class TrainableExperts(nn.Module):
    ...
    def _w_pilot(self, e, which):
        ...
        if t == "w3":
            import os as _os9
            if _os9.environ.get("FD_DENSE_W3", "0") == "1":
                return None  # dense fast path; w3 LUTs frozen this run
    ...
    def _evict_planes(self):
        ...
        if _os3.environ.get("FD_NOEVICT", "0") == "1":
            return
        if _os3.environ.get("FD_PILOT_RESIDENT", "0") == "1" and self.pilot:
            return  # keep only PILOT layers resident; non-pilot evict as usual
...
def cmd_fastdiag(a):
    """Fast-iteration convergence diagnostic (David 7/13): small fixed batch,
