...
def _parser() -> argparse.ArgumentParser:
    ...
    v7_joint = subparsers.add_parser(
        "qtip-v7-joint-repair",
        help="run persistent joint QTIP V7 LUT, RMSNorm, and output-gain training",
    )
    ...
    v7_joint.add_argument("--device", default="cuda")
    v7_joint.add_argument("--full-wire-hash", action="store_true")
    v7_joint.add_argument("--diagnostic-every", type=int, default=8)
    v7_joint.add_argument("--activation-checkpoint", action="store_true")
    v7_joint.add_argument("--frozen-cache-bytes", type=int, default=2 << 30)
    v7_joint.add_argument(
        "--fwht-backend", choices=("quack",), default="quack"
    )
    v7_joint.add_argument("--resume-checkpoint", type=Path)
    v7_joint.add_argument("--resume-checkpoint-sha256")
    v7_joint.add_argument(
        "--no-resume", dest="resume", action="store_false", default=True
    )

    v7_complete = subparsers.add_parser(
        "qtip-v7-complete",
        help="drive one hash-bound QTIP V7 command to its declared terminal receipt",
    )
    v7_complete.add_argument("--request", type=Path, required=True)
        ...
        if args.command == "export":
            ...
            result = {
                **run_joint_v7_repair(
                    ...
                    frozen_cache_bytes=args.frozen_cache_bytes,
                    fwht_backend=args.fwht_backend,
                    resume_checkpoint=args.resume_checkpoint,
                    resume_checkpoint_sha256=args.resume_checkpoint_sha256,
                ),
                "command": "qtip-v7-joint-repair",
            }
        elif args.command == "qtip-v7-complete":
            from .qtip_v7_completion import run_qtip_v7_completion

            result = {
                **run_qtip_v7_completion(args.request),
                "command": "qtip-v7-complete",
            }
