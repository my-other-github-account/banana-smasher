...
class Qtip2PhysicalLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        source_schema: str,
        fwht_backend: str = "bounded",
    ) -> None:
        super().__init__()
        if isinstance(tlut, torch.nn.Parameter):
            self.tlut = tlut
        elif tlut.requires_grad:
            # Preserve a differentiable view supplied by a persistent FP32
            # master. Wrapping this non-leaf tensor in Parameter would detach
            # the physical projection from the public joint optimizer surface.
            self.tlut = tlut
        else:
            self.tlut = torch.nn.Parameter(tlut.float().contiguous())
        self.frozen = frozen
        self.source_schema = source_schema
        if fwht_backend not in {"bounded", "quack"}:
            raise ValueError(f"unsupported QTIP2 FWHT backend {fwht_backend!r}")
        self.fwht_backend = fwht_backend
        self.checkpoint_depth = False
    ...
    def _weight(self) -> torch.Tensor:
        ...
        q = bounded_fwht(
            ...
            normalize=True,
            backend=self.fwht_backend,
        ).transpose(0, 1)
        q = q * self.frozen.sv[:, None]
        q = bounded_fwht(
            q.contiguous(), normalize=True, backend=self.fwht_backend
        ) * self.frozen.su
        return q
