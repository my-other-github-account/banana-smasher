...
__device__ __forceinline__ int decode_parent_index(const int16_t* tile_words, int logical_index) {
    ...
    for (int step = 0; step < 8; ++step) {
        const int code_position = (position + 249 + step) & 255;
        state = static_cast<uint16_t>((static_cast<uint32_t>(state) << 2) |
                                      branch_code(tile_words, code_position));
    }
...
__global__ void grouped_forward_kernel(
    ...
    const int output_column = tile_m * kTile + (thread & 15);
    float total = 0.0f;
    for (int tile_k = 0; tile_k < tiles_k; ++tile_k) {
        ...
        __syncthreads();
        if (row < end && row < rows) {
            const int input_base = row * (tiles_k * kTile) + tile_k * kTile;
            const int output_lane = thread & 15;
            #pragma unroll
            for (int k = 0; k < kTile; ++k) {
                total = fmaf(x[input_base + k], q[k * kTile + output_lane], total);
            }
        }
        __syncthreads();
    }
    if (row < end && row < rows) {
        output[row * (tiles_m * kTile) + output_column] = total;
    }
}
