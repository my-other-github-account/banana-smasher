...
V7_VENDOR = Path(os.environ.get("V7_VENDOR_ROOT", Path(__file__).resolve().parents[1] / "vendor")).resolve()
sys.path.insert(0, str(V7_VENDOR / "src_lp4"))
sys.path.insert(0, str(V7_VENDOR / "src"))
...
def fast_forward(student, hidden, ids, requires_grad):
    ...
    def run_layer(Li, h):
        return m.model.layers[Li](
            ...
            attention_mask=mask, input_ids=ids,
            past_key_values=(
                active_cache if not requires_grad else DynamicCache(config=config)
            ))
