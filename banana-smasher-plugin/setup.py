...
# PEP 517 asks setuptools for metadata before it builds a wheel.  Creating a
# CUDAExtension during ``egg_info``/``sdist`` makes even source packaging require
# CUDA_HOME.  Keep every CUDA source in the sdist, but instantiate the native
# extension only for the actual wheel/install build.
_METADATA_COMMANDS = {"dist_info", "egg_info", "sdist"}
_METADATA_ONLY = bool(_METADATA_COMMANDS.intersection(sys.argv))
EXT_MODULES = [] if _METADATA_ONLY else [
    CUDAExtension(
...
setup(
    ...
    cmdclass={} if _METADATA_ONLY else {"build_ext": BuildExtension},
    package_dir={"": "src"},
