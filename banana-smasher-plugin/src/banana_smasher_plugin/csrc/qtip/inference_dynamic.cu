...
__host__ static void decompress_matvec_dynamic_ptr(
    ...
    static_assert(S + V >= 3, "Codebook must have at least eight float16 elements");
    static_assert(S + 5 + V + 1 <= 16, "We can only use 64 KiB shared memory");
    static_assert(R == 2 || R == 3, "Dynamic P1145 path supports QTIP2/QTIP3");
