...
class BananaSmasherMoEMethod(FusedMoEMethodBase):
    ...
    def apply(
        ...
        fused = native_layer.forward(
            expanded, routed_ids, "fused13", route_weights=routed_weights
        )
        gate, up = fused.chunk(2, dim=-1)
        activated = F.silu(gate) * up
        down = native_layer.forward(
            activated, routed_ids, "down", route_weights=routed_weights
        )
        result = (
