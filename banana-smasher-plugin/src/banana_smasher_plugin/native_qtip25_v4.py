...
from banana_smasher.qtip25_native_v4 import decode_native_v4_torch
...
def dequantize_native_v4_blocks(packed: Any, tlut: Any) -> Any:
    """Decode physical 80-byte L16/B10/V4 blocks on the input Torch device."""
    import torch
    ...
    if packed.ndim < 1 or packed.shape[-1] != 80:
        raise ValueError("native QTIP2.5 V4 codes must end in 80 bytes per 16x16 block")
    ...
    flat = packed.reshape(-1, 80)
    scales = torch.ones(flat.shape[0], device=packed.device, dtype=tlut.dtype)
    decoded = decode_native_v4_torch(flat, scales, positions=256, tlut=tlut)
    _DECODE_CALLS += 1
