...
from banana_smasher.qtip25_native_v4 import NativeQtip25Geometry, decode_native_v4_torch
...
def dequantize_native_v4_blocks(packed: Any, tlut: Any, *, bpw: float = 2.5) -> Any:
    """Decode physical L16/V4 blocks at the requested transition-bit rate."""
    import torch
    ...
    transition_bits = round(float(bpw) * 4)
    if abs(float(bpw) * 4 - transition_bits) > 1e-9:
        raise ValueError("native QTIP V4 bpw must map exactly to an integer transition width")
    geometry = NativeQtip25Geometry(B=transition_bits)
    packed_bytes = 8 * transition_bits
    if packed.ndim < 1 or packed.shape[-1] != packed_bytes:
        raise ValueError(
            f"native QTIP V4 codes must end in {packed_bytes} bytes per 16x16 block"
        )
    ...
    flat = packed.reshape(-1, packed_bytes)
