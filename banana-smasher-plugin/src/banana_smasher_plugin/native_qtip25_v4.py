...
def dequantize_native_v4_blocks(packed: Any, tlut: Any, *, bpw: object = 2.5) -> Any:
    """Decode physical homogeneous L16/B/V4 blocks on the input Torch device."""
    import torch

    global _DECODE_CALLS, _DECODE_BLOCKS, _DECODE_CODE_BYTES, _DECODE_CUDA_CALLS
    geometry = native_v4_geometry(bpw)
    code_bytes = 64 * geometry.B // 8
    ...
    if packed.ndim < 1 or packed.shape[-1] != code_bytes:
        raise ValueError(
            f"native QTIP V4 codes must end in {code_bytes} bytes per 16x16 block"
        )
    ...
    flat = packed.reshape(-1, code_bytes)
