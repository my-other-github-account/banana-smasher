...
def test_legacy_flashinfer_strips_fp8_ds_mla_scale_tail(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def target(
        query,
    ...
    wrapped = api.utils.flashinfer_trtllm_batch_decode_sparse_mla_dsv4
    query = torch.zeros((2, 64, 512), dtype=torch.bfloat16)
    swa_packed = torch.zeros((1, 2, 1, 584), dtype=torch.uint8)
    compressed_packed = torch.zeros((1, 2, 1, 584), dtype=torch.uint8)
    wrapped(
        query=query,
        swa_kv_cache=swa_packed,
        workspace_buffer=torch.zeros(8, dtype=torch.uint8),
        sparse_indices=torch.zeros((2, 1, 128), dtype=torch.int32),
        compressed_kv_cache=compressed_packed,
        out=torch.empty_like(query),
        swa_topk_lens=torch.ones(2, dtype=torch.int32),
    )
    _, kwargs = api.calls[0]
    ...
    assert kwargs["swa_kv_cache"].dtype == torch.float8_e4m3fn
    assert kwargs["compressed_kv_cache"].dtype == torch.float8_e4m3fn
    assert kwargs["swa_kv_cache"].shape[-1] == 512
    assert kwargs["compressed_kv_cache"].shape[-1] == 512
    assert kwargs["swa_kv_cache"].data_ptr() == swa_packed.data_ptr()
    assert kwargs["compressed_kv_cache"].data_ptr() == compressed_packed.data_ptr()
