...
def _write_arm_checkpoint(
    *,
    ...
    for ordinal, (state, bank_member) in enumerate(
        ...
        atomic_npz(path, {"hidden": state}, root=root)
        members.append(
            {
                "ordinal": ordinal,
...
def _verify_arm_checkpoint(
    *,
    ...
    manifest_path = _confined_root_file(
        root,
        f"checkpoints/layer_{layer:03d}/{arm}/manifest.json",
    ...
    if (
        ...
        or manifest.get("schema") != CHECKPOINT_MANIFEST_SCHEMA
        ...
        or manifest.get("layer_descriptor") != expected_descriptor
    ):
        raise EvaluationError("CHECKPOINT_MANIFEST_MISMATCH")
    _verify_layer_descriptor(manifest["layer_descriptor"], layer=layer)
    rows = manifest.get("members")
    if (
        not isinstance(rows, list)
        or manifest.get("member_count") != len(bank_members)
        or len(rows) != len(bank_members)
    ):
...
def _verify_pair_checkpoint(
    *,
    ...
    marker_path = _confined_root_file(
        root,
        f"checkpoints/layer_{layer:03d}/PAIR_COMPLETE",
        label="PAIR_CHECKPOINT_MARKER",
    )
    ...
    if (
        ...
        != {
            ...
            "evaluation_spec_sha256",
            "completed_layer",
            "input_pair_checkpoint_sha256",
            "candidate_manifest_sha256",
            "reference_manifest_sha256",
        }
        or marker.get("schema") != PAIR_MARKER_SCHEMA
    ...
    candidate = _verify_arm_checkpoint(
        ...
        expected_descriptor=candidate_descriptor,
    )
    reference = _verify_arm_checkpoint(
        ...
        spec_sha256=spec_sha256,
        expected_input_sha256=expected_input_sha256,
        expected_descriptor=reference_descriptor,
    )
    ...
    for layer in range(layer_count):
        try:
            checkpoint = _verify_pair_checkpoint(
                ...
                candidate_descriptor=candidate.layer_descriptor(layer),
                reference_descriptor=reference.layer_descriptor(layer),
            )
...
def _write_arm_artifacts(
    *,
    ...
    for ordinal, (logits, bank_member) in enumerate(
        ...
        atomic_npz(path, artifact_arrays, root=root)
        rows.append(
            {
                "ordinal": ordinal,
                "window_id": bank_member["window_id"],
    ...
    path = atomic_json(arm_root / "manifest.json", manifest, root=root)
...
def verify_evaluation(
    ...
    try:
        ...
        raise EvaluationError(f"EVALUATION_SCHEMA_INVALID: {exc}") from exc
    if (
        set(marker)
        != {
            "schema",
    ...
    with output_lock(root):
        try:
            ...
            if resume_from_layer is None:
                ...
                for layer in range(resume_from_layer):
                    checkpoint = _verify_pair_checkpoint(
                        ...
                        spec_sha256=spec_sha256,
                        expected_input_sha256=previous_sha256,
                        candidate_descriptor=candidate_runner.layer_descriptor(layer),
                        reference_descriptor=reference_runner.layer_descriptor(layer),
                    )
                    previous_sha256 = checkpoint["marker_sha256"]
            ...
            for layer in range(start_layer, layer_count):
                ...
                pair_path = atomic_json(
                    _pair_marker_path(root, layer), pair_marker, root=root
                )
            ...
            receipt_path = atomic_json(root / "evaluation.json", receipt, root=root)
            atomic_json(
                marker_path,
                {
            ...
            safe_unlink_output(
                root / "EVALUATION_COMPLETE",
                root=root,
                label="EVALUATION_MARKER_UNLINK",
            )
            atomic_json(
