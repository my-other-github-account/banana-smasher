...
TIERS = ("native_mxfp4", "qtip2", "qtip3", "d4_k2048", "d4_k4096")
...
def load_full_wire_menu(
    ...
    *,
    ...
    expected_tiers: Sequence[str] = TIERS,
...
def project_group_logits(
    ...
    *,
    ...
    option_keys = [(group, tier) for group in group_names for tier in TIERS]
    ...
    for group in group_names:
        ...
        if len(values) != len(TIERS):
            raise ValueError(f"group {group} does not have five tier logits")
    ...
    for group in group_names:
        add_constraint(
            {option_index[(group, tier)]: 1.0 for tier in TIERS}, 1.0, 1.0
        )
    budget = {
    ...
    for group in group_names:
        selected = [tier for tier in TIERS if solved.x[option_index[(group, tier)]] >= 0.5]
        if len(selected) != 1:
...
def measured_objective(
    ...
    *,
    ...
    return (
        mean_kld
        + float(worst_class_weight) * max(values)
        + float(top1_penalty_weight) * max(0.0, float(top1_floor) - top1)
    )
...
def run_measured_spsa(
    ...
    *,
    ...
    if state_path.exists():
        ...
        if state.get("schema") != "banana-smasher-measured-spsa-state-v1":
            raise ValueError("SPSA resume state schema mismatch")
    else:
        group_by_cell = build_hierarchical_groups(routing)
        state = {
            "schema": "banana-smasher-measured-spsa-state-v1",
            "status": "RUNNING",
            "next_iteration": 0,
            "refined": False,
            "group_by_cell": group_by_cell,
            "logits": {
                group: [0.0] * len(TIERS) for group in sorted(set(group_by_cell.values()))
            },
            "curve": [],
            "best": None,
            "config": {
                ...
                "top1_penalty_weight": top1_penalty_weight,
                "seed": seed,
            },
        }
    ...
    for iteration in range(int(state["next_iteration"]), total_iterations):
        ...
        delta = generator.choice((-1.0, 1.0), size=(len(group_names), len(TIERS)))
