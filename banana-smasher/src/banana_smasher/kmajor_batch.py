...
def reset_batched_kmajor_vjp(
    *, batch_size: int | None = None, allow_reference: bool = False
) -> None:
    global _BATCH_SIZE, _ALLOW_REFERENCE, _DRAIN_CHECK_QUEUED
    ...
    with _LOCK:
        return {
            **copy.deepcopy(_STATS),
            ...
            "reference_opt_in": _ALLOW_REFERENCE,
        }
...
def _queue_drain_check() -> None:
    global _DRAIN_CHECK_QUEUED
    with _LOCK:
        if _DRAIN_CHECK_QUEUED:
            return
        _DRAIN_CHECK_QUEUED = True

    def require_drained() -> None:
        global _DRAIN_CHECK_QUEUED
        with _LOCK:
            _DRAIN_CHECK_QUEUED = False
            tails = {key: len(group["pending"]) for key, group in _GROUPS.items()}
            _GROUPS.clear()
        if tails:
            raise RuntimeError(f"undrained batched K-major VJP tail: {tails}")

    torch.autograd.Variable._execution_engine.queue_callback(require_drained)
...
class BatchedKMajorVQLinearFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        if not grad_out.is_cuda and not _ALLOW_REFERENCE:
            raise RuntimeError(
                "CPU batched K-major VJP is a debug path requiring explicit reference opt-in"
            )
