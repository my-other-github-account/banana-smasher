...
def _validate_packed_identity(
    codes: torch.Tensor,
    scales: torch.Tensor,
    num_codes: int,
) -> None:
    if not codes.is_contiguous() or not scales.is_contiguous():
        raise ValueError("packed codes and scales must be contiguous")
    ...
    if codes.numel():
        ...
        if minimum < 0 or maximum >= num_codes:
            raise ValueError(
                f"packed code index is outside [0, {num_codes}): "
                f"min={minimum}, max={maximum}"
            )


def _require_bfloat16_inputs(
    grad_output: torch.Tensor, activation: torch.Tensor
) -> None:
    if grad_output.dtype != torch.bfloat16 or activation.dtype != torch.bfloat16:
        raise ValueError(
            "fused input VJP requires bfloat16 grad_output and activation"
        )
...
def _validate_common(
    ...
    _validate_packed_identity(codes, scales, num_codes)
    if triton is None:
...
def fused_codebook_vjp(
    ...
    *,
    ...
    _validate_packed_identity(codes, scales, int(num_codes))
...
def fused_codebook_vjp_from_inputs(
    ...
    *,
    ...
    _require_bfloat16_inputs(grad_output, activation)
    _validate_packed_identity(codes, scales, int(num_codes))
    if (
