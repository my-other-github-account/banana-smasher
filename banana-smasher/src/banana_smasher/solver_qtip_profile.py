...
_TRUSTED_PUBLIC_QTIP_RUNNER_SHA256 = (
    "e7b68e1f8ecdb9c917fd66d2b5f18cfd8c3a3ed3bf03576f33cfdcb286cb0e9c"
)
...
def _install_configured_viterbi(
    ...
    *,
    ...
    fused_schedule = False
    if "viterbi_fused_schedule" in config:
        from .qtip_viterbi import resolve_fused_schedule
        fused_schedule = resolve_fused_schedule(
            sealed, config.get("projection"), config["viterbi_fused_schedule"],
    ...
    from .qtip_viterbi import resolve_traceback_l2
    traceback_l2 = resolve_traceback_l2(
        sealed, config.get("projection"), config.get("viterbi_traceback_l2"),
        stage4_schedule,
    )
    cb._banana_smasher_traceback_l2 = traceback_l2
    ...
    if backend in PERSISTENT_BACKENDS:
        ...
        if traceback_l2:
            identity.update(viterbi_traceback_l2=True, production_default=False)
        if fused_schedule:
