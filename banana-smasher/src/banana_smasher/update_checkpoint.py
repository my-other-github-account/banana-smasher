...
def _resolve_payload_path(root: Path, value: object) -> Path:
    """Resolve a checksummed payload without binding a checkpoint to one root."""
    if not isinstance(value, str) or not value:
        raise RuntimeError("checkpoint payload path is missing")
    recorded = Path(value)
    if recorded.is_absolute():
        # Older manifests recorded an absolute path. Rebind only the basename
        # into the selected checkpoint directory; hash and byte gates below
        # still authenticate the payload before deserialization.
        recorded = Path(recorded.name)
    if len(recorded.parts) != 1 or recorded.name in {".", ".."}:
        raise RuntimeError("checkpoint payload path escapes its checkpoint directory")
    return root / recorded.name
...
def commit_segment_checkpoint(
    ...
    *,
    ...
    if manifest_path.is_file():
        previous_payload = _resolve_payload_path(
            root, json.loads(manifest_path.read_text())["payload_path"]
        )
    payload_path = root / (
...
def load_checkpoint(
    ...
    *,
    ...
    payload_path = _resolve_payload_path(root, manifest.get("payload_path"))
