...
class RealAxisRunner:
    ...
    def __init__(self, root: str | Path, *, require_pack: bool = False) -> None:
        ...
        if require_pack:
            from .contract import PackValidationError, verify_pack

            try:
                verify_pack(self.root)
            except PackValidationError as exc:
                raise RealAxisError(
                    f"PACK_CANONICAL_VERIFICATION_FAILED: {exc}"
                ) from exc
