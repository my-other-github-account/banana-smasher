...
def materialize_contextual_change(
    ...
    *,
    ...
    if not isinstance(members, list) or not members:
        raise ContextualValuationError("target selection-group members are missing")
    member_by_cell: dict[str, Mapping[str, Any]] = {}
    for member in members:
        if not isinstance(member, Mapping):
            raise ContextualValuationError("target selection-group member is invalid")
        member_cell = member.get("cell")
        source_key = member.get("physical_source_key")
        member_bytes = member.get("payload_bytes")
        if (
            not isinstance(member_cell, str)
            or not member_cell
            or member_cell in member_by_cell
        ):
            raise ContextualValuationError("target member cell is invalid")
        if not isinstance(source_key, str) or source_key not in baseline.get(
            "source_bindings", {}
        ):
            raise ContextualValuationError("target member physical source is not bound")
        if not isinstance(member_bytes, int) or member_bytes < 0:
            raise ContextualValuationError("target member payload bytes are invalid")
        if member.get("physical_identity") != _prepared_member_identity(
            member, basis_sha256=str(basis_sha256)
        ):
            raise ContextualValuationError("target member physical identity mismatch")
        receipt_sha256 = member.get("physical_receipt_sha256")
        receipt_path_value = member.get("physical_receipt_path")
        if receipt_sha256 is not None:
            if not isinstance(receipt_sha256, str) or len(receipt_sha256) != 64:
                raise ContextualValuationError("target physical receipt SHA is invalid")
            if receipt_path_value is not None:
                receipt_path = Path(str(receipt_path_value)).expanduser()
                if receipt_path.is_symlink() or not receipt_path.is_file():
                    raise ContextualValuationError("target physical receipt is missing or unsafe")
                if _sha256(receipt_path) != receipt_sha256:
                    raise ContextualValuationError("target physical receipt identity mismatch")
        member_by_cell[member_cell] = member
    if sum(int(member["payload_bytes"]) for member in members) != payload_bytes:
        raise ContextualValuationError("target group payload bytes do not match its members")
    if option.get("physical_identity") != _group_physical_identity(
        cell, tier, members, basis_sha256=str(basis_sha256)
    ):
        raise ContextualValuationError("target group physical identity mismatch")
    ...
    for member_cell in member_by_cell:
        if member_cell in assignment and isinstance(assignment[member_cell], str):
            assignment[member_cell] = tier
            continue
        layer, expert, projection = _parse_cell(member_cell)
        try:
    ...
    for line in index_path.read_text().splitlines():
        ...
        member = member_by_cell.get(str(row.get("cell_id")))
        if member is not None:
            replacement = {
                "tier": tier,
                "source_key": member["physical_source_key"],
                "physical_bytes": member["payload_bytes"],
                "activation_artifact_ids": sorted(
                    str(activation["id"]) for activation in member["activations"]
                ),
            }
            if member.get("physical_receipt_sha256") is not None:
                replacement["physical_receipt_sha256"] = member[
                    "physical_receipt_sha256"
                ]
            row.update(replacement)
            changed_rows += 1
        rows.append(row)
    if changed_rows != len(member_by_cell):
        raise ContextualValuationError(
            "change group must resolve every canonical materialization member"
        )
