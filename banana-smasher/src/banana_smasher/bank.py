...
def verify_bank(bank: str | Path, *, require_complete: bool = True) -> dict[str, Any]:
    ...
    try:
        validate_portable_schema(
            manifest, "bs-teacher-bank-v1.schema.json", label="BANK_MANIFEST"
        )
    except DurabilityError as exc:
        raise BankError(f"BANK_MANIFEST_SCHEMA_MISMATCH: {exc}") from exc
    if (
        set(manifest)
        != {
            "schema",
...
def build_bank(
    *,
    ...
    with output_lock(root):
        ensure_output_subdirectory(root, "members", label="BANK_MEMBERS")
        try:
            ...
            if marker.exists():
                ...
                receipt_path = atomic_json(
                    root / "BANK_RECEIPT.json", receipt, root=root
                )
                return {
                    **receipt,
                    "receipt": {
            ...
            for window in population.windows:
                ...
                if existing is not None:
                    ...
                    safe_unlink_output(
                        member_path, root=root, label="BANK_MEMBER_UNLINK"
                    )
                    safe_unlink_output(
                        sidecar_path, root=root, label="BANK_SIDECAR_UNLINK"
                    )
                    ...
                    atomic_npz(member_path, arrays, root=root)
                    ...
                    atomic_json(sidecar_path, row, root=root)
                    _verify_member(
                        root,
                        row,
                        expected_build_spec_sha256=spec_sha256,
            ...
            manifest_path = atomic_json(root / "bank.json", manifest, root=root)
            marker_value = {
            ...
            atomic_json(marker, marker_value, root=root)
            seal = verify_bank(root, require_complete=True)
            elapsed = time.time() - started
            receipt = {
                "schema": BANK_RECEIPT_SCHEMA,
            ...
            receipt_path = atomic_json(root / "BANK_RECEIPT.json", receipt, root=root)
            atomic_json(
                progress_path,
                {
                    "schema": "bs-bank-progress-v1",
            ...
            safe_unlink_output(
                root / "BANK_COMPLETE", root=root, label="BANK_MARKER_UNLINK"
            )
            atomic_json(
