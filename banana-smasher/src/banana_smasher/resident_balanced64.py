...
class RepairArtifact:
    ...
    def open(cls, root: str | Path) -> "RepairArtifact":
        ...
        for path_field in ("teacher_dir", "candidate_dir_template"):
            _relative_path(root_path, score.get(path_field), f"score.{path_field}")
    ...
    def score(
        ...
        *,
        ...
        for window in selected:
            ...
            idx = _array(_required(teacher, "idx", teacher_path)).astype("int64")
            ref_lp = _array(_required(teacher, "logprob", teacher_path)).astype("float64")
            q_lp = _array(_required(candidate, "q_lp_at_ref", candidate_path)).astype("float64")
            q_argmax = _array(_required(candidate, "q_argmax", candidate_path)).astype("int64")
            if (
                idx.shape != (POSITIONS_PER_WINDOW, SUPPORT)
                or ref_lp.shape != (POSITIONS_PER_WINDOW, SUPPORT)
                or q_lp.shape != (POSITIONS_PER_WINDOW, SUPPORT)
                or q_argmax.shape != (POSITIONS_PER_WINDOW,)
            ):
                raise ArtifactError(
                    f"window {window} must have exact 1024x8192 support and 1024 argmax rows"
                )
            count = min(POSITIONS_PER_WINDOW, idx.shape[0], q_lp.shape[0], q_argmax.shape[0])
            if count != POSITIONS_PER_WINDOW:
                raise ArtifactError(f"window {window} has {count} positions; expected {POSITIONS_PER_WINDOW}")
            ref_lp = ref_lp[:count]
            q_lp = q_lp[:count]
            idx = idx[:count]
