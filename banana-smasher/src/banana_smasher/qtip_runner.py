...
def require_claim() -> tuple[str, dict[str, Any]]:
    ...
    if state.get("owner") != TASK or state.get("task_id") != TASK:
        raise RuntimeError(
            f"QTIP validation host claim not owned by {TASK}: "
...
def require_host() -> None:
    expected_host = os.environ.get("BANANA_SMASHER_QTIP_VALIDATION_HOST")
    if not expected_host:
        raise RuntimeError("BANANA_SMASHER_QTIP_VALIDATION_HOST is required")
    if socket.gethostname() != expected_host or not torch.cuda.is_available():
        raise RuntimeError(
            f"wrong host/cuda: expected={expected_host} "
            f"actual={socket.gethostname()} cuda={torch.cuda.is_available()}"
        )
...
def validate_manifest_packed_layout(
    cb, packed: torch.Tensor, m: int, k: int
) -> tuple[int, int]:
    """Derive and validate the canonical layout bound by the run manifest."""
    binding = getattr(cb, "_banana_smasher_public_runner_pack_contract", None)
    if not isinstance(binding, Mapping):
        raise RuntimeError("public QTIP runner lacks manifest packed-layout binding")
    fields = (
        "schema",
        "geometry",
        "matrix_shape",
    ...
    geometry = tuple(int(getattr(cb, key)) for key in ("L", "K", "V"))
    matrix_shape = (m, k)
    input_tile = binding.get("input_tile")
    words_per_k = binding.get("packed_words_per_tile_per_k")
    if (
        tuple(binding) != fields
        or binding.get("schema") != "banana-smasher-public-runner-pack-contract-v1"
        or binding.get("geometry") != geometry
        or binding.get("matrix_shape") != matrix_shape
...
def pack_kernel_layout(
    ...
    manifest_layout = getattr(
        cb, "_banana_smasher_public_runner_pack_contract", None
    )
    codebook_k = int(cb.K)
    if manifest_layout is not None:
        validate_manifest_packed_layout(cb, packed, m, k)
    else:
        expected_shape = ((m // 16) * (k // 16), 16 * codebook_k)
        if tuple(packed.shape) != expected_shape or packed.dtype != torch.uint16:
            raise RuntimeError(
                f"canonical packed shape/dtype mismatch {m}x{k}: "
                f"{tuple(packed.shape)} {packed.dtype} != {expected_shape} uint16"
...
def decode_packed(
    ...
    codebook_v = int(geometry["V"])
    tlut_bits = int(geometry["tlut_bits"])
    ...
    lut_index = (quadratic >> (codebook_l - tlut_bits - 1)) & ((1 << tlut_bits) - 1)
...
def build_qtip(
    ...
    args = types.SimpleNamespace(td_x=16, td_y=16, V=codebook_v)
    started = time.time()
