"""Exact-source resident adapter (two default; four/eight opt-in), unchanged science."""
import ast
import hashlib
...
def render(source, *, cell_limit=2, relocated_panel=False):
    if type(cell_limit) is not int or cell_limit not in (2, 4, 8):
        raise ValueError('only default two or explicitly authorized four/eight cells')
    ...
    if cell_limit in (4, 8):
        source = source.replace("len(prepared['rows'])==2", f"len(prepared['rows'])=={cell_limit}", 1)
        source = source.replace("len(set(s['cells']))==2 and all(cell.endswith('_down') for cell in s['cells'])", f"s.get('resident_cell_limit')=={cell_limit} and len(set(s['cells']))=={cell_limit} and all(cell.rsplit('_',1)[-1] in ('down','fused13') for cell in s['cells'])", 1)
        source = source.replace("len({Path(row['config']).name for row in prepared['rows']})==2", "len({Path(row['config']).name for row in prepared['rows']})==" + str(cell_limit), 1)
...
def main():
    ...
    parser.add_argument('--cell-limit', type=int, choices=(2, 4, 8), default=2,
                        help='four/eight require an explicit task-local integration grant; not production adoption')
