...
def _qsfp_host(
    value: object, fabric: ipaddress.IPv4Network | ipaddress.IPv6Network
) -> str:
...
def _validate_item(
    item: object,
    output_root: Path,
    fabric: ipaddress.IPv4Network | ipaddress.IPv6Network,
) -> dict[str, Any]:
    if not isinstance(item, dict):
        raise ValueError("stage item must be an object")
    host = _qsfp_host(item.get("source_host"), fabric)
...
def _validate_disjoint_file_targets(targets: list[str]) -> None:
    if len(set(targets)) != len(targets):
        raise ValueError("batched stage manifest has duplicate destination files")
    files = {Path(target) for target in targets}
    for target in files:
        if any(parent in files for parent in target.parents if parent.parts):
            raise ValueError("batched stage manifest has overlapping destination files")
...
def stage_qsfp_manifest(
    ...
    *,
    ...
    fabric_cidr = manifest.get("fabric_cidr")
    if not isinstance(fabric_cidr, str) or not fabric_cidr:
        raise ValueError("stage manifest requires an explicit fabric_cidr")
    try:
        fabric = ipaddress.ip_network(fabric_cidr, strict=True)
    except ValueError as exc:
        raise ValueError("stage manifest fabric_cidr is invalid") from exc
    batched = manifest["schema"] == BATCH_SCHEMA
    validator = _validate_batch_item if batched else _validate_item
    items = [validator(item, output_root, fabric) for item in manifest["items"]]
    if batched:
        targets = [
            (Path(item["destination"]) / relative).as_posix()
            for item in items
            for relative in item["relative_paths"]
        ]
        _validate_disjoint_file_targets(targets)
