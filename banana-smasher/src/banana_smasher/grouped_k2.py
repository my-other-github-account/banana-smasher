...
def _default_normalized_hadamard_128(device: Any, dtype: Any) -> torch.Tensor:
...
def configure_k2_math(*, tensor_core_permutation: Any, normalized_hadamard_128: Any) -> None:
    """Install official model-family math helpers without private path injection."""
...
def _inverse_permutation(device: torch.device) -> torch.Tensor:
    tensor_core_permutation = (
        _default_tensor_core_permutation
        if _TENSOR_CORE_PERMUTATION is None
        else _TENSOR_CORE_PERMUTATION
    )
