...
def _host_aliases(host: str, aliases: Sequence[str] = ()) -> list[str]:
    ...
    for value in tuple(identities):
        ...
        try:
            ...
            identities.update(
                _normalize_host(item) for item in (canonical, *names, *addresses)
            )
...
def inspect_joint_inputs(
    *,
    ...
    if layers != list(range(43)):
        raise ValueError(
            f"QTIP V7 joint repair requires exact layers 0..42, got {layers}"
        )
    ...
    if not isinstance(trainable_surface, dict):
        raise ValueError(
            "QTIP V7 manifest requires exact joint_trainable_surface keys and shapes"
        )
    expected_keys = {
    ...
    if (
        ...
        or len(windows) != 64
        or len({json.dumps(row, sort_keys=True) for row in windows}) != 64
    ):
        raise ValueError(
            "QTIP V7 teacher bank requires exactly 64 unique ordered windows"
        )
...
def _checkpoint_surface(
    ...
    if not isinstance(freeze_sha256, str) or not re.fullmatch(
        r"[0-9a-f]{64}", freeze_sha256
    ):
    ...
    if any(not isinstance(state.get(name), dict) for name in names):
        raise ValueError(
            "joint checkpoint state requires layer_luts/norms/outputs maps"
        )
    counts = {
    ...
    if counts != expected:
        raise ValueError(
            f"joint checkpoint trainable surface drift: expected={expected} actual={counts}"
        )
    for group in names:
        ...
        for name, tensor in state[group].items():
            if not isinstance(name, str) or not isinstance(tensor, torch.Tensor):
                raise ValueError(
                    f"joint checkpoint {group} entries must be named tensors"
                )
            ...
            if not bool(torch.isfinite(tensor).all()):
                raise ValueError(
                    f"joint checkpoint contains nonfinite tensor {group}/{name}"
                )
...
def _authenticated_kld(bank: dict[str, Any], predictions: object) -> tuple[float, int]:
    ...
    for ordinal, (window, predicted) in enumerate(
        ...
        if (
            ...
            raise ValueError(
                f"invalid authenticated KLD logits at teacher window {ordinal}"
            )
...
def _validate_continuity(
    ...
    if any(name not in continuity for name in required):
        raise ValueError(
            "joint checkpoint requires optimizer/scheduler/RNG/trainer continuity"
        )
...
def verify_joint_checkpoint(
    *, freeze: str | Path, checkpoint: str | Path, receipt: str | Path | None = None
    ...
    if not math.isclose(
        ...
        raise RuntimeError(
            "joint checkpoint self-reported teacher_kld does not match logits"
        )
    ...
    if receipt is not None:
        ...
        if receipt_path.stat().st_mode & 0o222:
            raise RuntimeError(
                "joint checkpoint PASS receipt must be immutable/read-only"
            )
        if expected != result:
            raise RuntimeError(
                f"joint checkpoint PASS receipt mismatch: {receipt_path}"
            )
...
def _train_joint(
    *,
    ...
    command = (
        ...
        else [os.environ.get("PYTHON", sys.executable), str(trainer_path)]
    )
    ...
    if payload["continuity"]["trainer_identity"] != trainer_sha256:
        raise ValueError(
            "joint checkpoint trainer identity does not match supplied trainer"
        )
    ...
    if result["update"] != target_update:
        raise RuntimeError(
            f"trainer checkpoint update drift: {result['update']} != {target_update}"
        )
...
def _parse_worker(value: str) -> tuple[str, str | None, Path | None, Path]:
    ...
    if (
        ...
        raise ValueError("remote --worker requires a shell-safe absolute REMOTE_ROOT")
    expected, at, route = host.partition("@")
    if not at:
        raise ValueError(
            "remote --worker requires EXPECTED_HOSTNAME@HOST route identity"
        )
...
def _cancel_workers(
    processes: Sequence[
        tuple[subprocess.Popen[bytes], dict[str, Any], Path, str | None]
    ],
) -> None:
...
def launch_balanced64_shards(
    *,
    ...
    if bank_sha != frozen["teacher_bank"]["sha256"]:
        raise RuntimeError(
            "BALANCED64 teacher bank differs from the frozen run identity"
        )
    root = Path(output).expanduser().resolve()
    parsed = [_parse_worker(value) for value in workers]
    remote_hosts = [
        target for target, _, remote_root, _ in parsed if remote_root is not None
    ]
    for route, expected_host, remote_root, _ in parsed:
        if remote_root is not None and (
            ...
            or (
                ...
                and _matches_any_host(expected_host, trainer_identities)
            )
        ):
            raise ValueError(
    ...
    for (target, _, remote_root, worker), (start, end) in zip(
        parsed, ranges, strict=True
    ):
        try:
            ...
            if remote_root is None:
                ...
                observed_hashes = [
                    line.split()[0] for line in readback.stdout.splitlines()
                ]
                if observed_hashes != [candidate_sha, bank_sha, _sha256(staged_worker)]:
                    raise RuntimeError(
                        f"remote staged SHA-256 readback drift on {target}"
                    )
                remote_env = {
                    **environment,
    ...
    try:
        ...
        for _, row, local_receipt, remote_receipt in processes:
            if remote_receipt is not None:
                subprocess.run(
                    ["scp", f"{row['target']}:{remote_receipt}", str(local_receipt)],
                    check=True,
                )
            _, receipt = _load_json(local_receipt)
            expected = (row["ordinal_start"], row["ordinal_end"], candidate_sha)
            observed = (
                receipt.get("ordinal_start"),
                receipt.get("ordinal_end"),
            ...
            expected_ordinals = list(
                range(row["ordinal_start"], row["ordinal_end"] + 1)
            )
            if (
                ...
                raise RuntimeError(
                    f"BALANCED64 shard receipt drift: expected={expected} actual={observed}"
                )
            rows.append(
                {
                    **row,
                    "receipt": str(local_receipt),
                    "receipt_sha256": _sha256(local_receipt),
                }
            )
    except Exception:
        _cancel_workers(processes)
...
def aggregate_balanced64(*, shards: str | Path, output: str | Path) -> dict[str, Any]:
    ...
    for path in receipts:
        ...
        if not isinstance(teacher_sha, str) or not re.fullmatch(
            r"[0-9a-f]{64}", teacher_sha
        ):
    ...
    if set(by_ordinal) != set(range(64)):
        raise ValueError(
            f"BALANCED64 aggregate requires exact ordinals 0..63, got {sorted(by_ordinal)}"
        )
...
def compare_aggregates(
    *, baseline: str | Path, candidate: str | Path, output: str | Path
) -> dict[str, Any]:
    ...
    candidate_nonworse = float(right["mean_kld"]) <= float(left["mean_kld"]) and int(
        right["top1_matches"]
    ) >= int(left["top1_matches"])
    champion = "candidate" if candidate_nonworse else "baseline"
    selected_path, selected = (
        (candidate_path, right) if candidate_nonworse else (baseline_path, left)
    )
    result = {
...
def materialize_joint(
    *,
    ...
    if _sha256(manifest_path) != frozen["manifest"]["sha256"]:
        raise RuntimeError(
            "joint materialization manifest differs from frozen run identity"
        )
    ...
    try:
        ...
        for row, source_path in zip(
            document["members"], source.member_paths, strict=True
        ):
        ...
        for prefix, values in (
            ...
            for name, tensor in sorted(values.items()):
                dense[f"{prefix}/{name}"] = np.ascontiguousarray(
        ...
        if not result["packed_identity"]:
            raise RuntimeError(
                "joint materialization changed fixed QTIP member identity"
            )
        _write_json(
            output_path / "QTIP_V7_JOINT_MATERIALIZATION.json", result, exclusive=True
        )
        return result
    except Exception:
