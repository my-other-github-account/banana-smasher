...
def _score(
    value: dict[str, Any], label: str, *, assignment_map_sha256: str
) -> dict[str, Any]:
    ...
    if value.get("assignment_map_sha256") != assignment_map_sha256:
        raise ValueError(f"{label} score does not bind its solve-arm assignment")
...
def select_measured_nonworse(
    ...
    *,
    ...
    baseline_measurement = _score(
        baseline_score_raw_value,
        "baseline",
        assignment_map_sha256=baseline["assignment_map_sha256"],
    )
    expanded_measurement = _score(
        expanded_score_raw_value,
        "expanded",
        assignment_map_sha256=expanded["assignment_map_sha256"],
    )
