#!/usr/bin/env python3
"""Run a resumable shard of W28 full-model sensitivity probes (PROBE_MANIFEST_V5_COMPLETE20).

Differences from ``sensitivity_probe_worker.py`` (v1), all forced by the v2
manifest and by the run6989 post-mortem:

* binds the exact complete-source 20-probe V5 roster;
...
def main() -> int:
    ...
    if manifest.get("basis_sha256") != BASIS or manifest.get("probe_count") != PROBE_MANIFEST_V5_COMPLETE20_COUNT:
