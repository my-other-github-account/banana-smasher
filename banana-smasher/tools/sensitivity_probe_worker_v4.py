#!/usr/bin/env python3
"""Run a resumable shard of W28 full-model sensitivity probes (PROBE_MANIFEST_V4_COMPLETE20).

Differences from ``sensitivity_probe_worker_v2.py``, all forced by the v2
manifest and by the run6989 post-mortem:

* binds the immutable complete-source 20-probe V3 roster selected from physically present QTIP_UNIT.pt + producer receipts;
...
PROBE_MANIFEST_V4_COMPLETE20_SHA = "4bb7a4e40eee7113eab5598a09424684a21e81687cb98c618a90905f7228733c"
PROBE_MANIFEST_V4_COMPLETE20_COUNT = 20
...
def resolve_target_root_map(
    ...
    * a pool map was supplied -> bind the pool. Every declared producer path in
      PROBE_MANIFEST_V4_COMPLETE20 has been deleted fleet-wide, so the pool is where the
...
def main() -> int:
    ...
    if sha(args.probe_manifest) != PROBE_MANIFEST_V4_COMPLETE20_SHA:
        raise RuntimeError("PROBE_MANIFEST_V4_COMPLETE20_SHA_RED")
    if sha(args.ledger) != LEDGER_SHA:
        raise RuntimeError("LEDGER_SHA_RED")
    manifest = json.loads(args.probe_manifest.read_text())
    if manifest.get("basis_sha256") != BASIS or manifest.get("probe_count") != PROBE_MANIFEST_V4_COMPLETE20_COUNT:
    ...
    for probe in selected:
        ...
        receipt = {
            "schema": "banana-smasher-sensitivity-w28-probe-v4",
            "status": "PASS",
            "task_id": task_id,
            "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
            "canonical_git_pin": os.environ.get("BANANA_SMASHER_PIN"),
            "basis_sha256": BASIS,
            "probe_manifest_sha256": PROBE_MANIFEST_V4_COMPLETE20_SHA,
            "probe_id": probe_id,
            "role": probe.get("role"),
        ...
        atomic(root / "PROGRESS.json", {
            "schema": "banana-smasher-sensitivity-probe-progress-v4",
            "status": "RUNNING",
            "task_id": task_id,
            "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
            "shard": shard_label,
    ...
    atomic(root / "TERMINAL.json", {
        "schema": "banana-smasher-sensitivity-probe-shard-terminal-v4",
        "status": "PASS",
        "task_id": task_id,
        "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
        "basis_sha256": BASIS,
