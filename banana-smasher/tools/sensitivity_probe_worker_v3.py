#!/usr/bin/env python3
"""Run a resumable shard of W28 full-model sensitivity probes (PROBE_MANIFEST_V3_COMPLETE20).
...
PROBE_MANIFEST_V3_COMPLETE20_SHA = "17a973ee4becbad9c7e456ab95cb3fd5b4866712a069a04a8d0a13306e3a1445"
PROBE_MANIFEST_V3_COMPLETE20_COUNT = 20
...
def resolve_target_root_map(
    ...
    * a pool map was supplied -> bind the pool. Every declared producer path in
      PROBE_MANIFEST_V3_COMPLETE20 has been deleted fleet-wide, so the pool is where the
...
def main() -> int:
    ...
    if sha(args.probe_manifest) != PROBE_MANIFEST_V3_COMPLETE20_SHA:
        raise RuntimeError("PROBE_MANIFEST_V3_COMPLETE20_SHA_RED")
    if sha(args.ledger) != LEDGER_SHA:
        raise RuntimeError("LEDGER_SHA_RED")
    manifest = json.loads(args.probe_manifest.read_text())
    if manifest.get("basis_sha256") != BASIS or manifest.get("probe_count") != PROBE_MANIFEST_V3_COMPLETE20_COUNT:
    ...
    for probe in selected:
        ...
        receipt = {
            "schema": "banana-smasher-sensitivity-w28-probe-v3",
            "status": "PASS",
            "task_id": task_id,
            "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
            "canonical_git_pin": os.environ.get("BANANA_SMASHER_PIN"),
            "basis_sha256": BASIS,
            "probe_manifest_sha256": PROBE_MANIFEST_V3_COMPLETE20_SHA,
            "probe_id": probe_id,
            "role": probe.get("role"),
        ...
        atomic(root / "PROGRESS.json", {
            "schema": "banana-smasher-sensitivity-probe-progress-v3",
            "status": "RUNNING",
            "task_id": task_id,
            "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
            "shard": shard_label,
    ...
    atomic(root / "TERMINAL.json", {
        "schema": "banana-smasher-sensitivity-probe-shard-terminal-v3",
        "status": "PASS",
        "task_id": task_id,
        "board_run_id": int(os.environ.get("BANANA_SMASHER_RUN_ID", "0")),
        "basis_sha256": BASIS,
