...
from real_axis_fixtures import real_axis_fixture, write_runtime
...
def test_resealed_evaluation_rejects_core_contract_drift(
    ...
    with pytest.raises(
        EvaluationError, match="EVALUATION_(COMPLETION|SCHEMA)_INVALID"
    ):
        verify_evaluation(root)
...
def test_resealed_evaluation_rejects_aggregate_and_evaluation_id_drift(
    ...
    with pytest.raises(
        EvaluationError,
        match="EVALUATION_(METRICS|ID)_MISMATCH|EVALUATION_SCHEMA_INVALID",
    ):
        verify_evaluation(root)
...
def test_pack_manifest_seal_binds_real_axis_descriptor(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ValueError,
        match="PACK_(CANONICAL_VERIFICATION_FAILED|MANIFEST_REAL_AXIS_IDENTITY_MISMATCH)",
    ):
        _evaluate(paths)
...
def test_require_pack_rejects_minimal_noncanonical_manifest(tmp_path: Path) -> None:
    root = write_runtime(
        tmp_path / "minimal-pack",
        instance_id="minimal",
        layer_count=1,
        pack=False,
    )
    ...
    with pytest.raises(RealAxisError, match="PACK_CANONICAL_VERIFICATION_FAILED"):
        RealAxisRunner(root, require_pack=True)


@pytest.mark.parametrize("drift", ["member_path", "member_count", "descriptor"])
def test_resealed_checkpoint_cannot_redirect_or_misdescribe_state(
    tmp_path: Path, drift: str
) -> None:
    paths = real_axis_fixture(tmp_path, layer_count=2)
    _bank(paths)
    ...
    def stop(layer: int) -> None:
        if layer == 0:
            raise EvaluationError("STOP_AFTER_LAYER_ZERO")

    with pytest.raises(EvaluationError, match="STOP_AFTER_LAYER_ZERO"):
        _evaluate(paths, layer_hook=stop)

    layer_root = paths["evaluation"] / "checkpoints/layer_000"
    candidate_path = layer_root / "candidate/manifest.json"
    reference = json.loads((layer_root / "reference/manifest.json").read_text())
    candidate = json.loads(candidate_path.read_text())
    if drift == "member_path":
        candidate["members"][0] = reference["members"][0]
    elif drift == "member_count":
        candidate["member_count"] = len(candidate["members"]) + 1
    else:
        candidate["layer_descriptor"] = reference["layer_descriptor"]
    candidate_path.write_text(json.dumps(candidate, sort_keys=True) + "\n")
    pair_path = layer_root / "PAIR_COMPLETE"
    pair = json.loads(pair_path.read_text())
    pair["candidate_manifest_sha256"] = sha256_file(candidate_path)
    pair_path.write_text(json.dumps(pair, sort_keys=True) + "\n")

    result = _evaluate(paths)
    assert result["resume"]["started_from_layer"] == 0


@pytest.mark.parametrize("drift", ["resume", "artifact", "elapsed"])
def test_resealed_evaluation_rejects_schema_invalid_receipt_metadata(
    tmp_path: Path, drift: str
) -> None:
    paths = real_axis_fixture(tmp_path)
    _bank(paths)
    ...
    receipt = json.loads((paths["evaluation"] / "evaluation.json").read_text())
    if drift == "resume":
        receipt["resume"]["started_from_layer"] = "zero"
    elif drift == "artifact":
        del receipt["artifacts"]["root"]
    else:
        receipt["elapsed_seconds"] = float("nan")
    _reseal_evaluation(paths["evaluation"], receipt)

    with pytest.raises(EvaluationError, match="EVALUATION_SCHEMA_INVALID"):
        verify_evaluation(paths["evaluation"])


@pytest.mark.parametrize("descendant", ["checkpoints", "arms"])
def test_evaluation_rejects_symlinked_output_descendant_before_external_write(
    tmp_path: Path, descendant: str
) -> None:
    paths = real_axis_fixture(tmp_path)
    _bank(paths)
    outside = tmp_path / f"outside-{descendant}"
    outside.mkdir()
    paths["evaluation"].mkdir()
    (paths["evaluation"] / descendant).symlink_to(outside, target_is_directory=True)
