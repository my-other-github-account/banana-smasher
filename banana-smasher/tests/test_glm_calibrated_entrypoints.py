...
def test_calibrated_cell_uses_public_solver_and_exact_capture_roster(tmp_path, monkeypatch):
    def put(path, obj):
    ...
    windows=[]
    for i in range(2):
        path=tmp_path/f'x{i}.json';put(path,{'win':0})
        windows.append({'ordinal':0,'item_id':f'book:{i}','path':str(path),'sha256':sha(path)})
    captures=tmp_path/'captures.json';put(captures,{'status':'PASS_NATIVE_CAPTURE','ledger_sha256':sha(ledger),'windows':windows})
    ...
    spec=tmp_path/'spec.json';put(spec,{'output':str(output),'canonical_commit':'test-only','calibration_ledger':str(ledger),'calibration_ledger_sha256':sha(ledger),'capture_results':[str(captures)],'windows':2,'template_config':str(template),'intended_basis':sha(model)})
    calls=[]
    ...
    resumed=json.loads(spec.read_text());resumed.update(prepared_config=str(output/'q2_cell/L003_E000_fused13.json'),solve_root=str(output/'q2_cell_attempt2'),result_name='CORRECTION_A2_RESULT.json')
