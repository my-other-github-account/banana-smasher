...
class Tests(unittest.TestCase):
 def test_event_fence_order_and_restore(self):
  trace=[]
  class T:
  ...
  stream=object()
  class E:
   def record(self,s):self_stream=s;assert s is stream;trace.append('record')
   def synchronize(self):trace.append('event_sync')
  torch=SimpleNamespace(Tensor=T,empty_like=alloc,cuda=SimpleNamespace(current_stream=lambda d:stream,Event=E))
  original=T.cpu
  with self.assertRaisesRegex(RuntimeError,'sentinel'):
   with m.pinned_cpu_scope(torch,True,event_fence=True) as events:
    out=T().cpu();self.assertIsInstance(out,Out);self.assertEqual(trace,['copy','record','event_sync']);self.assertEqual(len(events),1)
