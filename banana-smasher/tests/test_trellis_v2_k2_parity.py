...
def test_k2_exact_contract_enumerates_all_canonical_branches() -> None:
    ...
    assert '"lut_vector_width_bytes": 16' in exact_source
    assert '"lut_load_instructions_per_pair_branch": 1' in exact_source
    ...
    assert "const float4 lut_pair =" in cuda_source
    assert "reinterpret_cast<const float4*>(lut_aos)" in cuda_source
