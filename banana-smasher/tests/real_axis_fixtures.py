...
def _write_qtip2_source(root: Path, layer_count: int) -> Path:
    root.mkdir(parents=True)
    (root / "config.json").write_text(
        json.dumps({"model_type": "deepseek_v4", "hidden_size": 16}) + "\n"
    )
    for layer in range(layer_count):
        layer_root = root / f"layers/layer_{layer:03d}"
        (layer_root / "experts").mkdir(parents=True)
        (layer_root / "qtip2").mkdir()
        np.save(layer_root / "experts/tier_map.npy", np.zeros(256, dtype=np.uint8))
        np.save(layer_root / "qtip2/codes.npy", np.arange(32, dtype=np.uint8))
        np.save(layer_root / "qtip2/scales.npy", np.arange(8, dtype=np.uint8))
        np.save(
            layer_root / "qtip2/codebooks.npy",
            np.arange(16, dtype=np.float16).reshape(4, 4),
        )
        np.save(
            layer_root / "qtip2/expert_ids.npy",
            np.arange(256, dtype=np.int16),
        )
        np.save(
            layer_root / "qtip2/tensor_offsets.npy",
...
def write_runtime(
    ...
    *,
    ...
    if pack:
        source = _write_qtip2_source(root.parent / f".{root.name}-source", layer_count)
        export_pack(
            source_root=source,
            output=root,
    ...
    if pack:
        ...
        bound_paths = {row["path"] for row in manifest["files"]}
        for path in sorted(root.rglob("*")):
            if not path.is_file() or path.name == "BANANA_PACK_MANIFEST.json":
                continue
