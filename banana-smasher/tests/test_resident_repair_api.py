...
def identity(root: Path, *, kind: str, tiers: list[dict], provenance: dict | None = None) -> str:
...
class Rails:
    ...
    def build_uniform(self, model: Path, tier: str, output: Path) -> Path:
        ...
        identity(output, kind="uniform-qtip-v7", tiers=[{"layer": 0, "tiers": {tier: 2, "native": 1}}])
        return output

    def mix(self, builds, bpw_target: float, output: Path) -> Path:
        self.calls.append(("mix", tuple(row.tier for row in builds), bpw_target, output))
        provenance = {
            "uniform_builds": [
...
def test_full_pipeline_builds_uniforms_then_mixes_without_resolving(tmp_path: Path) -> None:
    ...
    assert [row[0] for row in rails.calls] == [
        "build_uniform", "build_uniform", "mix", "score", "train", "score"
    ]
...
def test_tier_space_is_integer_v7_only_and_rejects_d4_or_fractional(tmp_path: Path) -> None:
...
def test_mix_fails_closed_if_declared_provenance_does_not_bind_builds(tmp_path: Path) -> None:
