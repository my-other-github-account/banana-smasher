...
def test_actual_rebase_keeps_unreachable_costs():
 ...
 guards=[n for n in ast.walk(kernel) if isinstance(n,ast.If) and ast.unparse(n.test)=='DISTANCE_ALPHABET' and any(isinstance(a,ast.Assign) and any(isinstance(t,ast.Name) and t.id=='minimum_cost' for t in a.targets) for a in n.body)]
 assert len(guards)==1
 class TL:
