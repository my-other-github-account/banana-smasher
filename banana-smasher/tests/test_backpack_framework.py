...
def _fixture_plan(tmp_path: Path, *, exact_bytes: int = 53344) -> dict[str, object]:
    ...
    for expert_ids in (range(128), range(128, 256)):
        for projection in ("fused13", "down"):
            ...
            expert_values = np.stack(
                [
                    value + np.float32(expert_id + 1) / np.float32(65536.0)
                    for expert_id in expert_ids
                ]
            ).astype(np.float32)
            np.save(
                model / f"cell{index}.experts.npy",
                expert_values,
                allow_pickle=False,
            )
            ...
            cells.append(
                {
                    ...
                    "expert_path": f"cell{index}.experts.npy",
                    "feature_slice": [index * 16, (index + 1) * 16],
                    "layer": 0,
    ...
    features = rng.normal(size=(64, 64)).astype(np.float32)
...
def test_inspect_requires_complete_expert_partition_per_projection(tmp_path: Path) -> None:
    ...
    expert_path = model_root / manifest["cells"][1]["expert_path"]
    expert_weights = np.load(expert_path, allow_pickle=False)
    np.save(expert_path, np.resize(expert_weights, (255, 16)), allow_pickle=False)
...
def test_materializer_rejects_nested_parent_symlink_without_touching_outside(
    tmp_path: Path,
) -> None:
    plan = _fixture_plan(tmp_path)
    run_root = tmp_path / "run"
    ...
    with pytest.raises(BackpackPlanError, match="symlink"):
        materialize_backpack_source(
    ...
    assert not (outside / "pre-repair-source").exists()
...
def test_vector_candidate_quantizes_each_expert_source_independently(tmp_path: Path) -> None:
    expert_weights = np.stack(
        [
            np.asarray([0] * 4 + [1] * 4 + [0] * 4 + [1] * 4, dtype=np.float32),
            np.asarray([100] * 8 + [102] * 8, dtype=np.float32),
        ]
    )
    receipt = generate_vector_vq_backpack_candidate(
        ...
        cell={
            ...
            "projection": "fused13",
            "expert_ids": [0, 1],
            "weights": expert_weights.mean(axis=0, dtype=np.float32),
            "expert_weights": expert_weights,
        },
    )
    root = Path(receipt["wire"]["path"]).parent  # type: ignore[index]
    offsets = np.load(root / "tensor_offsets.npy", allow_pickle=False)
    codes = (root / "wire.bin").read_bytes()
    codebooks = np.load(root / "codebooks.npy", allow_pickle=False)

    assert codes[offsets[0, 0] : offsets[1, 0]] != codes[
        offsets[1, 0] : offsets[2, 0]
    ]
    assert not np.array_equal(codebooks[0], codebooks[1])
    assert receipt["source_records"][0]["weight_sha256"] != receipt[
        "source_records"
    ][1]["weight_sha256"]
...
def test_custom_teacher_is_hash_bound_and_used(tmp_path: Path) -> None:
    plan = _fixture_plan(tmp_path)
    teacher = tmp_path / "teacher.npy"
    np.save(teacher, np.zeros(64, dtype=np.float32), allow_pickle=False)
    plan["anchor"]["teacher"] = str(teacher)  # type: ignore[index]
...
def test_missing_pre_repair_anchor_receipt_invalidates_stage_resume(tmp_path: Path) -> None:
    plan = _fixture_plan(tmp_path)
    run_root = tmp_path / "run"
    build_backpack(plan, run_root=run_root)
    (run_root / "anchors" / "pre-repair-backpack.json").unlink()
...
def test_missing_inspect_artifact_size_invalidates_stage_resume(tmp_path: Path) -> None:
    plan = _fixture_plan(tmp_path)
    run_root = tmp_path / "run"
    build_backpack(plan, run_root=run_root)
    receipt_path = run_root / "stages" / "01-inspect.json"
    receipt = json.loads(receipt_path.read_text())
    del receipt["result"]["model_manifest"]["bytes"]
...
def test_mutated_custom_teacher_invalidates_inspect_resume(tmp_path: Path) -> None:
    ...
    run_root = tmp_path / "run"
    build_backpack(plan, run_root=run_root)
    np.save(teacher, np.ones(64, dtype=np.float32), allow_pickle=False)
...
def test_final_score_uses_repair_bundle_materialized_codebook(
    ...
    model_root = Path(str(plan["model"]["root"]))  # type: ignore[index]
    original = np.load(model_root / "cell0.experts.npy", allow_pickle=False)[0]
    source_codebook = np.asarray(
        quantize_vector_cell(original, dimension=4, bits=2)["codebook"]
    )
    ...
    assert next(row for row in result["assignment"] if row["cell_id"] == "cell0")[
        "tier"
    ] == "d4-k4"
    manifest = load_manifest(Path(result["final_pack"]))
    ...
    assert not np.array_equal(materialized[1], np.zeros((4, 4), dtype=np.float16))
    ...
    teacher = np.concatenate(
        [np.load(model_root / f"cell{index}.npy", allow_pickle=False) for index in range(4)]
    )
    parsed = BackpackPlan.from_mapping(plan)
    _model_manifest, cells = backpack_module._load_cells(parsed)
    exported_weights = backpack_module._final_pack_weights(
        parsed,
        Path(result["final_pack"]),
        result["assignment"],
        cells,
        {"candidate_tiers": result["candidate_tiers"]},
    )
...
def test_packaged_qtip_imports_solved_units_into_executable_candidate(
    tmp_path: Path,
) -> None:
    ...
    (source_root / "QTIP_RUN_MANIFEST.json").parent.mkdir(parents=True)
    ...
    geometry_row = {key: value for key, value in zip(("L", "K", "V"), geometry)}
    geometry_map = {}
    reconstructed = []
    for expert in (0, 1):
        identity = (0, expert, "fused13")
        geometry_map[identity] = geometry
        config = source_root / "L000" / f"E{expert:03d}_fused13.json"
        ...
        unit = source_root / "solve" / "L000" / f"E{expert:03d}_fused13"
        unit.mkdir(parents=True)
        artifact = unit / "QTIP_UNIT.pt"
        trellis = torch.tensor([expert, expert + 1], dtype=torch.int64)
        decoded = torch.arange(16, dtype=torch.float16).reshape(4, 4) + expert * 32
        reconstructed.append(decoded.float().reshape(-1).numpy())
        torch.save(
            {
                ...
                "tlut": torch.arange(8, dtype=torch.float16).reshape(4, 2) + expert,
                "reconstructed_weight": decoded,
                "geometry": {**geometry_row, "tlut_bits": 9},
            },
            artifact,
        )
    ...
    cell = {
        ...
        "projection": "fused13",
        "expert_ids": [0, 1],
        "weights": np.mean(np.stack(reconstructed), axis=0, dtype=np.float32),
    }
    ...
    receipt = generate_qtip_backpack_candidate(
        ...
        tier=tier,
        cell=cell,
        geometry_by_identity=geometry_map,
    )
    ...
    assert backpack_module._validate_candidate_receipt(
        str(tmp_path / "run" / "candidates" / "qtip-2.0" / "cell0" / "RECEIPT.json"),
        tier=tier,
        cell=cell,
        geometry_by_identity=geometry_map,
    )
...
def test_lifecycle_exports_use_one_pack_abi_and_preserve_pre_post_geometry(
    ...
    for result in exports.values():
        ...
        assert result["expert_plane_bytes"] == result["expert_plane_target_bytes"]
        assert result["expert_plane_file_bytes"] == (
            result["expert_plane_bytes"]
            + result["expert_plane_container_overhead_bytes"]
        )
        assert result["whole_model_file_bytes"] == result["accounted_file_bytes"]
        assert result["accounted_file_bytes"] == sum(
            result[field]
            for field in (
