...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'fused_pair_run8545'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='aa455253fa0cd8944a4148112d44c537938d40c4'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1826696 and old['start_ticks']==8241004
 ...
 try:
  for tarname,dirname in [('fused_pair_pin_run8545.tar','fused_pair_code_run8545'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  jobs=[('B1','baseline'),('B2','baseline'),('C1','candidate'),('C2','candidate'),('quality',None)];rows=[]
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'fused_pair_code_run8545/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'fused_pair_code_run8545/banana-smasher/tests'
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(tests/'test_qtip_reference_ldl.py'),*[str(p) for p in tests.glob('test_qtip_batch*.py')],'-q']))
sys.path.insert(0,str(ROOT/'fused_pair_code_run8545/banana-smasher/src'))
import torch
...
if name=='quality':
 runner=Path(sp.__file__).with_name('qtip_runner.py');qv=sp._load_public_qtip_runner(runner,sha(runner));cfg=json.loads((CONTROL/'B1/warm/E242_fused13/CONFIG.json').read_text());qv.QTIP=Path(cfg['qtip_root']);bits,ldlq,math,kd=qv.load_official_qtip();start=time.perf_counter();rows=[]
 for expert in [242,243]:
  cell=f'E{expert}_fused13';cfg=json.loads((OUT/f'C1/warm/GROUP_FUSED/{cell}/CONFIG.json').read_text());captures=sp._load_captures(Path(cfg['fit_capture_root']),4,16);fit,_=sp._prepare_fit_windows(qv,captures,model_root=Path(cfg['model_root']),layer=4,expert=expert,projection='fused13',device=torch.device('cuda'));sp._release_capture_bank(Path(cfg['fit_capture_root']),4,16,captures);w,_=sp._load_weight(Path(cfg['model_root']),4,expert,'fused13')
  control=CONTROL/f'B1/warm/{cell}/build/solve/L004/{cell}/QTIP_UNIT.pt' if expert==242 else OUT/f'B1/warm/NEW_FUSED/build/solve/L004/{cell}/QTIP_UNIT.pt';ref=qv.decode_packed_weight(torch.load(control,map_location='cpu',weights_only=True),kd,torch.device('cuda')).half().cpu()
  for label in (['C1','C2'] if expert==242 else ['B1','B2','C1','C2']):
   for phase in ['setup','warm']:
    group='NEW_FUSED' if label.startswith('B') else 'GROUP_FUSED';path=OUT/f'{label}/{phase}/{group}/build/solve/L004/{cell}/QTIP_UNIT.pt';x=qv.decode_packed_weight(torch.load(path,map_location='cpu',weights_only=True),kd,torch.device('cuda')).half().cpu();metrics=qv.split_metrics(fit,w,ref,x,torch.device('cuda'));assert torch.isfinite(x).all();rows.append(dict(passed=metrics['qtip_hyb']['sse_ratio_vs_true_vq']<=1.0001+1e-12,cell=cell,arm=label,phase=phase,artifact_sha256=sha(path),reference_sha256=sha(control),decoded_equal=torch.equal(x,ref),max_abs=float((x.float()-ref.float()).abs().max()),metrics=metrics));put(OUT/'QUALITY.json',dict(rows=rows,validation_seconds=time.perf_counter()-start,scope='canonical decode and clean-fit only; not heldout',limit=1.0001))
 assert len(rows)==12
...
rows=[];pin=CAND
for phase in ['setup','warm']:
 for group,cells in ([('NEW_FUSED',[(243,'fused13')])] if arm=='baseline' else [('GROUP_FUSED',[(242,'fused13'),(243,'fused13')])]):
  D=OUT/name/phase/group;D.mkdir(parents=True);paths=[]
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();source_cfg=CONTROL/f'B1/warm/E{expert}_{"down" if expert==243 else "fused13"}/CONFIG.json';cfg=json.loads(source_cfg.read_text());cfg['projection']='fused13';cfg['rht_seed']=sp._canonical_rht_seed(cfg['rht_seed_material'],4,expert,'fused13');assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True;cfg['block_ldl_reference']=True
   manifest=json.loads(Path(cfg['materialization']['run_manifest']).read_text());manifest['canonical_commit']=pin;manifest['cells']=[f'L004/{cell}'];manifest['derived_source_config_sha256']=sha(source_cfg);manifest['tiers'][0]['bindings']['qtip_runner']=dict(path=str(runner),sha256=sha(runner));mp=cd/'MANIFEST.json';put(mp,manifest);cfg['materialization']['run_manifest']=str(mp);cfg['materialization']['run_manifest_sha256']=sha(mp)
  ...
  dest=D/'build';dest.mkdir();put(dest/'SHARDS.json',json.loads((ROOT/'SHARDS.json').read_text()));torch.cuda.synchronize();torch.cuda.reset_peak_memory_stats();t=time.perf_counter();result=sp.main(paths[0],dest,4,profile_mode=False) if arm=='baseline' else main_batch(paths,dest,4);torch.cuda.synchronize();rows.append(dict(phase=phase,group=group,cells=cells,seconds=time.perf_counter()-t,result=result,peak_allocated=torch.cuda.max_memory_allocated(),peak_reserved=torch.cuda.max_memory_reserved()));put(OUT/name/'RESULT.json',dict(pin=pin,rows=rows,scope='New E243fused baseline only plus retained E242fused control; distinct two-fused grouping. Original source/seed material retained with canonical projection seed. Historical mixed control timing, not cold JIT or production.'))
