...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'ds4_quality_run8528';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='6a40f916a6def348cb6a65bcd648006ab3242211';CAND='7391e41b2e7dad4cd0ebeda7dc6e95e05cae92e0'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  prior=Path(old['mission']);assert prior==ROOT/'ds4_resident_r2_run8528' and json.loads((prior/'TERMINAL.json').read_text())['returncode']==0;assert dead(json.loads((prior/'CHILD.json').read_text())['pid'])
 ...
 try:
  ...
  assert rc==0
  tests=['test_qtip_solve_resume.py','test_qtip_public_api.py','test_qtip_resident_ring_dispatch.py','test_qtip_batch.py']
  env=dict(os.environ,PYTHONPATH=str(OLD/'main_run8524/banana-smasher/src'))
  cmd=[sys.executable,'-m','pytest',*[str(OLD/'main_run8524/banana-smasher/tests'/t) for t in tests],'-q','--tb=short']
  ...
  put(OUT/'TESTS.json',dict(command=cmd,returncode=rc,pin=BASE));put(OUT/'TERMINAL.json',dict(returncode=rc));assert rc==0
...
R=ROOT/'ds4_resident_r2_run8528';B=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_run8474');MODEL=Path('/home/dnola/models/hf/DeepSeek-V4-Flash-0731')
