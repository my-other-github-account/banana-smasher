...
ROOT=Path('/dev/shm/t_6ac2b6f5');OUT=ROOT/'isolated_run8560';CONTROL=Path('/dev/shm/t_1269dc5f/traceback_run8542r3');CLAIM=Path('/home/dnola/HOST_CLAIM.json');CAND='c769d086701769b08fdc85d5191afc2aa321e3e7'
...
if arm=='baseline': sys.path.insert(0,str(ROOT/'baseline_code8560/banana-smasher/src'))
import torch
...
rows=[];pin=CAND if arm=='candidate' else '7e67a48ab10e955ae6e0cbc5506dbcc9c51c1c6c'
for phase in ['setup','warm']:
 for group,cells in [('GROUP_DOWN',[(242,'down'),(243,'down')]),('GROUP_FUSED',[(242,'fused13')])]:
  ...
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();cfg=json.loads((CONTROL/f'B1/warm/{cell}/CONFIG.json').read_text());assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True;cfg['block_ldl_reference']=True;cfg['viterbi_distance_alphabet']=True;cfg['packed_conformance_on_device']=True
