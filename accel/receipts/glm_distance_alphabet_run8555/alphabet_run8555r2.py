...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'alphabet_run8555r2'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='bc886ea64719f914bbe198f8bf54b7cff5ee2e1f'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1858242 and old['start_ticks']==8808854
 ...
 try:
  for tarname,dirname in [('alphabet_pin_run8555r2.tar','alphabet_code_run8555r2'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'alphabet_code_run8555r2/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'alphabet_code_run8555r2/banana-smasher/tests'
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(tests/'test_qtip_distance_alphabet.py'),*[str(p) for p in tests.glob('test_qtip_batch*.py')],'-q']))
sys.path.insert(0,str((ROOT/'alphabet_code_run8555r2' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
