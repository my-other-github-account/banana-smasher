...
rows=[sp.main(cp,OUT,3,profile_mode=False)]
torch.cuda.synchronize();put(OUT/'API_RESULT.json',dict(rows=rows,public_main_seconds=time.perf_counter()-t,config_sha256=sha(cp),scope='diagnostic single-cell public API, not production adoption'))

artifact=OUT/'solve/L003/E145_down/QTIP_UNIT.pt';unit=torch.load(artifact,map_location='cpu',weights_only=True)
x=qv.decode_packed_weight(unit,kd,torch.device('cuda')).half().cpu()
original=torch.load(BASE/'OWNER_ORIGINAL_run8503.pt',map_location='cpu',weights_only=True);ref=qv.decode_packed_weight(original,kd,torch.device('cuda')).half().cpu()
quality=dict(original_sha256=sha(BASE/'OWNER_ORIGINAL_run8503.pt'),artifact_sha256=sha(artifact),same_decoded=torch.equal(x,ref),max_abs=float((x.float()-ref.float()).abs().max()),metrics=qv.split_metrics(fit,w,ref,x,torch.device('cuda')))
assert quality['metrics']['qtip_hyb']['sse_ratio_vs_true_vq']<=1.0001+1e-12
put(OUT/'API_QUALITY.json',quality)
