...
TASK='t_ebcba52e';ROOT=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_verify_run8474');CLAIM=Path('/home/dnola/HOST_CLAIM.json');INPUT=Path('/dev/shm/t_ebcba52e_ds4_k3_input_run8465');STAGE=INPUT/'source';PIN='7927793657c7a48e1e02f5cb9e437e1c5705002d';API=Path('/dev/shm/t_ebcba52e_api_79277936/banana-smasher/src');BASIS='98efab455cf08dfbbbaaba6f570e1bf10bf927d2b4c3c453a59c2f6f0e3be92b';MODEL=Path('/home/dnola/models/hf/DeepSeek-V4-Flash-0731')
def sha(p):
 h=hashlib.sha256()
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 assert hashlib.sha256(raw).hexdigest()==sys.argv[1] and c['task_id']==TASK and c['pid']==1057548 and c['stage']=='t_ebcba52e_ds4_k3_fused_run8474_terminal_retained'
 assert not Path('/proc/1057548').exists() and (INPUT/'RESULT.json').exists()
...
try:
 ...
 B=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_run8474');U=B
 result=json.loads((B/'RESULT.json').read_text());limits={int(k):v for k,v in json.loads((B/'FROZEN_LIMITS.json').read_text())['limits'].items()}
 put(ROOT/'SCIENCE.json',dict(baseline_candidate_sha256=sha(B/'RESULT.json'),limits_sha256=sha(B/'FROZEN_LIMITS.json'),basis=BASIS,independent_decode=True,heldout_kld=None))
 def decode(root,rel,e):
  p=root/rel/('solve/L026/E%03d_fused13/QTIP_UNIT.pt'%e);unit=torch.load(p,map_location='cpu',mmap=True,weights_only=True);m,n=unit['shape'];assert {k:unit['geometry'][k] for k in ['K','L','V']}==dict(K=3,L=16,V=2)
 ...
 for e in [78,79,80]:
  gate();w,_=_load_weight(MODEL,26,e,'fused13');ref,_=decode(B,'baseline_warm2',e)
  for root,rel in [(B,a) for a in ['baseline_setup','baseline_warm1','baseline_warm2','candidate_setup','candidate_warm1','candidate_warm2']]:
  ...
  del w,ref
 speed={'warm':sum(r['wall_seconds'] for r in result['rows'] if r['arm'].startswith('baseline_warm'))/sum(r['wall_seconds'] for r in result['rows'] if r['arm'].startswith('candidate_warm'))}
 assert len(rows)==18
 put(ROOT/'RESULT.json',dict(state='SEALED_INDEPENDENT_NUMERICAL_COLD_GATE',identity=identity,rows=rows,count=len(rows),all_pass=all(r['pass_numerical'] for r in rows),speedups=speed,validation_wall=time.perf_counter()-started,heldout_kld=None,production_adoption=False,quality_scope='three fused cells independent numerical/decode; no currentK3 heldout frontier'))
