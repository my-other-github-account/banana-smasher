...
TASK='t_ebcba52e';ROOT=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_cold_verify_run8474');CLAIM=Path('/home/dnola/HOST_CLAIM.json');INPUT=Path('/dev/shm/t_ebcba52e_ds4_k3_input_run8465');STAGE=INPUT/'source';PIN='7927793657c7a48e1e02f5cb9e437e1c5705002d';API=Path('/dev/shm/t_ebcba52e_api_79277936/banana-smasher/src');BASIS='98efab455cf08dfbbbaaba6f570e1bf10bf927d2b4c3c453a59c2f6f0e3be92b';MODEL=Path('/home/dnola/models/hf/DeepSeek-V4-Flash-0731')
def sha(p):
 h=hashlib.sha256()
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 assert hashlib.sha256(raw).hexdigest()==sys.argv[1] and c['task_id']==TASK and c['pid']==1062029 and c['stage']=='t_ebcba52e_ds4_k3_fused_cold_run8474_terminal_retained'
 assert not Path('/proc/1062029').exists() and (INPUT/'RESULT.json').exists()
...
try:
 ...
 B=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_run8474');C=Path('/dev/shm/t_ebcba52e_ds4_k3_fused_cold_run8474');cold=json.loads((C/'RESULT.json').read_text());U=B
 ...
 put(ROOT/'SCIENCE.json',dict(baseline_candidate_sha256=sha(B/'RESULT.json'),cold_sha256=sha(C/'RESULT.json'),limits_sha256=sha(B/'FROZEN_LIMITS.json'),basis=BASIS,independent_decode=True,heldout_kld=None))
 ...
 for e in [78,79,80]:
  ...
  for root,rel in [(C,a+'/'+phase) for a in ['baseline1','candidate1','candidate2','baseline2'] for phase in ['cold','warm']]:
