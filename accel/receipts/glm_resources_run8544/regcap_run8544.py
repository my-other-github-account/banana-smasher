...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'regcap_run8544'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='8d7fb1c3d15785cafbbe9081107dbc930fdc1c8c'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1800218 and old['start_ticks']==8076525
 ...
 try:
  ...
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'regcap_code_run8544/banana-smasher/src'))
  ...
  with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
   fcntl.flock(lock,fcntl.LOCK_EX);c=json.loads(CLAIM.read_text());assert c['pid']==os.getpid();c.update(stage='register_cap64_terminal_retained',terminal_at=time.time());put(CLAIM,c)
 raise SystemExit()
name,arm=sys.argv[1:];assert json.loads(CLAIM.read_text())['pid']==os.getppid()
if name=='gpu_tests':
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(ROOT/'regcap_code_run8544/banana-smasher/tests/test_qtip_register_cap.py'),str(ROOT/'regcap_code_run8544/banana-smasher/tests/test_qtip_lut_cache_policy.py'),'-q']))
sys.path.insert(0,str((ROOT/'regcap_code_run8544' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
...
if name.startswith('resources_'):
 ...
 for overlap in [False,True]:
  ...
  if arm=='candidate':kw['maxnreg']=64
