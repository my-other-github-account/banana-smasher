...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'licm_run8544'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='a295e9b2d795cced8d943d93ff6072a5c00879f2'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1797117 and old['start_ticks']==8056458
 ...
 try:
  ...
  jobs=[('resources_baseline','baseline'),('resources_candidate','candidate'),('gpu_tests',None),('C1','candidate'),('C2','candidate'),('quality',None)];rows=[]
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'licm_code_run8544/banana-smasher/src'))
  ...
  with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
   fcntl.flock(lock,fcntl.LOCK_EX);c=json.loads(CLAIM.read_text());assert c['pid']==os.getpid();c.update(stage='licm_terminal_retained',terminal_at=time.time());put(CLAIM,c)
 raise SystemExit()
name,arm=sys.argv[1:];assert json.loads(CLAIM.read_text())['pid']==os.getppid()
if name=='gpu_tests':
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(ROOT/'licm_code_run8544/banana-smasher/tests/test_qtip_loop_licm.py'),str(ROOT/'licm_code_run8544/banana-smasher/tests/test_qtip_lut_cache_policy.py'),'-q']))
sys.path.insert(0,str((ROOT/'licm_code_run8544' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
...
if name.startswith('resources_'):
 ...
 for overlap in [False,True]:
  ...
  if arm=='candidate':kw['DISABLE_LICM']=True
  k=kernel.warmup(*args,grid=(256,),**kw);k._init_handles()
  rows.append(dict(kwargs=kw,n_regs=k.n_regs,n_spills=k.n_spills,metadata=k.metadata._asdict(),cubin_sha256=hashlib.sha256(k.asm['cubin']).hexdigest(),kernel_hash=k.hash))
