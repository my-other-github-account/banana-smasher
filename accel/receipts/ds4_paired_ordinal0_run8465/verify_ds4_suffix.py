...
TASK='t_ebcba52e';CLAIM=Path('/home/dnola/HOST_CLAIM.json');ROOT=Path('/dev/shm/t_ebcba52e_ds4_verify_run8461');RUN=Path('/dev/shm/t_ebcba52e_ds4_suffix_run8461_a2');PREFIX=Path('/dev/shm/t_ebcba52e_ds4_prefix_run8461_a2')
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 assert hashlib.sha256(raw).hexdigest()==sys.argv[1] and c['task_id']==TASK and c['stage']=='t_ebcba52e_ds4_suffix_run8461_a2_terminal_retained'
...
try:
 ...
 science=json.loads((RUN/'SCIENCE.json').read_text());assert science['ordinal']==0 and science['window']==28 and science['positions']==1024 and science['suffix_layers']==[13,42] and science['intended_basis']==BASIS
 ...
 for L in range(13,43):
  ...
  for arm in science['arms']:
   ...
   assert sha(rec['path'])==rec['sha256'] and rec['consumed_sha256']==sha(RUN/(arm+'_CONSUMED.json'))
   stage_shas[arm+'/'+str(L)]=rec['sha256']
  counts[str(L)]=len(rows)
 for arm in science['arms']:
  consumed=json.loads((RUN/(arm+'_CONSUMED.json')).read_text());expected={(r['expert'],r['projection']):r for r in bindings['rows'][arm]};assert len(consumed)==len(expected)==4
  for r in consumed:
   e=expected[(r['expert'],r['projection'])];assert r['artifact_sha256']==e['sha256'] and sha(e['path'])==e['sha256']
