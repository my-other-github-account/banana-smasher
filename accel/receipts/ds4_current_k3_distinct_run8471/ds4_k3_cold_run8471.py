...
TASK='t_ebcba52e';PIN='eec3bfc578c2fac4678373331f779de4e2abb766'
ROOT=Path('/dev/shm/t_ebcba52e_ds4_k3_cold_run8471');CLAIM=Path('/home/dnola/HOST_CLAIM.json')
PRIOR=Path('/dev/shm/t_ebcba52e_ds4_k3_distinct_run8471')
API=Path('/dev/shm/t_ebcba52e_api_eec3bfc5/banana-smasher/src')
...
if '--child' in sys.argv:
 ...
 for phase in ['cold','warm']:
  ...
  if arm.startswith('candidate'):result=main_batch(paths,dest,9)
  else:
   for cp in paths:result.extend(main_batch([cp],dest,9))
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 assert c['stage']=='t_ebcba52e_ds4_k3_unitwise_run8471_terminal_retained'
 assert not Path('/proc/'+str(c['pid'])).exists()
 assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
 put(ROOT/'CLAIM_PREIMAGE.json',c)
 put(ROOT/'SHARDS.json',dict(intended_basis=BASIS,rows=[dict(owner=TASK,layer=9,experts=[90,91,92],projections=['down'],K=3,operation='new_private_cache_cold_comparison',receipt=str(ROOT/'IDENTITY.json'))]))
