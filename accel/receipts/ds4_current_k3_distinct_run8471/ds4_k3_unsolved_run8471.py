...
TASK='t_ebcba52e';ROOT=Path('/dev/shm/t_ebcba52e_ds4_k3_unsolved_run8471');CLAIM=Path('/home/dnola/HOST_CLAIM.json');INPUT=Path('/dev/shm/t_ebcba52e_ds4_k3_input_run8465');STAGE=INPUT/'source';PIN='f39437eb860fd90e7a367b955b807907003ad97b';API=Path('/dev/shm/t_ebcba52e_api_f39437eb/banana-smasher/src');BASIS='98efab455cf08dfbbbaaba6f570e1bf10bf927d2b4c3c453a59c2f6f0e3be92b';MODEL=Path('/home/dnola/models/hf/DeepSeek-V4-Flash-0731')
def sha(p):
 h=hashlib.sha256()
...
with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
 ...
 assert hashlib.sha256(raw).hexdigest()==sys.argv[1] and c['task_id']==TASK and c['pid']==1047377 and c['stage']=='t_ebcba52e_ds4_k3_verify_run8471_a2_terminal_retained'
 assert not Path('/proc/1047377').exists() and (INPUT/'RESULT.json').exists()
...
try:
 ...
 for cfg in configs:
  cfg['block_ldl_unitwise']=True;cfg['ldlq_update_unsolved_only']=True
  runner=API/'banana_smasher/qtip_runner.py';assert sha(Path(cfg['qtip_runner']))==sha(runner)
  manifest=json.loads(Path(cfg['materialization']['run_manifest']).read_text());manifest['canonical_git_pin']=PIN;manifest['tiers'][0]['bindings']['qtip_runner']=dict(path=str(runner),bytes=runner.stat().st_size,sha256=sha(runner));mp=ROOT/('MANIFEST_E'+str(cfg['expert'])+'.json');put(mp,manifest);cfg['materialization']['run_manifest']=str(mp);cfg['materialization']['run_manifest_sha256']=sha(mp);cfg['qtip_runner']=str(runner);cfg['exact_solver']='banana_smasher.qtip_viterbi@'+PIN
 ...
 put(ROOT/'SCIENCE.json',dict(basis=BASIS,baseline_result_sha256=sha(PRIOR/'RESULT.json'),frozen_limits_sha256=sha(PRIOR/'FROZEN_LIMITS.json'),baseline_replayed=False,candidate_not_executed=True,delta_vs_singleton=dict(block_ldl_unitwise=True,ldlq_update_unsolved_only=True),delta_vs_incumbent=dict(ldlq_update_unsolved_only=True),incumbent_result_sha256=sha('/dev/shm/t_ebcba52e_ds4_k3_unitwise_run8471/RESULT.json'),timing='different-process shared-cache warm against sealed singleton controls; not matched cold',heldout_kld=None,production_adoption=False))
 ...
 put(ROOT/'RESULT.json',dict(state='SEALED_DISTINCT_CURRENT_K3_UNSOLVED_COMPOSITION',identity=identity,rows=rows,warm_speedup=b/c,reconstruction_pass=all(c['reconstruction_pass'] for r in rows if r['arm'].startswith('candidate') for c in r['cells']),limits=limits,heldout_kld=None,production_adoption=False,scope='three distinct current-K3 L009 down cells; shared-cache warm only; no fused/fullmodel/adoption'))
