...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'squares_run8557'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='bcea65ec8745799b55c802e69696a8acf507c4ea'
...
if len(sys.argv)==1:
 ...
 put(OUT/'AUTHORITY.json',dict(card_comment='1788920954 explicit dedicated bs01 s6 supersedes stale remote allocation',driver_text=Path('/home/dnola/missions/DRIVER_GOALS.md').read_text(),script_sha256=sha(__file__),hypothesis='Factor squared per-step differences over1024 signed quantlut values before gathering; changes possible FMA contraction so bounded numerical gate mandatory; exact full-LUT validation and version-bound preparation; additional4MiB admission; default off, retaining broadcast, full-state pointers, LUT policy, branch arithmetic, full forward and traceback. Frozen clean-fit SSE ratio<=1.0001; no tuning from heldout.'))
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1861745 and old['start_ticks']==8832748
 ...
 try:
  for tarname,dirname in [('squares_pin_run8557.tar','squares_code_run8557'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'squares_code_run8557/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'squares_code_run8557/banana-smasher/tests'
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(tests/'test_qtip_distance_alphabet.py'),str(tests/'test_qtip_alphabet_square_hoist.py'),*[str(p) for p in tests.glob('test_qtip_batch*.py')],'-q']))
sys.path.insert(0,str((ROOT/'squares_code_run8557' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
