...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'ds4_distinct_run8528';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='6a40f916a6def348cb6a65bcd648006ab3242211';CAND='7391e41b2e7dad4cd0ebeda7dc6e95e05cae92e0'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  prior=Path(old['mission']);assert prior==ROOT/'ds4_quality_run8528' and json.loads((prior/'TERMINAL.json').read_text())['returncode']==0;assert dead(json.loads((prior/'CHILD.json').read_text())['pid'])
...
for phase in phases:
 ...
 for expert in ([78] if phase=='0' else [79,80]):
