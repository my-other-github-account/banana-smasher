...
P=pathlib.Path
B=P('/home/dnola/missions/DS4_EMPTY_FIT_t_68e5892b'); R=B/'s8-K3-L011DOWN242-run8495'; CAN=B/'canonical-39da421e-run8495'; C=P('/home/dnola/HOST_CLAIM.json'); PLAN=B/'PLAN_L011_SUCCESSOR_run8495.json'
...
def worker():
 ...
 offset=int(sys.argv[2]);selected=plan['source_rows'][offset:offset+3];accepted=[json.loads(l) for l in (R/'ADMISSIONS.jsonl').read_text().splitlines()] if (R/'ADMISSIONS.jsonl').exists() else [];assert len(accepted)==offset;cps=[];configs=[]
 for row in selected:
 ...
 closure=load(B/'canonical-4921456b-run8384/IMPORT_CLOSURE.json');closure['canonical']=plan['canonical'];g=solver._read_qtip_config(cps[0]);path,digest=solver._declared_public_qtip_runner(g);runner=solver._load_public_qtip_runner(path,digest);runner.QTIP=solver._config_path(g,'qtip_root');runner.load_official_qtip();coord.verify_imports(mods(),closure);save(R/f'PRE_SOLVE_IMPORTS_{offset:04d}.json',mods())
 import torch
 f,t=torch.cuda.mem_get_info();save(R/f'CUDA_PREFLIGHT_{offset:04d}.json',dict(free_bytes=f,total_bytes=t,reserve_bytes=4<<30,conservative_batch_estimate_bytes=8<<30));assert f-(4<<30)>(8<<30),'BATCH_CAPACITY_REFUSAL'
 gate();begin=time.time();results=main_batch(cps,R,11,kernel_cache_root=R/'kernel-cache');end=time.time();coord.verify_imports(mods(),closure);save(R/f'POST_SOLVE_IMPORTS_{offset:04d}.json',mods());new_accepted=[]
 for row,cfg,cp,r in zip(selected,configs,cps,results,strict=True):
  ...
  new_accepted.append(dict(K=3,cell=row['cell'],artifact=ref(a),receipt=ref(rp),config=ref(cp),imports=ref(R/f'POST_SOLVE_IMPORTS_{offset:04d}.json'),source_input=row['source_input'],started=begin,accepted=time.time(),qualification=cfg['fit_qualification'],producer_pin=PIN,producer_identity=dict(pin=PIN,publication=plan['publication']),scope='bounded-L011down-missing-continuation'))
 with (R/'ADMISSIONS.jsonl').open('a') as f:
  for a in new_accepted:f.write(json.dumps(a)+'\n')
  f.flush();os.fsync(f.fileno())
 accepted.extend(new_accepted);progress=dict(completed_cells=len(accepted),target_cells=len(cells),K=3,started=load(R/'PREFLIGHT.json')['started'],observed=time.time(),main_batch_wall_seconds=end-begin,accepted_artifact_bytes=sum(a['artifact']['bytes'] for a in accepted),last_cell=selected[-1]['cell']);save(R/f'BATCH_PROGRESS_{offset:04d}.json',progress);save(R/'PROGRESS.json',progress)
def supervisor():
 ...
 prev=P(prior['mission']);assert load(prev/'SHARDS.json')==plan['prior_shards'];assert sha(prior['terminal_path'])==prior['terminal_sha256'];assert load(prior['terminal_path'])['status']=='PASS' and load(prior['terminal_path'])['progress']['completed_cells']==3;aw=load(prev/'ACTIVE_WORKER.json');assert coord.ticks(aw['pid']) is None
 ...
 assert len(cells)==len(set(map(tuple,cells)))==242
 ...
 try:
  gate();env=dict(os.environ,TORCHINDUCTOR_CACHE_DIR=str(R/'inductor-cache'),TRITON_CACHE_DIR=str(R/'triton-cache'),TORCH_EXTENSIONS_DIR=str(R/'torch-extensions'),HOME=user.pw_dir,USER='dnola',LOGNAME='dnola',PYTHONDONTWRITEBYTECODE='1',OMP_NUM_THREADS='1',OPENBLAS_NUM_THREADS='1',PATH='/home/dnola/humming_env/bin:'+os.environ['PATH'])
  with (R/'SOLVE.log').open('x') as log:
   for offset in range(0,len(cells),3):
    gate();available=int(next(l.split()[1] for l in P('/proc/meminfo').read_text().splitlines() if l.startswith('MemAvailable:')))*1024;assert available-(4<<30)>(96<<30),'CHUNK_CAPACITY_REFUSAL'
    child=subprocess.Popen(['/home/dnola/humming_env/bin/python','-u',__file__,'worker',str(offset)],stdout=log,stderr=subprocess.STDOUT,user=user.pw_uid,group=user.pw_gid,env=env,cwd=R);identity=dict(pid=child.pid,startticks=coord.ticks(child.pid),started=time.time(),offset=offset);save(R/'ACTIVE_WORKER.json',identity);save(R/f'WORKER_{offset:04d}.json',identity)
