...
P=pathlib.Path
B=P('/home/dnola/missions/DS4_EMPTY_FIT_t_68e5892b'); R=B/'s8-K3-L011DOWN3-run8495r3'; CAN=B/'canonical-39da421e-run8495'; C=P('/home/dnola/HOST_CLAIM.json'); PLAN=B/'PLAN_TRIAL_run8495r3.json'
...
def worker():
 ...
 gate();begin=time.time();results=main_batch(cps,R,11,kernel_cache_root=R/'kernel-cache');end=time.time();coord.verify_imports(mods(),closure);save(R/'POST_SOLVE_IMPORTS.json',mods());accepted=[]
...
def supervisor():
 ...
 prev=P(prior['mission']);assert load(prev/'SHARDS.json')==plan['prior_shards'];assert sha(prior['terminal_path'])==prior['terminal_sha256'];assert load(prior['terminal_path'])['status']=='FAIL' and not (prev/'ADMISSIONS.jsonl').exists() and not (prev/'solve').exists();aw=load(prev/'ACTIVE_WORKER.json');assert coord.ticks(aw['pid']) is None
