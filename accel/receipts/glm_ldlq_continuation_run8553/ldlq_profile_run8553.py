...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'ldlq_profile_run8553'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='8b2c8543c0bb038dadca5b03478bef4e016a2147'
...
if len(sys.argv)==1:
 ...
 put(OUT/'AUTHORITY.json',dict(card_comment='1788920954 explicit bs01 dedicated s6 supersedes stale remote allocation',driver_text=Path('/home/dnola/missions/DRIVER_GOALS.md').read_text(),script_sha256=sha(__file__)))
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1829105 and old['start_ticks']==8262312
 ...
 try:
  for tarname,dirname in [('ldlq_profile_pin_run8553.tar','ldlq_profile_code_run8553'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  jobs=[('P1','candidate')];rows=[]
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'ldlq_profile_code_run8553/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'ldlq_profile_code_run8553/banana-smasher/tests'
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(tests/'test_qtip_ldlq_profile.py'),*[str(p) for p in tests.glob('test_qtip_batch*.py')],'-q']))
sys.path.insert(0,str((ROOT/'ldlq_profile_code_run8553' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
...
rows=[];pin=CAND
for phase in ['setup','profile']:
 for group,cells in [('GROUP_DOWN',[(242,'down'),(243,'down')])]:
  D=OUT/name/phase/group;D.mkdir(parents=True);paths=[]
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();cfg=json.loads((CONTROL/f'B1/warm/{cell}/CONFIG.json').read_text());assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True;cfg['block_ldl_reference']=True
  ...
  dest=D/'build';dest.mkdir();put(dest/'SHARDS.json',json.loads((ROOT/'SHARDS.json').read_text()));torch.cuda.synchronize();torch.cuda.reset_peak_memory_stats();t=time.perf_counter();
  if phase=='profile':
   with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA],record_shapes=True) as prof:
    result=main_batch(paths,dest,4)
   prof.export_chrome_trace(str(OUT/'TRACE.json'));put(OUT/'PROFILE.json',dict(events=[dict(key=e.key,count=e.count,cpu_us=e.cpu_time_total,self_cpu_us=e.self_cpu_time_total,device_us=e.device_time_total,self_device_us=e.self_device_time_total,input_shapes=e.input_shapes) for e in prof.key_averages(group_by_input_shape=True)],scope='Instrumented distinct bottleneck profile; NOT speed denominator'))
  else:result=main_batch(paths,dest,4)
  torch.cuda.synchronize();rows.append(dict(phase=phase,group=group,cells=cells,seconds=time.perf_counter()-t,result=result,peak_allocated=torch.cuda.max_memory_allocated(),peak_reserved=torch.cuda.max_memory_reserved()));put(OUT/name/'RESULT.json',dict(pin=pin,rows=rows,scope='unitwise 2-down grouped plus single fused through main_batch vs retained singleton controls; no cold ratio'))
