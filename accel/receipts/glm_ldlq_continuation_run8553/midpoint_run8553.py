...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'midpoint_run8553'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='13f5fb4109ed509dd8d1d87ed7c75839ac8b6717'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1846515 and old['start_ticks']==8676361
 ...
 try:
  for tarname,dirname in [('midpoint_pin_run8553.tar','midpoint_code_run8553'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'midpoint_code_run8553/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'midpoint_code_run8553/banana-smasher/tests'
...
sys.path.insert(0,str((ROOT/'midpoint_code_run8553' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
