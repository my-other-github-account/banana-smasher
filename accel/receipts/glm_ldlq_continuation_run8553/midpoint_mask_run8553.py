...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'midpoint_mask_run8553'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='ccf11b1c440ed3256f1b4b3fa36a34c3f0a4ab37'
...
if len(sys.argv)==1:
 ...
 put(OUT/'AUTHORITY.json',dict(card_comment='1788920954 explicit dedicated bs01 s6 supersedes stale remote allocation',driver_text=Path('/home/dnola/missions/DRIVER_GOALS.md').read_text(),script_sha256=sha(__file__),hypothesis='First pass official consumer reads only state64. Preserve full forward and branch arithmetic; predicate unused stores0..63 without dynamic control-flow branch and traceback63..0, retain all physical allocation and memory gates. Frozen clean-fit SSE ratio<=1.0001; no tuning from heldout.'))
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1848757 and old['start_ticks']==8701383
 ...
 try:
  for tarname,dirname in [('midpoint_mask_pin_run8553.tar','midpoint_mask_code_run8553'),('traceback_base_run8542.tar','traceback_base_run8542')]:
   code=ROOT/dirname
   if not code.exists():
    code.mkdir()
  ...
  for name,arm in jobs:
   env=dict(os.environ,TRITON_CACHE_DIR='/dev/shm/t_ebcba52e_ldlq_run8503/cache/triton',BANANA_SMASHER_KERNEL_CACHE='/dev/shm/t_ebcba52e_ldlq_run8503/cache/kernels',XDG_CACHE_HOME='/dev/shm/t_ebcba52e_ldlq_run8503/cache/xdg',PYTHONPATH=str(ROOT/'midpoint_mask_code_run8553/banana-smasher/src'))
...
if name=='gpu_tests':
 tests=ROOT/'midpoint_mask_code_run8553/banana-smasher/tests'
 raise SystemExit(subprocess.call([sys.executable,'-m','pytest',str(tests/'test_qtip_midpoint_seed.py'),*[str(p) for p in tests.glob('test_qtip_batch*.py')],'-q']))
sys.path.insert(0,str((ROOT/'midpoint_mask_code_run8553' if arm=='candidate' else ROOT/'traceback_base_run8542')/'banana-smasher/src'))
import torch
...
for phase in ['setup','warm']:
 for group,cells in [('GROUP_DOWN',[(242,'down'),(243,'down')]),('GROUP_FUSED',[(242,'fused13')])]:
  ...
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();cfg=json.loads((CONTROL/f'B1/warm/{cell}/CONFIG.json').read_text());assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True;cfg['block_ldl_reference']=True;cfg['viterbi_midpoint_seed']=True
