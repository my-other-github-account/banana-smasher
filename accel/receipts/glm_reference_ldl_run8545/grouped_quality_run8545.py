...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'grouped_quality_run8545'; PRODUCTS=ROOT/'grouped_run8544r2'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='a2a7df42114ace02c39454963abe05f588eefac5'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1818777 and old['start_ticks']==8181912
...
if name=='quality':
 runner=Path(sp.__file__).with_name('qtip_runner.py');qv=sp._load_public_qtip_runner(runner,sha(runner));cfg=json.loads((CONTROL/'B1/warm/E242_down/CONFIG.json').read_text());qv.QTIP=Path(cfg['qtip_root']);bits,ldlq,math,kd=qv.load_official_qtip();start=time.perf_counter();rows=json.loads((PRODUCTS/'QUALITY.json').read_text())['rows'];done={(r['cell'],r['arm'],r['phase']) for r in rows}
