...
ROOT=Path('/dev/shm/t_1269dc5f');OUT=ROOT/'grouped_run8544r2'; CONTROL=ROOT/'traceback_run8542r3';OLD=Path('/dev/shm/t_182fbc9d');CLAIM=Path('/home/dnola/HOST_CLAIM.json');BASE='2a118a825ef00ff67dd0cbc65d07cc67a2919089';CAND='a2a7df42114ace02c39454963abe05f588eefac5'
...
if len(sys.argv)==1:
 ...
 with Path('/home/dnola/HOST_CLAIM.lock').open('a+') as lock:
  ...
  assert old['pid']==1817550 and old['start_ticks']==8173194
 ...
 try:
  ...
  jobs=[('C1','candidate'),('C2','candidate'),('quality',None)];rows=[]
...
for phase in ['setup','warm']:
 for group,cells in [('GROUP_DOWN',[(242,'down'),(243,'down')]),('GROUP_FUSED',[(242,'fused13')])]:
  ...
  for expert,projection in cells:
   cell=f'E{expert}_{projection}';cd=D/cell;cd.mkdir();cfg=json.loads((CONTROL/f'B1/warm/{cell}/CONFIG.json').read_text());assert sha(cfg['input_identity']['model_index']['path'])==json.loads((ROOT/'SHARDS.json').read_text())['intended_basis'];runner=Path(sp.__file__).with_name('qtip_runner.py');cfg['qtip_runner']=str(runner);cfg['block_ldl_unitwise']=True
